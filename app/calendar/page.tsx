...
export default function CalendarPage() {return (<AuthGuard>
 <CalendarContent />
 </AuthGuard>)}

function CalendarContent() {const router = useRouter()
 const {user} = useAuth()
 ...
 // Filter appointments
 const filteredAppointments = appointments.filter(apt => {// Filter by patient
 ...
 filteredAppointments.forEach(apt => {const aptDate = new Date(apt.dateTime)
 if (aptDate.getMonth() === month && aptDate.getFullYear() === year) {const day = aptDate.getDate()
 ...
 const getDriverStatusColor = (status: string) => {switch (status) {case'accepted':
 ...
 const isToday = (day: number) => {return day === today.getDate() &&
 month === today.getMonth() &&
 year === today.getFullYear()}

 return (<div className="min-h-screen bg-background">
 <PageHeader
 title="Family Calendar"subtitle="View all appointments for your family"backHref="/medical"/>
 ...
 key={index}
 className={`min-h-[120px] border-r border-b border-border p-2 ${day ?'bg-card' :'bg-background/50'}`}
 >
 {day && (<>
 ...
 {appointmentsByDay[day]?.slice(0, 3).map(apt => (<button
 key={apt.id}
 onClick={() => router.push(`/appointments/${apt.id}`)}
 className="w-full text-left px-2 py-1 rounded text-xs bg-primary-light hover:bg-primary-light /30 transition-colors border-l-2 border-purple-500">
 <div className="font-medium text-foreground truncate">
 {formatTime(apt.dateTime)} - {apt.patientName}
 </div>
 {apt.requiresDriver && apt.assignedDriverName && (<div className="flex items-center gap-1 mt-0.5">
 ...
 {appointmentsByDay[day]?.length > 3 && (<div className="text-xs text-muted-foreground px-2">

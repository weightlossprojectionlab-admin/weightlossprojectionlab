...
export default function MedicationsPage() {return (<AuthGuard>
 <MedicationsContent />
 </AuthGuard>)}

function MedicationsContent() {const {user} = useAuth()
 ...
 useEffect(() => {if (selectedPatientId) {loadMedications(selectedPatientId)}}, [selectedPatientId])

 const loadPatients = async () => {try {const data = await medicalOperations.patients.getPatients()
 setPatients(data)

 // Auto-select patient (priority: primary preference > self > first)
 if (data.length > 0) {let defaultPatient: PatientProfile | undefined

 // Priority 1: Use primaryPatientId from user preferences
 if (profile?.preferences?.primaryPatientId) {defaultPatient = data.find(p => p.id === profile.preferences.primaryPatientId)}

 // Priority 2: Fall back to"Self"patient
 if (!defaultPatient) {defaultPatient = data.find(p => p.relationship ==='self')}

 // Priority 3: Use first patient in list
 if (!defaultPatient) {defaultPatient = data[0]}

 if (defaultPatient) {setSelectedPatientId(defaultPatient.id)}}} catch (error) {logger.error('[Medications Page] Error loading patients', error as Error)
 toast.error('Failed to load patients')} finally {setLoading(false)}}

 const loadMedications = async (patientId: string) => {try {const data = await medicalOperations.medications.getMedications(patientId)
 setMedications(data)} catch (error) {logger.error('[Medications Page] Error loading medications', error as Error)
 toast.error('Failed to load medications')}}

 // Get unique conditions for filtering
 const uniqueConditions = useMemo(() => {const conditions = new Set<string>()
 medications.forEach((med: PatientMedication) => {if (med.prescribedFor) {conditions.add(med.prescribedFor)}})
 return Array.from(conditions).sort()}, [medications])

 // Filter medications by condition
 const filteredMedications = useMemo(() => {if (!filterByCondition) return medications
 return medications.filter(med => med.prescribedFor === filterByCondition)}, [medications, filterByCondition])

 // Handle scanned medication
 const handleMedicationScanned = async (scannedData: ScannedMedication) => {if (!selectedPatientId) {toast.error('No patient selected')
 return}

 try {await medicalOperations.medications.addMedication(selectedPatientId, {name: scannedData.name,
 ...
 // Reload medications
 await loadMedications(selectedPatientId)} catch (error: any) {logger.error('[Medications Page] Error adding medication', error)
 ...
 const handleEditMedication = (medication: PatientMedication) => {setEditingMedication(medication)
 setShowEditModal(true)}

 // Save edited medication
 const handleSaveEdit = async (updates: Partial<PatientMedication>) => {if (!selectedPatientId || !editingMedication) return

 try {await medicalOperations.medications.updateMedication(selectedPatientId,
 ...
 // Reload medications
 await loadMedications(selectedPatientId)} catch (error) {logger.error('[Medications Page] Error updating medication', error as Error)
 toast.error('Failed to update medication')}}

 // Delete medication handler
 const handleDeleteMedication = async (medicationId: string, medicationName: string) => {if (!selectedPatientId) return

 if (!confirm(`Are you sure you want to delete ${medicationName}?`)) {return}

 try {await medicalOperations.medications.deleteMedication(selectedPatientId, medicationId)
 toast.success(`${medicationName} removed successfully`)

 // Reload medications
 await loadMedications(selectedPatientId)} catch (error) {logger.error('[Medications Page] Error deleting medication', error as Error)
 ...
 <PageHeader
 title="Medication Management"subtitle={selectedPatient ?`Medications for ${selectedPatient.name}`:'Track and manage medications'}
 actions={<button
 ...
 {/* Filter Bar */}
 {uniqueConditions.length > 0 && (<div className="bg-card rounded-lg shadow-sm p-4 mb-6">
 ...
 {uniqueConditions.map(condition => (<option key={condition} value={condition}>
 {condition}
 </option>))}
 </select>
 </div>
 ...
 {/* Clear Filters */}
 {filterByCondition && (<button
 onClick={() => setFilterByCondition(null)}
 className="px-3 py-1.5 text-sm text-primary hover:text-primary-dark font-medium underline">
 Clear Filter
 </button>)}
 </div>
 ...
 {/* Medications Display */}
 {medications.length === 0 ? (<div className="bg-card rounded-lg shadow-sm p-12 text-center">
 ...
 </div>) : filteredMedications.length === 0 ? (<div className="bg-card rounded-lg shadow-sm p-12 text-center">
 <div className="text-6xl mb-4">🔍</div>
 ...
 </div>) : (<div className="bg-card rounded-lg shadow-sm p-6">
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
 {filteredMedications.map((med: PatientMedication) => (<MedicationCard
 key={med.id}
 medication={med}
 ...
 {showEditModal && editingMedication && selectedPatientId && (<EditMedicationModal

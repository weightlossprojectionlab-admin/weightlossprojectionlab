...
export default function WeightHistoryPage() {return (<AuthGuard>
 <WeightHistoryContent />
 </AuthGuard>)}

type TimeRange ='7d' |'30d' |'90d' |'all'

function WeightHistoryContent() {const router = useRouter()
 const {user} = useAuth()
 ...
 if (timeRange !=='all') {const days = timeRange ==='7d' ? 7 : timeRange ==='30d' ? 30 : 90
 ...
 (snapshot) => {const logs: WeightLog[] = snapshot.docs.map(doc => {const data = doc.data()
 return {id: doc.id,
 userId: user.uid,
 weight: data.weight,
 unit: data.unit,
 ...
 setWeightLogs(logs)
 setLoading(false)},
 (error) => {logger.error('Error in weight logs snapshot', error as Error)
 toast.error('Failed to load weight history')
 setLoading(false)})

 // Cleanup subscription on unmount
 return () => unsubscribe()} catch (error) {logger.error('Error setting up weight logs listener', error as Error)
 toast.error('Failed to load weight history')
 setLoading(false)}}, [user, timeRange])

 const handleExportCSV = () => {if (weightLogs.length === 0) {toast.error('No data to export')
 return}

 // Create CSV content
 const headers = ['Date','Weight','Unit','Notes']
 const rows = weightLogs.map(log => [new Date(log.loggedAt).toLocaleDateString(),
 log.weight.toString(),
 log.unit,
 log.notes ||''])
 ...
 link.href = url
 link.download =`weight-history-${new Date().toISOString().split('T')[0]}.csv`document.body.appendChild(link)
 link.click()
 ...
 // Calculate stats
 const stats = {totalEntries: weightLogs.length,
 ...
 if (stats.totalEntries >= 2) {stats.totalChange = stats.currentWeight - stats.startWeight}

 return (<div className="min-h-screen bg-background">
 <PageHeader
 title="Weight History"subtitle="Track your weight progress over time"backHref="/dashboard"actions={<div className="relative">
 <button
 onClick={() => setShowExportMenu(!showExportMenu)}
 className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium">
 Export
 </button>
 {showExportMenu && (<div className="absolute right-0 mt-2 w-48 bg-background rounded-lg shadow-lg border border-border z-10">
 <button
 ...
 {/* Summary Stats */}
 {weightLogs.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
 ...
 <p className={`text-2xl font-bold ${stats.totalChange < 0 ?'text-success' :
 stats.totalChange > 0 ?'text-error' :'text-foreground'}`}>
 ...
 {(['7d','30d','90d','all'] as TimeRange[]).map((range) => (<button
 key={range}
 ...
 <Link
 href="/log-weight"className="px-4 py-2 bg-primary-light text-primary-dark rounded-lg hover:bg-purple-200 transition-colors text-sm font-medium">
 + Add Entry
 </Link>
 </div>
 ...
 {/* Best Practices Tip */}
 {weightLogs.length === 0 && !loading && (<div className="mt-6 bg-primary-light border border-primary-light rounded-lg p-6">
 ...
 <Link
 href="/log-weight"className="inline-block mt-4 px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors font-medium text-sm">

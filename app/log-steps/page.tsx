...
interface StepLogData {id: string
 steps: number
 date: string
 loggedAt: string
 source: string}

function LogStepsContent() {// Get automatic tracking status from provider
 ...
 useEffect(() => {const fetchRecentLogs = async () => {try {const response = await stepLogOperations.getStepLogs({limit: 7})
 setRecentLogs(response.data || [])} catch (error) {logger.error('Error fetching recent logs:', error as Error)} finally {setLoadingLogs(false)}}

 fetchRecentLogs()}, [])

 const handleToggleTracking = async () => {setTogglingTracking(true)
 try {if (isEnabled) {disableTracking()
 toast.success('Automatic tracking disabled')} else {await enableTracking()
 toast.success('Automatic tracking enabled! Your steps will be counted in the background.')}} catch (err) {toast.error('Failed to toggle tracking. Please check device permissions.')} finally {setTogglingTracking(false)}}

 const handleManualSave = async () => {setSavingProgress(true)
 try {await manualSave()
 toast.success('Progress saved successfully!')
 // Refresh recent logs
 const response = await stepLogOperations.getStepLogs({limit: 7})
 setRecentLogs(response.data || [])} catch (err) {const message = err instanceof Error ? err.message :'Failed to save progress'
 toast.error(message)} finally {setSavingProgress(false)}}

 // Format relative date (Today, Yesterday, X days ago)
 const formatRelativeDate = (dateString: string) => {const logDate = new Date(dateString)
 ...
 <Link
 href="/dashboard"className="text-primary hover:text-primary-hover"aria-label="Back to dashboard">
 ← Back
 </Link>
 ...
 <div className={`border rounded-lg p-6 shadow-sm ${isEnabled ?'bg-success-light /20 border-success' :'bg-card border-border'}`}>
 <div className="flex items-center justify-between mb-4">
 <h2 className="text-lg font-medium text-foreground">Today's Steps</h2>
 {isEnabled && isTracking && (<span className="flex items-center space-x-2 text-sm text-success-dark">
 ...
 <div
 className="bg-primary h-3 rounded-full transition-all duration-300"style={{width:`${progressPercentage}%`}}
 ...
 {currentSteps >= goalSteps ? (<span className="text-success font-medium">🎉 Goal achieved!</span>) : (<span>
 ...
 {isEnabled ? (<div className="space-y-3">
 <div className="bg-success-light /20 border border-success rounded-lg p-3">
 ...
 {/* Save Progress Info */}
 {currentSteps > lastSaveStepCount && (<div className="bg-secondary-light border border-secondary-light rounded-lg p-3">
 ...
 {/* Manual Save Button */}
 {currentSteps > lastSaveStepCount && (<button
 ...
 {loadingLogs ? (<div className="text-center py-8">
 ...
 </div>) : recentLogs.length === 0 ? (<div className="text-center py-8">
 ...
 {recentLogs.map((log, index) => {const goalProgress = Math.round((log.steps / goalSteps) * 100)
 ...
 {goalProgress >= 100 ? (<span className="text-sm text-success">✓ Goal reached</span>) : goalProgress >= 75 ? (<span className="text-sm text-accent-dark">{goalProgress}% of goal</span>) : (<span className="text-sm text-warning">{goalProgress}% of goal</span>)}
 </div>
 </div>)})}
 </div>

 <Link
 href="/dashboard"className="inline-flex items-center mt-4 text-sm text-primary hover:text-primary-hover"aria-label="View full activity history">
 View full history →
 </Link>
 </>)}
 </div>
...
export default function LogStepsPage() {return (<AuthGuard>

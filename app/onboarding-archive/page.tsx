...
import {calculateBMR,
 ...
 type ActivityLevel,
 type Gender,
 type Units} from'@/lib/health-calculations'
import {calculateAge,
 validateAge,
 ...
 formatDateForInput,
 getMaxBirthDate,
 getMinBirthDate} from'@/lib/age-utils'
import {Spinner} from'@/components/ui/Spinner'
import type {UserProfile, UserGoals, UserPreferences} from'@/types'
import {logger} from'@/lib/logger'

interface OnboardingData {// Step 1: About You
 birthDate?: Date
 ...
 mealSchedule?: {breakfastTime: string
 lunchTime: string
 dinnerTime: string
 hasSnacks: boolean
 snackWindows?: string[]}
 notifications?: boolean
 mealReminderTimes?: {breakfast?: string
 lunch?: string
 dinner?: string
 snacks?: string}
 weightCheckInFrequency?:'daily' |'weekly' |'biweekly' |'monthly'}

function OnboardingContent() {const router = useRouter()
 const {user} = useAuth()
 const [currentStep, setCurrentStep] = useState(1)
 const [data, setData] = useState<OnboardingData>({units:'imperial',
 notifications: true,
 weightCheckInFrequency:'weekly',
 // Meal schedule defaults (7 AM, 12 PM, 6 PM)
 mealSchedule: {breakfastTime:'07:00',
 lunchTime:'12:00',
 dinnerTime:'18:00',
 hasSnacks: false,
 snackWindows: []},
 ...
 setValidationErrors(prev => {const newErrors = {...prev}
 updatedFields.forEach(field => {delete newErrors[field]})
 return newErrors})}

 // Load draft data from Firebase when user is authenticated
 useEffect(() => {if (!user) {logger.debug('User not authenticated yet, waiting')
 return}

 if (draftLoaded) {logger.debug('Draft already loaded, skipping')
 return}

 const loadDraft = async () => {try {logger.info('User authenticated, loading onboarding draft from Firebase', {uid: user.uid})
 const profile = await userProfileOperations.getUserProfile()
 logger.debug('Profile data from Firebase', {profile})

 if (profile && !profile.profile?.onboardingCompleted) {// User has draft data - load it
 ...
 if (profile.profile?.height) {draftData.height = profile.profile.height
 // Try to extract feet/inches if imperial
 const units = profile.preferences?.units ||'imperial'
 if (units ==='imperial' && profile.profile.height) {draftData.heightFeet = Math.floor(profile.profile.height / 12)
 ...
 toast.success(`📋 Resumed from Step ${resumeStep}`, {duration: 3000})} else if (profile && profile.profile?.onboardingCompleted) {logger.info('Onboarding already completed, redirecting to dashboard')
 toast.success('Welcome back! Redirecting to dashboard...', {duration: 2000})
 router.push('/dashboard')
 return} else {logger.info('No profile found, starting fresh onboarding')}} catch (error: any) {logger.error('Error loading draft', error as Error)
 // Don't show error to user - just start fresh} finally {setDraftLoaded(true)}}

 loadDraft()}, [user, router]) // Re-run when user changes

 // Save current step data to Firebase
 const saveStepData = async (stepToSave?: number) => {try {const partialUpdate: any = {}
 ...
 partialUpdate.profile = {onboardingCompleted: false,
 ...
 if (data.targetWeight || data.primaryGoal || data.dailyCalorieGoal) {partialUpdate.goals = {}
 ...
 if (data.units || data.dietaryPreferences || data.notifications !== undefined || data.mealSchedule) {partialUpdate.preferences = {}
 ...
 if (Object.keys(partialUpdate).length > 0) {logger.debug('Saving step data', {step: stepNumber,
 ...
 logger.info('Auto-saved onboarding progress to Firebase', {step: stepNumber})}} catch (error) {logger.error('Error auto-saving onboarding', error as Error, {step: stepToSave !== undefined ? stepToSave : currentStep,
 ...
 const getFieldError = (field: string): string | undefined => {return validationErrors[field]}

 const isFieldInvalid = (field: string): boolean => {return !!validationErrors[field]}

 const setFieldError = (field: string, message: string) => {setValidationErrors(prev => ({...prev, [field]: message}))}

 const clearValidationErrors = () => {setValidationErrors({})}

 const nextStep = async () => {// Validate current step before proceeding
 if (validateStep(currentStep)) {setLoading(true)
 try {// Clear warnings when moving to next step
 setWarnings([])
 ...
 if (currentStep === 3) {// Calculate targets after goals step
 calculateAndSetTargets()}
 setCurrentStep(nextStepNumber)} finally {setLoading(false)}}}

 const prevStep = async () => {// Calculate previous step number
 ...
 const validateStep = (step: number): boolean => {// Clear previous errors
 ...
 switch (step) {case 1:
 // Check birthday
 if (!data.birthDate) {setFieldError('birthDate','Birthday is required')
 missingFields.push('Birthday')
 isValid = false} else {// Validate age from birthdate
 const ageValidation = validateAge(data.birthDate)
 if (!ageValidation.valid) {setFieldError('birthDate', ageValidation.error!)
 ...
 // Check gender
 if (!data.gender) {setFieldError('gender','Gender is required')
 missingFields.push('Gender')
 isValid = false}

 // Check height
 if (!data.height) {setFieldError('height','Height is required')
 missingFields.push('Height')
 isValid = false}

 if (!isValid) {toast.error(`Please complete: ${missingFields.join(',')}`, {duration: 5000})}
 return isValid

 case 2:
 // Check current weight
 if (!data.currentWeight) {setFieldError('currentWeight','Current weight is required')
 missingFields.push('Current Weight')
 isValid = false}

 // Check activity level
 if (!data.activityLevel) {setFieldError('activityLevel','Activity level is required')
 ...
 case 3:
 // Check primary goal
 if (!data.primaryGoal) {setFieldError('primaryGoal','Primary goal is required')
 missingFields.push('Primary Goal')
 isValid = false}

 // Check target weight
 if (!data.targetWeight) {setFieldError('targetWeight','Target weight is required')
 missingFields.push('Target Weight')
 isValid = false}

 // Check weekly weight loss goal
 if (!data.weeklyWeightLossGoal) {setFieldError('weeklyWeightLossGoal','Weekly weight loss goal is required')
 missingFields.push('Weekly Goal')
 isValid = false}

 // Validate target weight for weight loss goal
 if (data.primaryGoal ==='lose-weight' && data.targetWeight && data.currentWeight && data.targetWeight >= data.currentWeight) {setFieldError('targetWeight','Target weight must be less than current weight')
 ...
 const calculateAndSetTargets = () => {if (!data.age || !data.gender || !data.height || !data.currentWeight || !data.activityLevel || !data.units) {return}

 // Clear previous warnings
 setWarnings([])

 // Calculate BMR
 const bmr = calculateBMR({weight: data.currentWeight,
 height: data.height,
 age: data.age,
 gender: data.gender,
 units: data.units})
 ...
 const lifestyleImpact = calculateLifestyleImpact({baseTDEE,
 ...
 lifestyleImpact.warnings.forEach(warning => {setWarnings(prev => [...prev, warning])})

 // Calculate daily calorie target with lifestyle-adjusted TDEE
 let dailyCalorieGoal = calculateCalorieTarget({tdee: adjustedTDEE,
 weeklyWeightLossGoal: data.weeklyWeightLossGoal || 1,
 units: data.units})

 // If heavy drinker, reduce calorie budget to account for alcohol
 if (lifestyleImpact.alcoholCaloriesPerDay > 150) {dailyCalorieGoal -= lifestyleImpact.alcoholCaloriesPerDay
 setWarnings(prev => [...prev,`Your meal calorie budget is reduced by ${Math.round(lifestyleImpact.alcoholCaloriesPerDay)} cal/day to account for alcohol consumption`])}
 ...
 if (data.healthConditions?.includes('Type 2 Diabetes')) {minCalories = Math.max(minCalories, 1400)
 setWarnings(prev => [...prev,'Diabetics require higher minimum calorie intake (1400 cal/day) for blood sugar stability'])}
 if (data.healthConditions?.includes('Heart Disease')) {minCalories = Math.max(minCalories, 1500)
 setWarnings(prev => [...prev,'Heart disease patients require higher minimum calorie intake (1500 cal/day) for safety'])}
 if (data.healthConditions?.includes('Pregnancy/Nursing')) {minCalories = 1800
 ...
 dailyCalorieGoal = Math.max(minCalories,
 Math.min(calorieRange.max, dailyCalorieGoal))

 // Warn user if goal was adjusted for safety
 if (dailyCalorieGoal !== originalGoal && dailyCalorieGoal > originalGoal) {const adjustmentMessage =`Your calorie goal was increased to ${dailyCalorieGoal} calories for your health and safety. ${calorieRange.warning ||'This ensures you maintain adequate nutrition.'}`setWarnings(prev => [...prev, adjustmentMessage])}
 ...
 updateData({dailyCalorieGoal,
 macroTargets: {protein: macroPercentages.protein,
 ...
 const handleComplete = async () => {setLoading(true)
 try {// Prepare profile data
 // IMPORTANT: Convert Date objects to ISO strings for Zod validation
 const profile: Partial<UserProfile> = {birthDate: data.birthDate!.toISOString() as unknown as Date,
 age: data.age!,
 gender: data.gender!,
 height: data.height!,
 ...
 lifestyle: {smoking: data.smoking ||'never',
 ...
 // Prepare goals data
 const goals: Partial<UserGoals> = {targetWeight: data.targetWeight!,
 ...
 macroTargets: {protein: data.macroTargets!.protein,
 carbs: data.macroTargets!.carbs,
 fat: data.macroTargets!.fat}}

 // Prepare preferences data
 const preferences: Partial<UserPreferences> = {units: data.units!,
 ...
 await userProfileOperations.updateUserProfile({profile,
 goals,
 preferences})
 ...
 let verified = false
 for (let attempt = 0; attempt < 3; attempt++) {// Wait before checking (exponential backoff)
 const delay = 500 * Math.pow(2, attempt) // 500ms, 1000ms, 2000ms
 await new Promise(resolve => setTimeout(resolve, delay))

 // Refetch profile to verify onboardingCompleted flag
 try {const savedProfile = await userProfileOperations.getUserProfile()
 const isCompleted = savedProfile?.data?.profile?.onboardingCompleted === true

 logger.info('Onboarding completion verification attempt', {attempt: attempt + 1,
 onboardingCompleted: isCompleted,
 delayMs: delay,
 timestamp: new Date().toISOString()})

 if (isCompleted) {verified = true
 logger.info('✅ Onboarding completion verified successfully')
 break}} catch (verifyError) {logger.warn('Verification attempt failed', {attempt: attempt + 1,
 error: verifyError instanceof Error ? verifyError.message : String(verifyError)})}}

 if (!verified) {logger.error('Failed to verify onboarding completion after 3 attempts')
 ...
 try {logger.info('Creating initial weight log from onboarding data')
 await weightLogOperations.createWeightLog({weight: data.currentWeight!,
 unit: data.units ==='metric' ?'kg' :'lbs',
 notes:'Starting weight from onboarding',
 loggedAt: new Date().toISOString()})
 logger.info('Initial weight log created successfully')} catch (weightLogError) {// Don't block onboarding completion if weight log fails
 ...
 if (data.healthConditions && data.healthConditions.length > 0) {try {logger.info('Generating AI health profile from onboarding data', {conditions: data.healthConditions,
 ...
 const response = await fetch('/api/ai/health-profile/generate', {method:'POST',
 headers: {'Authorization':`Bearer ${token}`,'Content-Type':'application/json'}})

 if (response.ok) {const result = await response.json()
 logger.info('AI health profile generated successfully', {status: result.status,
 confidence: result.confidence,
 restrictionsCount: result.restrictionsCount})

 // Show appropriate message based on status
 if (result.status ==='approved') {toast.success('AI health profile created with dietary recommendations')} else {toast('Health profile pending admin review', {icon:'⏳'})}} else {// Parse error response for detailed logging
 let errorDetails: any = {status: response.status, statusText: response.statusText}
 try {const errorBody = await response.json()
 errorDetails = {...errorDetails, ...errorBody}} catch {// If JSON parsing fails, just use status info}

 throw new Error(`Health profile generation failed: ${response.status} - ${errorDetails.error || errorDetails.message || response.statusText}`)}} catch (healthProfileError) {// Don't block onboarding completion if health profile generation fails
 logger.warn('Failed to generate AI health profile', {error: healthProfileError instanceof Error ? healthProfileError.message : String(healthProfileError),
 ...
 router.push('/dashboard?onboardingComplete=true')} catch (error) {logger.error('Error completing onboarding', error as Error)
 toast.error('Failed to save your profile. Please try again.')} finally {setLoading(false)}}
 ...
 <div
 className="progress-bar-fill progress-bar-primary transition-all duration-300"style={{width:`${(currentStep / totalSteps) * 100}%`}}
 />
 </div>
 </div>
 ...
 isFieldInvalid={isFieldInvalid}
 />)}
 {currentStep === 4 && (<StepFour data={data} updateData={updateData} warnings={warnings} />)}
 {currentStep === 5 && (<StepFive data={data} updateData={updateData} />)}
 {currentStep === 6 && (<StepSix data={data} updateData={updateData} />)}
 </div>
...
function StepOne({data,
 ...
 getFieldError,
 isFieldInvalid,
 setWarnings}: {data: OnboardingData
 updateData: (data: Partial<OnboardingData>) => void
 getFieldError: (field: string) => string | undefined
 isFieldInvalid: (field: string) => boolean
 setWarnings: React.Dispatch<React.SetStateAction<string[]>>}) {return (<div className="space-y-6">
 ...
 onChange={(e) => {const month = parseInt(e.target.value)
 updateData({birthMonth: month})

 // Construct date if all fields are filled
 if (month && data.birthDay && data.birthYear) {const birthDate = new Date(data.birthYear, month - 1, data.birthDay)
 const validation = validateAge(birthDate)

 if (validation.valid) {updateData({birthDate,
 age: validation.age})

 if (validation.warning) {setWarnings(prev => [...prev, validation.warning!])}}}}}
 ...
 <select
 value={data.birthDay ||''}
 onChange={(e) => {const day = parseInt(e.target.value)
 updateData({birthDay: day})

 // Construct date if all fields are filled
 if (data.birthMonth && day && data.birthYear) {const birthDate = new Date(data.birthYear, data.birthMonth - 1, day)
 const validation = validateAge(birthDate)
 ...
 {(() => {let maxDays = 31
 if (data.birthMonth && data.birthYear) {// Get last day of the month
 maxDays = new Date(data.birthYear, data.birthMonth, 0).getDate()} else if (data.birthMonth) {// Use current year if year not selected
 maxDays = new Date(new Date().getFullYear(), data.birthMonth, 0).getDate()}
 return Array.from({length: maxDays}, (_, i) => i + 1).map(day => (<option key={day} value={day}>{day}</option>))})()}
 </select>
 </div>
 ...
 onChange={(e) => {const year = parseInt(e.target.value)
 updateData({birthYear: year})

 // Construct date if all fields are filled
 if (data.birthMonth && data.birthDay && year) {const birthDate = new Date(year, data.birthMonth - 1, data.birthDay)
 const validation = validateAge(birthDate)

 if (validation.valid) {updateData({birthDate,
 age: validation.age})

 if (validation.warning) {setWarnings(prev => [...prev, validation.warning!])}} else {toast.error(validation.error!)}}}}
 className={`form-input ${isFieldInvalid('birthDate') ?'border-error' :''}`}
 >
 <option value="">Year</option>
 {/* Generate years from current year - 13 (COPPA) down to current year - 120 */}
 {Array.from({length: 108}, (_, i) => new Date().getFullYear() - 13 - i).map(year => (<option key={year} value={year}>{year}</option>))}
 </select>
 </div>
 </div>

 {/* Show calculated age */}
 {data.birthDate && (<p className="text-caption mt-1">
 ...
 {/* Error message */}
 {getFieldError('birthDate') && (<p className="text-error text-xs mt-1">{getFieldError('birthDate')}</p>)}
 </div>

 {/* Age-based health warnings */}
 {data.age && getAgeSpecificHealthNotices(data.age).map((notice, index) => (<div key={index} className={`notification notification-${notice.type}`}>
 ...
 {value:'prefer-not-to-say', label:'Prefer not to say', emoji:'😊'}].map((option) => (<button
 key={option.value}
 type="button"onClick={() => updateData({gender: option.value as Gender})}
 className={`p-4 border-2 rounded-lg text-left transition-all ${data.gender === option.value
 ?'border-primary bg-primary-light /20 ring-2 ring-primary'
 ...
 {/* Error message */}
 {getFieldError('gender') && (<p className="text-error text-xs mt-1">{getFieldError('gender')}</p>)}
 </div>
 ...
 <div className="grid grid-cols-2 gap-3">
 <button
 type="button"onClick={() => updateData({units:'imperial'})}
 className={`p-4 border-2 rounded-lg transition-all ${data.units ==='imperial'
 ?'border-primary bg-primary-light /20 ring-2 ring-primary'
 :'border-border hover:border-primary/50'}`}
 >
 ...
 <button
 type="button"onClick={() => updateData({units:'metric'})}
 className={`p-4 border-2 rounded-lg transition-all ${data.units ==='metric'
 ?'border-primary bg-primary-light /20 ring-2 ring-primary'
 :'border-border hover:border-primary/50'}`}
 >
 ...
 {data.units ==='imperial' ? (// Imperial: Separate feet and inches inputs
 <>
 <div className="flex items-center gap-3">
 <div className="flex-1">
 <div className="flex items-center gap-2">
 <input
 type="number"min="3"max="8"value={data.heightFeet ||''}
 onChange={(e) => {const feet = parseInt(e.target.value) || 0
 ...
 <div className="flex items-center gap-2">
 <input
 type="number"min="0"max="11"value={data.heightInches ||''}
 onChange={(e) => {const inches = parseInt(e.target.value) || 0
 ...
 <div className="flex items-center gap-2">
 <input
 type="number"min="90"max="240"value={data.height ||''}
 ...
 {/* Error message */}
 {getFieldError('height') && (<p className="text-error text-xs mt-1">{getFieldError('height')}</p>)}
 </div>
 </div>)}
...
function StepTwo({data,
 updateData,
 getFieldError,
 isFieldInvalid}: {data: OnboardingData
 updateData: (data: Partial<OnboardingData>) => void
 getFieldError: (field: string) => string | undefined
 isFieldInvalid: (field: string) => boolean}) {return (<div className="space-y-6">
 <div className="text-center mb-6">
 ...
 <input
 type="number"min="50"max="500"step="0.1"value={data.currentWeight ||''}
 onChange={(e) => {const weight = parseFloat(e.target.value)
 ...
 />
 {/* Error message */}
 {getFieldError('currentWeight') && (<p className="text-error text-xs mt-1">{getFieldError('currentWeight')}</p>)}
 </div>
 ...
 {value:'extremely-active', label:'Extremely Active', desc:'Athlete level', emoji:'🏅'}].map((option) => (<button
 key={option.value}
 type="button"onClick={() => updateData({activityLevel: option.value as ActivityLevel})}
 className={`w-full p-4 border-2 rounded-lg text-left transition-all ${data.activityLevel === option.value
 ?'border-primary bg-primary-light /20 ring-2 ring-primary'
 ...
 {data.activityLevel === option.value && (<span className="text-primary">✓</span>)}
 </div>
 </button>))}
 </div>
 {/* Error message */}
 {getFieldError('activityLevel') && (<p className="text-error text-xs mt-1">{getFieldError('activityLevel')}</p>)}
 </div>
 </div>)}
...
function StepThree({data,
 ...
 updateData: (data: Partial<OnboardingData>) => void
 getFieldError: (field: string) => string | undefined
 isFieldInvalid: (field: string) => boolean}) {// Auto-calculate optimal targets when user selects primary goal
 useEffect(() => {if (data.currentWeight &&
 data.height &&
 data.age &&
 data.gender &&
 data.activityLevel &&
 data.primaryGoal) {const optimalTargets = calculateOptimalTargets({currentWeight: data.currentWeight,
 height: data.height,
 age: data.age,
 gender: data.gender,
 activityLevel: data.activityLevel,
 ...
 updateData({targetWeight: optimalTargets.targetWeight,
 weeklyWeightLossGoal: optimalTargets.weeklyWeightLossGoal,
 dailySteps: optimalTargets.dailySteps})}}, [data.primaryGoal, data.healthConditions]) // Recalculate when goal or health conditions change

 const projection = data.currentWeight && data.targetWeight && data.weeklyWeightLossGoal
 ? projectWeightLoss({startWeight: data.currentWeight,
 ...
 ? calculateOptimalTargets({currentWeight: data.currentWeight,
 height: data.height,
 age: data.age,
 gender: data.gender,
 activityLevel: data.activityLevel,
 ...
 {value:'improve-health', label:'Get Healthier', emoji:'❤️'}].map((option) => (<button
 key={option.value}
 type="button"onClick={() => updateData({primaryGoal: option.value as any})}
 className={`p-4 border-2 rounded-lg transition-all ${data.primaryGoal === option.value
 ?'border-primary bg-primary-light /20 ring-2 ring-primary'
 ...
 {/* Error message */}
 {getFieldError('primaryGoal') && (<p className="text-error text-xs mt-1">{getFieldError('primaryGoal')}</p>)}
 </div>

 {/* Auto-Calculated Targets Display */}
 {optimalTargets && (<div className="bg-gradient-to-r from-purple-100 to-blue-50 /20 /20 border-2 border-primary rounded-lg p-5 space-y-4">
 <div className="flex items-start space-x-3">
 <span className="text-2xl">🎯</span>
 <div className="flex-1">
 <h3 className="font-bold text-primary-dark mb-1">Your Optimized Targets</h3>
 <p className="text-xs text-primary-dark/80 /80">
 ...
 {/* Safety Notice */}
 <div className="bg-indigo-100 /20 border border-accent rounded-lg p-3">
 ...
 {/* Projection */}
 {projection && (<div className="bg-muted border-2 border-border rounded-lg p-4">
...
function StepFour({data,
 updateData,
 warnings}: {data: OnboardingData
 updateData: (data: Partial<OnboardingData>) => void
 warnings: string[]}) {// Get the auto-calculated targets and reasoning
 const optimalTargets = data.currentWeight && data.height && data.age && data.gender && data.activityLevel && data.primaryGoal
 ? calculateOptimalTargets({currentWeight: data.currentWeight,
 height: data.height,
 age: data.age,
 ...
 <div className="bg-primary-light /20 border-2 border-primary rounded-lg p-5">
 <label className="text-label block mb-3">
 Daily Calorie Goal <span className="text-xs text-muted-foreground ml-2">(Auto-Calculated)</span>
 </label>
 <div className="flex items-center space-x-4 mb-2">
 <input
 type="number"value={data.dailyCalorieGoal ||''}
 readOnly
 className="form-input flex-1 text-lg font-semibold bg-muted cursor-not-allowed"placeholder="Auto-calculated"/>
 ...
 {optimalTargets && (<div className="bg-card border-2 border-border rounded-lg p-5">
 <label className="text-label block mb-3">
 Daily Steps Goal <span className="text-xs text-muted-foreground ml-2">(Auto-Calculated)</span>
 </label>
 <div className="flex items-center space-x-4 mb-2">
 <input
 type="number"value={optimalTargets.dailySteps}
 readOnly
 ...
 {warnings.length > 0 && (<div className="space-y-3">
 {warnings.map((warning, index) => (<div key={index} className="notification notification-warning">
 ...
 {data.macroTargets && (<div className="grid grid-cols-3 gap-4 text-center">
...
function StepFive({data, updateData}: {data: OnboardingData; updateData: (data: Partial<OnboardingData>) => void}) {const dietaryOptions = ['Vegan','Vegetarian','Keto','Paleo','Gluten-Free','Dairy-Free','Low-Carb','Mediterranean']
 ...
 useEffect(() => {if (healthRisk && healthRisk.likelyConditions.length > 0 && (!data.healthConditions || data.healthConditions.length === 0)) {updateData({healthConditions: healthRisk.likelyConditions})}}, [healthRisk?.bmiCategory]) // Only run when BMI category is calculated

 // Classify medications and suggest conditions when medications change
 useEffect(() => {const classifyMeds = async () => {if (data.medications && data.medications.length > 0 && !isClassifyingMeds) {setIsClassifyingMeds(true)
 try {const classifications = await classifyMedicationConditions(data.medications)
 const suggested = getSuggestedConditions(classifications)

 // Normalize condition names to match app's condition list
 const normalizedSuggestions = suggested.map(s => ({...s,
 ...
 if (highConfidenceConditions.length > 0) {// Merge with existing conditions (don't override user selections)
 const mergedConditions = Array.from(new Set([...(data.healthConditions || []),
 ...highConfidenceConditions]))
 updateData({healthConditions: mergedConditions})

 toast.success(`Based on your medications, WLPL identified ${highConfidenceConditions.length} condition(s)`)}} catch (error) {logger.error('[Onboarding] Medication classification failed', error as Error)} finally {setIsClassifyingMeds(false)}}}

 classifyMeds()}, [data.medications?.length]) // Re-run when medication count changes

 const toggleDietaryPref = (pref: string) => {const current = data.dietaryPreferences || []
 const updated = current.includes(pref) ? current.filter(p => p !== pref) : [...current, pref]
 updateData({dietaryPreferences: updated})}

 const toggleAllergy = (allergy: string) => {const current = data.foodAllergies || []
 const updated = current.includes(allergy) ? current.filter(a => a !== allergy) : [...current, allergy]
 updateData({foodAllergies: updated})}

 const toggleHealthCondition = (condition: string) => {const current = data.healthConditions || []
 const isSelected = current.includes(condition)

 if (!isSelected) {// User is selecting the condition
 const updated = [...current, condition]
 updateData({healthConditions: updated})

 // Check if this condition has a detailed questionnaire
 if (hasDetailedQuestionnaire(condition)) {// Open modal for detailed questions
 setSelectedCondition(condition)
 setModalOpen(true)}} else {// User is deselecting the condition
 ...
 const handleModalSave = (conditionKey: string, responses: Record<string, any>) => {// Save responses for this condition
 const updatedResponses = {...conditionResponses,
 ...
 Object.entries(updatedResponses).forEach(([condition, conditionData]) => {// Look for medication-scanner question responses
 Object.entries(conditionData).forEach(([questionId, value]) => {// Check if this is a medication array
 if (Array.isArray(value) && value.length > 0 && value[0]?.name) {// This is a medications array from a medication-scanner question
 ...
 <div className="bg-primary-light /20 border-2 border-primary rounded-lg p-4 mt-4">
 <h3 className="font-bold text-primary-dark mb-2">You're Seen</h3>
 ...
 <div className="bg-error-light /20 border-2 border-error rounded-lg p-4 mt-4">
 ...
 {bmiData && healthRisk && (<div className="bg-gradient-to-r from-orange-50 to-red-50 /20 /20 border-2 border-orange-400 rounded-lg p-5">
 <div className="flex items-start space-x-3">
 ...
 {healthRisk.warnings.map((warning, i) => (<p key={i} className="text-xs text-orange-700">• {warning}</p>))}
 </div>
 </div>
 </div>
 </div>)}
 ...
 {value:'current-heavy', label:'Yes, 10+ cigarettes/day (1+ pack)'}].map((option) => (<button
 key={option.value}
 type="button"onClick={() => updateData({smoking: option.value as any})}
 className={`w-full p-3 border-2 rounded-lg text-left transition-all text-sm ${data.smoking === option.value
 ?'border-primary bg-primary-light /20 ring-2 ring-primary'
 :'border-border hover:border-primary/50'}`}
 >
 {option.label}
 ...
 {(data.smoking ==='current-light' || data.smoking ==='current-heavy') && (<p className="text-xs text-warning mt-2">
 ⚠️ Smokers burn 200-300 extra cal/day. If you quit during your journey, expect 5-10 lb weight gain from metabolic slowdown.
 </p>)}
 {data.smoking ==='quit-recent' && (<p className="text-xs text-warning mt-2">
 ...
 <input
 type="number"min="0"max="50"value={data.weeklyDrinks || 0}
 onChange={(e) => updateData({weeklyDrinks: parseInt(e.target.value) || 0})}
 className="form-input w-full"placeholder="0"/>
 </div>
 </div>
 {data.weeklyDrinks && data.weeklyDrinks > 0 && (<p className="text-xs text-warning mt-2">
 ...
 {value:'regular', label:'Yes, regularly (weekly or more)'}].map((option) => (<button
 key={option.value}
 type="button"onClick={() => updateData({recreationalDrugs: option.value as any})}
 className={`w-full p-3 border-2 rounded-lg text-left transition-all text-sm ${data.recreationalDrugs === option.value
 ?'border-primary bg-primary-light /20 ring-2 ring-primary'
 :'border-border hover:border-primary/50'}`}
 >
 {option.label}
 ...
 <div className="space-y-4">
 <div className="bg-primary-light /20 border-2 border-primary rounded-lg p-4">
 <h3 className="font-medium text-primary-dark mb-1">📷 Scan Your Medications</h3>
 <p className="text-sm text-primary-dark">
 ...
 label="Your Medications"description="Scan prescription labels with NDC barcode or OCR. WLPL will auto-suggest health conditions based on your medications."/>

 {/* Show suggested conditions from medications */}
 {suggestedConditions.length > 0 && (<div className="bg-success-light /20 border-2 border-green-400 rounded-lg p-4">
 <h4 className="font-semibold text-green-900 mb-2">
 ✓ WLPL identified conditions from your medications:
 </h4>
 <ul className="space-y-1">
 {suggestedConditions.map((suggestion, index) => (<li key={index} className="text-sm text-green-800">
 ...
 {isClassifyingMeds && (<div className="text-center py-4">
 ...
 {healthRisk && healthRisk.likelyConditions.length > 0 && (<div className="mb-6">
 <label className="text-label block mb-2">
 ...
 {healthRisk.likelyConditions.map((condition) => (<button
 key={condition}
 type="button"onClick={() => toggleHealthCondition(condition)}
 className={`px-4 py-2 rounded-lg border-2 transition-all text-sm ${data.healthConditions?.includes(condition)
 ?'border-warning bg-warning-light /20 text-warning-dark font-medium'
 :'border-warning/30 /50 hover:border-warning/50 /70 text-warning-dark /60'}`}
 >
 <div className="flex items-center justify-between">
 <span>{condition}</span>
 {hasDetailedQuestionnaire(condition) && (<span className="text-xs bg-primary text-white px-1.5 py-0.5 rounded ml-1">?</span>)}
 </div>
 </button>))}
 </div>
 <p className="text-xs text-muted-foreground mt-2">
 ...
 <div className="grid grid-cols-2 gap-2">
 {healthRisk?.otherConditions.map((condition) => (<button
 key={condition}
 type="button"onClick={() => toggleHealthCondition(condition)}
 className={`px-4 py-2 rounded-lg border-2 transition-all text-sm ${data.healthConditions?.includes(condition)
 ...
 <button
 type="button"onClick={() => updateData({dietaryPreferences: []})}
 className={`px-4 py-2 rounded-lg border-2 transition-all text-sm ${data.dietaryPreferences?.length === 0
 ?'border-success bg-success-light /20 text-success-dark font-bold'
 :'border-border hover:border-success/50'}`}
 >
 ✓ None (No restrictions)
 </button>
 {dietaryOptions.map((pref) => (<button
 key={pref}
 type="button"onClick={() => toggleDietaryPref(pref)}
 className={`px-4 py-2 rounded-lg border-2 transition-all text-sm ${data.dietaryPreferences?.includes(pref)
 ...
 {/* Explicit"None"option */}
 <button
 type="button"onClick={() => updateData({foodAllergies: []})}
 className={`px-4 py-2 rounded-lg border-2 transition-all text-sm ${data.foodAllergies?.length === 0
 ?'border-success bg-success-light /20 text-success-dark font-bold'
 :'border-border hover:border-success/50'}`}
 >
 ✓ None (No allergies)
 </button>
 {commonAllergies.map((allergy) => (<button
 key={allergy}
 type="button"onClick={() => toggleAllergy(allergy)}
 className={`px-4 py-2 rounded-lg border-2 transition-all text-sm ${data.foodAllergies?.includes(allergy)
 ?'border-error bg-error-light /20 text-error'
 ...
 {selectedCondition && (<HealthConditionModal
...
// Step 6: Meal Schedule & Notifications
function StepSix({data, updateData}: {data: OnboardingData; updateData: (data: Partial<OnboardingData>) => void}) {// Convert 24-hour time to 12-hour format with AM/PM
 ...
 const handleNotificationToggle = () => {const newValue = !data.notifications

 if (newValue) {// Initialize reminder times from current meal schedule
 updateData({notifications: newValue,
 mealReminderTimes: {breakfast: convertTo12Hour(data.mealSchedule?.breakfastTime ||'07:00'),
 lunch: convertTo12Hour(data.mealSchedule?.lunchTime ||'12:00'),
 dinner: convertTo12Hour(data.mealSchedule?.dinnerTime ||'18:00'),
 snacks: data.mealSchedule?.hasSnacks ? convertTo12Hour(data.mealSchedule.snackWindows?.[0] ||'15:00') : undefined}})} else {updateData({notifications: newValue})}}

 // Update meal schedule time and auto-sync reminder
 const updateMealTime = (meal:'breakfastTime' |'lunchTime' |'dinnerTime', time: string) => {const mealKey = meal.replace('Time','') as'breakfast' |'lunch' |'dinner'

 updateData({mealSchedule: {...data.mealSchedule!,
 [meal]: time},
 // Auto-sync reminder times if notifications enabled
 mealReminderTimes: data.notifications ? {...data.mealReminderTimes,
 [mealKey]: convertTo12Hour(time)} : data.mealReminderTimes})}

 // Toggle snacks and auto-sync reminder
 const toggleSnacks = () => {const newHasSnacks = !data.mealSchedule?.hasSnacks
 ...
 mealReminderTimes: data.notifications ? {...data.mealReminderTimes,
 snacks: newHasSnacks ? convertTo12Hour(defaultSnackTime) : undefined} : data.mealReminderTimes})}
 ...
 <input
 type="time"value={data.mealSchedule?.breakfastTime ||'07:00'}
 ...
 <input
 type="time"value={data.mealSchedule?.lunchTime ||'12:00'}
 ...
 <input
 type="time"value={data.mealSchedule?.dinnerTime ||'18:00'}
 ...
 <button
 type="button"onClick={toggleSnacks}
 ...
 <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${data.mealSchedule?.hasSnacks ?'bg-primary' :'bg-gray-300'}`}>
 <span className={`inline-block h-4 w-4 transform rounded-full bg-background transition-transform ${data.mealSchedule?.hasSnacks ?'translate-x-6' :'translate-x-1'}`} />
 </div>
 </button>
 </div>
 </div>
 ...
 <button
 type="button"onClick={handleNotificationToggle}
 className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${data.notifications ?'bg-primary' :'bg-gray-300'}`}
 >
 <span
 className={`inline-block h-4 w-4 transform rounded-full bg-background transition-transform ${data.notifications ?'translate-x-6' :'translate-x-1'}`}
 />
 </button>
 </div>
 </div>
 ...
 {data.notifications && (<div className="bg-muted rounded-lg p-4">
 ...
 {key:'dinner', label:'Dinner', emoji:'🌙'}].map((meal) => (<div key={meal.key} className="flex items-center space-x-3 bg-card rounded-lg p-3 border border-border">
 ...
 {value:'monthly', label:'Monthly', emoji:'📆'}].map((option) => (<button
 key={option.value}
 type="button"onClick={() => updateData({weightCheckInFrequency: option.value as any})}
 className={`p-3 border-2 rounded-lg transition-all ${data.weightCheckInFrequency === option.value
 ...
 {/* Summary */}
 <div className="bg-success-light /20 border-2 border-success rounded-lg p-4 mt-6">
 <div className="flex items-start space-x-3">
 <span className="text-2xl">✨</span>
 <div>
 <div className="font-medium text-success-dark">You're all set!</div>
 <div className="text-sm text-success-dark mt-1">
...
export default function OnboardingPage() {return (<AuthGuard>
 <OnboardingRouter>
 <OnboardingContent />
 </OnboardingRouter>
 </AuthGuard>)}

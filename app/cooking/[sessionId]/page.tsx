...
function CookingSessionContent() {const router = useRouter()
 ...
 // Load session
 useEffect(() => {async function loadSession() {try {const sessionData = await cookingSessionOperations.getCookingSession(sessionId)
 setSession(sessionData as CookingSession)} catch (error) {logger.error('Error loading cooking session', error as Error)
 toast.error('Failed to load cooking session')
 router.push('/dashboard')} finally {setLoading(false)}}

 if (sessionId) {loadSession()}}, [sessionId, router])

 // Get recipe details
 const recipe = session ? MEAL_SUGGESTIONS.find(r => r.id === session.recipeId) : null

 const handleNextStep = async () => {if (!session || !recipe) return

 const nextStep = session.currentStep + 1

 if (nextStep >= session.totalSteps) {// Completed all steps!
 setShowCompletionModal(true)} else {// Move to next step
 try {await cookingSessionOperations.updateCookingSession(sessionId, {currentStep: nextStep})
 setSession({...session, currentStep: nextStep})} catch (error) {logger.error('Error updating step', error as Error)
 toast.error('Failed to update step')}}}

 const handlePreviousStep = async () => {if (!session) return

 const prevStep = Math.max(0, session.currentStep - 1)

 try {await cookingSessionOperations.updateCookingSession(sessionId, {currentStep: prevStep})
 setSession({...session, currentStep: prevStep})} catch (error) {logger.error('Error updating step', error as Error)
 toast.error('Failed to update step')}}

 const handlePauseSession = async () => {if (!session) return

 try {await cookingSessionOperations.updateCookingSession(sessionId, {status:'paused',
 pausedAt: new Date()})
 toast.success('Session paused')
 router.push('/dashboard')} catch (error) {logger.error('Error pausing session', error as Error)
 toast.error('Failed to pause session')}}

 const handleCompleteSession = async () => {if (!session) return

 try {// Mark session as completed
 await cookingSessionOperations.updateCookingSession(sessionId, {status:'completed',
 completedAt: new Date()})

 // Consume ingredients from inventory
 if (session.scaledIngredients && session.scaledIngredients.length > 0) {const ingredientResults = checkIngredientsWithQuantities(session.scaledIngredients,
 inventoryItems)
 ...
 .map(async (result) => {try {await consumeItem(result.item!.id)
 consumedCount++} catch (err) {logger.error(`Failed to consume item ${result.item!.id}`, err as Error)}})

 await Promise.all(consumePromises)

 if (consumedCount > 0) {toast.success(`✓ ${consumedCount} ingredient${consumedCount > 1 ?'s' :''} consumed from inventory`)}}

 // Redirect to meal logging with recipe data
 const queryParams = new URLSearchParams({fromRecipe:'true',
 recipeId: session.recipeId,
 sessionId: sessionId,
 mealType: session.mealType,
 servings: session.servingSize.toString()})

 router.push(`/log-meal?${queryParams.toString()}`)} catch (error) {logger.error('Error completing session', error as Error)
 toast.error('Failed to complete session')}}

 const toggleIngredient = (index: number) => {setCheckedIngredients(prev => {const newSet = new Set(prev)
 if (newSet.has(index)) {newSet.delete(index)} else {newSet.add(index)}
 return newSet})}

 if (loading) {return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-100 flex items-center justify-center">
 <Spinner size="lg"/>
 </div>)}

 if (!session || !recipe) {return (<div className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-100 flex items-center justify-center p-4">
 ...
 <Link
 href="/dashboard"className="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors">
 Back to Dashboard
 </Link>
 </div>
 </div>)}
 ...
 return (<main className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-100">
 <PageHeader
 ...
 <div
 className="bg-success h-3 rounded-full transition-all duration-500"style={{width:`${progress}%`}}
 />
 </div>
 </div>
 ...
 {currentStepData.duration && (<CookingTimer
 ...
 {!currentStepData.duration && (<div className="bg-muted rounded-lg p-4 text-center">
 ...
 {session.scaledIngredients.map((ingredient, idx) => (<label
 key={idx}
 className={`flex items-start space-x-3 p-2 rounded hover:bg-muted cursor-pointer transition-colors ${checkedIngredients.has(idx) ?'bg-primary-light /20/30' :''}`}
 >
 <input
 type="checkbox"checked={checkedIngredients.has(idx)}
 ...
 {/* Completion Modal */}
 {showCompletionModal && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
...
export default function CookingSessionPage() {return (<AuthGuard>

...
import {getFAQsGroupedBySection,
 getPopularFAQs,
 getShopperSections,
 getSectionDisplayName,
 getRelatedFAQs} from'@/lib/faq-operations'
import {FAQItem, FAQSection} from'@/types/support'

export default function ShopperSupportPage() {const [selectedFAQ, setSelectedFAQ] = useState<FAQItem | null>(null)
 ...
 <FAQSearch
 category="shopper"availableSections={shopperSections}
 ...
 <Link
 href="/shopper/earnings"className="bg-card border border-border rounded-lg p-4 hover:border-green-500 transition-colors">
 ...
 <Link
 href="/shopper/orders"className="bg-card border border-border rounded-lg p-4 hover:border-green-500 transition-colors">
 <div className="text-3xl mb-2">📦</div>
 ...
 <Link
 href="/support/contact?type=shopper"className="bg-card border border-border rounded-lg p-4 hover:border-green-500 transition-colors">
 <div className="text-3xl mb-2">💬</div>
 ...
 <Link
 href="/shopper/training"className="bg-card border border-border rounded-lg p-4 hover:border-green-500 transition-colors">
 <div className="text-3xl mb-2">📚</div>
 ...
 <a
 href="tel:555-123-4567"className="btn bg-red-600 hover:bg-red-700 text-white text-sm">
 Call 24/7 Support: (555) 123-4567
 </a>
 <Link
 href="/support/live-chat?priority=urgent"className="btn btn-secondary text-sm">
 Emergency Chat
 </Link>
 </div>
 </div>
 ...
 {popularFAQs.map(faq => (<PopularFAQCard key={faq.id} faq={faq} onClick={() => setSelectedFAQ(faq)} />))}
 </div>
 </div>
 ...
 {showAllFAQs ? (<div className="space-y-6">
 {shopperSections.map(section => (<CategorySection
 key={section}
 section={section}
 ...
 {shopperSections.map(section => (<CategoryCard
 key={section}
 section={section}
 ...
 <a
 href="tel:555-123-4567"className="btn bg-green-600 hover:bg-green-700 text-white">
 Call (555) 123-4567
 </a>
 <Link
 href="/support/contact?type=shopper"className="btn btn-secondary">
 Submit Ticket
 </Link>
 <Link
 href="/support/live-chat"className="btn btn-secondary">
 Live Chat
 </Link>
 </div>
 </div>
 ...
 {/* Selected FAQ Modal */}
 {selectedFAQ && (<FAQDetailModal faq={selectedFAQ} onClose={() => setSelectedFAQ(null)} />)}
 </div>
 </div>)}

interface PopularFAQCardProps {faq: FAQItem
 onClick: () => void}

function PopularFAQCard({faq, onClick}: PopularFAQCardProps) {const helpfulPercentage = Math.round((faq.helpfulCount / (faq.helpfulCount + faq.notHelpfulCount || 1)) * 100)
 ...
 <svg
 className="w-5 h-5 text-muted-foreground flex-shrink-0 ml-4"fill="none"stroke="currentColor"viewBox="0 0 24 24">
 <path strokeLinecap="round"strokeLinejoin="round"strokeWidth={2} d="M9 5l7 7-7 7"/>
 </svg>
 </div>
 </button>)}

interface CategoryCardProps {section: FAQSection
 count: number
 onClick: () => void}

function CategoryCard({section, count, onClick}: CategoryCardProps) {const icons: Record<string, string> = {'shopper-getting-started':'🚀','shopping-orders':'🛒','shopper-delivery':'🚚','payments':'💰','issues-escalation':'🆘'}
...
interface CategorySectionProps {section: FAQSection
 faqs: FAQItem[]
 onSelectFAQ: (faq: FAQItem) => void}

function CategorySection({section, faqs, onSelectFAQ}: CategorySectionProps) {const [expanded, setExpanded] = useState(false)
...
interface FAQDetailModalProps {faq: FAQItem
 onClose: () => void}

function FAQDetailModal({faq, onClose}: FAQDetailModalProps) {const relatedFAQs = getRelatedFAQs(faq.id)

 return (<div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
 ...
 {/* Tags */}
 {faq.tags.length > 0 && (<div className="flex flex-wrap gap-2 mb-6">
 {faq.tags.map(tag => (<span
 key={tag}
 ...
 {/* Related Articles */}
 {relatedFAQs.length > 0 && (<div className="border-t border-border pt-6">
 <h3 className="font-semibold text-foreground mb-3">Related Articles</h3>
 <div className="space-y-2">
 {relatedFAQs.map(relatedFAQ => (<button
 key={relatedFAQ.id}
 onClick={() => onClose()}

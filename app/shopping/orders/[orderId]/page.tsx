...
const statusEmojis: Record<string, string> = {draft:'📝',
 submitted:'✅',
 assigned:'👤',
 shopping_started:'🛒',
 shopping_complete:'✔️',
...
const statusLabels: Record<string, string> = {draft:'Draft',
...
export default function OrderDetailPage() {const params = useParams()
 ...
 return () => clearInterval(interval)}, [orderId, user])

 async function loadOrder() {if (!user) return

 try {const orderData = await getOrder(orderId)
 if (!orderData) {toast.error('Order not found')
 router.push('/shopping/orders')
 return}

 if (orderData.userId !== user.uid) {toast.error('Unauthorized')
 router.push('/shopping/orders')
 return}

 setOrder(orderData)} catch (error) {console.error('Error loading order:', error)
 toast.error('Failed to load order')} finally {setLoading(false)}}
 ...
 if (!order) {return (<div className="min-h-screen flex items-center justify-center p-4">
 <div className="text-center max-w-md">
 <p className="text-6xl mb-4">❌</p>
 <h1 className="text-2xl font-bold text-foreground mb-2">Order Not Found</h1>
 <Link
 href="/shopping/orders"className="inline-block bg-primary text-primary-foreground px-6 py-3 rounded-lg font-medium hover:bg-primary/90 mt-4">
 View All Orders
 </Link>
 </div>
 </div>)}
 ...
 <div className="mb-6">
 <Link
 href="/shopping/orders"className="text-primary hover:underline inline-flex items-center gap-2 mb-4">
 ← All Orders
 </Link>
 ...
 {(order.status ==='out_for_delivery' || order.status ==='delivered') && (<div className="bg-card rounded-lg shadow-lg p-6 border mb-6">
 ...
 {lateAddFees > 0 && (<div className="flex justify-between text-foreground">
 ...
 {refunded > 0 && (<div className="flex justify-between text-green-600">
 <span>Refunded</span>
 ...
 {order.status ==='delivered' && !order.inspectionCompleted && (<div className="bg-yellow-50 /20 border border-yellow-200 rounded-lg p-6 mb-6">
 ...
 {order.inspectionCompleted && (<div className="bg-green-50 /20 border border-green-200 rounded-lg p-6 mb-6">
 ...
 <div className="text-center">
 <Link
 href="/support/customer"className="text-primary hover:underline inline-flex items-center gap-2">

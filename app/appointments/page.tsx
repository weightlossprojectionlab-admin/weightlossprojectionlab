...
export default function AppointmentsPage() {return (<AuthGuard>
 <AppointmentsContent />
 </AuthGuard>)}

function AppointmentsContent() {const router = useRouter()
 ...
 <PageHeader
 title="Appointments"subtitle="Manage medical appointments"backHref="/medical"/>
 ...
 </div>) : appointments.length === 0 ? (<div className="text-center py-12 bg-card rounded-lg shadow-sm">
 ...
 {appointment.reason && (<p className="text-sm text-muted-foreground mt-2 line-clamp-2">
 {appointment.reason}
 </p>)}
 </div>
 </div>))}
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
 {pastAppointments.map(appointment => (<div
 key={appointment.id}
 onClick={() => router.push(`/appointments/${appointment.id}`)}

...
export default function AppointmentDetailPage() {return (<AuthGuard>
 <AppointmentDetailContent />
 </AuthGuard>)}

function AppointmentDetailContent() {const params = useParams()
 ...
 useEffect(() => {const fetchAppointment = async () => {try {setLoading(true)
 const data = await medicalOperations.appointments.getAppointment(appointmentId)
 setAppointment(data)} catch (error: any) {toast.error('Failed to load appointment details')} finally {setLoading(false)}}

 fetchAppointment()}, [appointmentId])

 const handleDelete = async () => {if (!confirm('Are you sure you want to delete this appointment?')) {return}

 try {setDeleting(true)
 await medicalOperations.appointments.deleteAppointment(appointmentId)
 toast.success('Appointment deleted successfully')
 router.push('/appointments')} catch (error: any) {toast.error(error.message ||'Failed to delete appointment')} finally {setDeleting(false)}}

 const handleDriverAccept = async () => {if (!appointment) return

 try {setRespondingToDriver(true)
 const updatedAppointment = await medicalOperations.appointments.updateAppointment(appointmentId, {driverStatus:'accepted',
 driverAcceptedAt: new Date().toISOString()})
 setAppointment(updatedAppointment)
 toast.success('You accepted the driver assignment!')} catch (error: any) {toast.error(error.message ||'Failed to accept driver assignment')} finally {setRespondingToDriver(false)}}

 const handleDriverDecline = async () => {if (!appointment) return

 const reason = prompt('Optional: Why are you declining?')

 try {setRespondingToDriver(true)
 const updatedAppointment = await medicalOperations.appointments.updateAppointment(appointmentId, {driverStatus:'declined',
 driverDeclinedAt: new Date().toISOString(),
 driverDeclineReason: reason || undefined})
 setAppointment(updatedAppointment)
 toast.success('You declined the driver assignment')} catch (error: any) {toast.error(error.message ||'Failed to decline driver assignment')} finally {setRespondingToDriver(false)}}
 ...
 const getStatusColor = (status: string) => {switch (status) {case'confirmed':
 return'bg-green-100 /20 text-success-dark'
 case'scheduled':
 return'bg-blue-100 text-blue-700'
 case'cancelled':
 return'bg-red-100 /20 text-error-dark'
 case'completed':
 ...
 if (!appointment) {return (<div className="min-h-screen flex items-center justify-center bg-background">
 ...
 <PageHeader
 title="Appointment Details"subtitle={appointment.patientName}
 ...
 disabled={deleting}
 className="px-4 py-2 text-error hover:bg-error-light /20 rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50">
 ...
 {/* Date and Time */}
 <div className="bg-purple-50 /10 rounded-lg p-6">
 <div className="flex items-center gap-3 mb-2">
 ...
 {appointment.requiresDriver && (<div className="border-l-4 border-blue-500 bg-secondary-light /10 rounded-r-lg p-6">
 <h3 className="text-sm font-medium text-blue-700 mb-3 flex items-center gap-2">
 <UserIcon className="w-5 h-5"/>
 Transportation
 </h3>

 {appointment.driverStatus ==='pending' && appointment.assignedDriverName && (<div className="space-y-3">
 ...
 {user?.uid === appointment.assignedDriverId ? (<div className="flex gap-3 mt-4">
 <button
 ...
 {appointment.driverStatus ==='accepted' && appointment.assignedDriverName && (<div className="space-y-3">
 <div className="flex items-center gap-2">
 ...
 {appointment.pickupTime && (<p className="text-sm text-muted-foreground">
 Pickup time: {formatDate(appointment.pickupTime)} at {formatTime(appointment.pickupTime)}
 </p>)}
 {appointment.driverAcceptedAt && (<p className="text-xs text-muted-foreground">
 Accepted on {formatDate(appointment.driverAcceptedAt)}
 </p>)}
 </div>)}

 {appointment.driverStatus ==='declined' && appointment.assignedDriverName && (<div className="space-y-3">
 <div className="flex items-center gap-2">
 ...
 {appointment.driverDeclineReason && (<p className="text-sm text-muted-foreground">
 ...
 {appointment.driverStatus ==='not-needed' && (<p className="text-muted-foreground">
 Patient does not require transportation
 </p>)}
 </div>)}

 {/* Location */}
 {appointment.location && (<div className="flex items-start gap-4">
 ...
 {/* Notes */}
 {appointment.notes && (<div className="border-t border-border pt-6">

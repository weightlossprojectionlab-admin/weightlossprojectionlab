...
interface PatientFix {patientId: string | null
 currentName: string
 targetName: string
...
export default function FixPatientsPage() {const {user} = useAuth()
 ...
 useState(() => {if (!patientsLoading && patients.length > 0) {const foundFixes: PatientFix[] = []

 patients.forEach((patient) => {// Check for percyrice -> Percy Rice
 if (patient.name.toLowerCase() ==='percyrice') {foundFixes.push({patientId: patient.id,
 ...
 if (patient.name ==='Darlene Rice' && patient.type ==='pet') {foundFixes.push({patientId: patient.id,
 currentName: patient.name,
 ...
 const handleFix = async (fix: PatientFix) => {if (!fix.patientId) {toast.error('Patient ID not found')
 return}

 setFixing(fix.patientId)

 try {const updates: Partial<PatientProfile> = {}

 if (fix.currentName !== fix.targetName) {updates.name = fix.targetName}

 if (fix.currentType !== fix.targetType) {updates.type = fix.targetType}

 if (fix.currentDOB !== fix.targetDOB) {updates.dateOfBirth = fix.targetDOB}
 ...
 setFixes(fixes.filter(f => f.patientId !== fix.patientId))} catch (error) {console.error('Error fixing patient:', error)
 toast.error('Failed to fix patient data')} finally {setFixing(null)}}

 const handleFixAll = async () => {for (const fix of fixes) {if (fix.patientId) {await handleFix(fix)}}}
 ...
 {/* Loading State */}
 {patientsLoading && (<div className="bg-card rounded-lg p-8 text-center">
 <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"/>
 <p className="text-muted-foreground">Scanning patient records...</p>
 </div>)}

 {/* No Issues Found */}
 {!patientsLoading && fixes.length === 0 && (<div className="bg-success-light /20 border-2 border-success rounded-lg p-6">
 <div className="flex items-center gap-3">
 <span className="text-3xl">✅</span>
 <div>
 <h3 className="font-bold text-success-dark">
 ...
 <div className="bg-warning-light /20 border-2 border-warning-dark rounded-lg p-4 mb-6">
 <div className="flex items-center gap-3">
 <span className="text-2xl">⚠️</span>
 <div>
 <h3 className="font-bold text-warning-dark">
 ...
 {fixing === fix.patientId ? (<span className="flex items-center gap-2">
 ...
 {/* Name Change */}
 {fix.currentName !== fix.targetName && (<div className="grid grid-cols-2 gap-4">
 <div>
 ...
 {/* Type Change */}
 {fix.currentType !== fix.targetType && (<div className="grid grid-cols-2 gap-4">
 <div>
 ...
 {/* DOB Change */}
 {fix.currentDOB !== fix.targetDOB && (<div className="grid grid-cols-2 gap-4">
 <div>
 ...
 <div className="mt-8">
 <a
 href="/patients"className="text-primary hover:text-primary-hover font-medium">
 ← Back to Patients
 </a>
 </div>
 </div>

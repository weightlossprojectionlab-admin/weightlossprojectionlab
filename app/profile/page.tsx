...
import {isBiometricSupported,
 ...
 hasBiometricCredential} from'@/lib/webauthn'
...
function ProfileContent() {const {user} = useAuth()
 ...
 useEffect(() => {if (effectivePatientId) {logger.debug('[Profile] Patient ID for medications', {selectedMemberId,
 userId: user?.uid,
 effectivePatientId,
 currentMember: currentlyViewingMember?.name})}}, [effectivePatientId, selectedMemberId, user?.uid, currentlyViewingMember])

 useEffect(() => {setMounted(true)
 checkBiometricStatus()}, [user])

 // Fetch user profile data (or selected member profile)
 useEffect(() => {const fetchProfile = async () => {if (!user) return

 try {// If a family member is selected, load their profile from patients
 if (selectedMemberId && selectedMemberId !== user.uid) {const selectedMember = familyMembers.find(m => m.id === selectedMemberId)
 if (selectedMember) {setProfileData(selectedMember)
 return}}

 // Otherwise load own profile
 const profile = await userProfileOperations.getUserProfile()
 setProfileData(profile.data)} catch (error) {logger.error('Error fetching profile', error as Error)
 toast.error('Failed to load profile data')}}

 fetchProfile()}, [user, selectedMemberId, familyMembers])

 // Detect platform and health app
 useEffect(() => {const platform = detectPlatform()
 const app = getHealthAppForPlatform(platform)
 setHealthApp(app)}, [])

 const checkBiometricStatus = async () => {try {const supported = await isBiometricSupported()
 setBiometricSupported(supported)

 if (supported && user) {const enabled = await hasBiometricCredential(user.uid)
 setBiometricEnabled(enabled)}} catch (error) {logger.error('Error checking biometric status', error as Error)}}

 const handleEnableBiometrics = async () => {if (!user || !biometricSupported) return

 setLoading(true)
 try {await registerBiometric(user.uid, user.email ||'')
 setBiometricEnabled(true)
 toast.success('Biometric authentication enabled successfully!')} catch (error: any) {logger.error('Failed to enable biometrics', error as Error)
 toast.error('Failed to set up biometric authentication:' + error.message)} finally {setLoading(false)}}

 const handleDisableBiometrics = async () => {if (!user) return

 const confirmed = await confirm({title:'Remove Biometric Authentication?',
 message:'You will need to use your password to sign in after removing biometric authentication.',
 confirmText:'Remove',
 cancelText:'Keep It',
 variant:'warning'})

 if (confirmed) {removeBiometricCredential(user.uid)
 setBiometricEnabled(false)
 toast.success('Biometric authentication removed.')}}

 const handleToggleStepTracking = async () => {try {if (stepTrackingEnabled) {await disableTracking()
 toast.success('Automatic step tracking disabled')} else {await enableTracking()
 toast.success('Automatic step tracking enabled! Your steps will be counted in the background.')}} catch (error) {logger.error('Toggle step tracking error', error as Error)
 toast.error('Failed to toggle step tracking. Please check device permissions.')}}

 const handleSignOut = async () => {setSignOutLoading(true)
 try {await signOut()} catch (error) {logger.error('Sign out error', error as Error)
 toast.error('Failed to sign out. Please try again.')} finally {setSignOutLoading(false)}}

 const handleSendTestNotification = async () => {setSendingTestNotif(true)
 try {// Check if browser supports notifications
 if (!('Notification' in window)) {toast.error('Your browser does not support notifications')
 return}
 ...
 if (permission ==='denied') {toast.error('Notification permission denied. Please enable notifications in your browser settings.')
 return}

 if (permission ==='granted') {// Send a browser notification directly
 const notification = new Notification('🎉 Test Notification', {body:'Your notifications are working! You\'ll receive helpful reminders to stay on track.',
 ...
 logger.info('[Test Notification] Browser notification sent successfully')}} catch (error) {logger.error('Error sending test notification:', error as Error)
 toast.error('Failed to send test notification:' + (error as Error).message)} finally {setSendingTestNotif(false)}}

 const handleResetAllData = async () => {const confirmed = await confirm({title:'⚠️ Reset All Data & Start Over?',
 ...
 const doubleConfirmed = await confirm({title:'Final Confirmation',
 ...
 try {await userProfileOperations.resetAllData()
 toast.success('All data has been reset. Redirecting to onboarding...')

 // Wait a moment for the user to see the message
 setTimeout(() => {router.push('/onboarding')}, 2000)} catch (error) {logger.error('Reset all data error', error as Error)
 toast.error('Failed to reset data. Please try again or contact support.')
 setResetLoading(false)}}

 const handleSaveAdvancedProfile = async (updates: any) => {try {// If viewing a family member, update their patient record
 if (currentlyViewingMember) {// Merge the updates with existing patient data
 const updatedPatientData = {...currentlyViewingMember,
 ...updates,
 ...
 const response = await fetch(`/api/patients/${currentlyViewingMember.id}`, {method:'PUT',
 ...
 setProfileData(result.patient || {...currentlyViewingMember, ...updates})} else {// Saving own profile - need to structure the data correctly for user profile API
 const userProfileUpdates = {profile: {foodAllergies: updates.foodAllergies,
 ...
 setProfileData(updated.data)}} catch (error: any) {console.error('Full save error:', error)
 ...
 const handleEnableHealthSync = () => {// The HealthSyncCard will handle the actual sync enabling
 ...
 <Link
 href="/dashboard"className="text-primary hover:text-primary"aria-label="Back to dashboard">
 ← Back
 </Link>
 <div className="flex-1">
 <h1 className="text-xl font-semibold text-foreground">
 {currentlyViewingMember
 ?`${currentlyViewingMember.name}'s Profile`:'Your Profile & Settings'}
 </h1>
 {currentlyViewingMember && (<p className="text-xs text-muted-foreground">
 ...
 {currentlyViewingMember
 ?`${currentlyViewingMember.relationship}'s Health Profile`:'Your Personal Health Profile'}
 </p>
 </div>
 </div>
 {currentlyViewingMember && (<div className="flex flex-col items-end gap-2">
 ...
 {patientsLoading ? (<div className="border-t border-gray-300 pt-4">
 <p className="text-sm text-muted-foreground">Loading family members...</p>
 </div>) : familyMembers.length > 0 ? (<div className="border-t border-gray-300 pt-4">
 ...
 {familyMembers.map(member => (<option key={member.id} value={member.id}>
 ...
 </div>) : (<div className="border-t border-gray-300 pt-4">
 ...
 {profileData && (<AdvancedHealthProfile
 ...
 {subscription && !subscriptionLoading && (<div className="bg-card rounded-lg p-6 shadow-sm">
 ...
 <div
 className={`h-full rounded-full transition-all ${percentage >= 100 ?'bg-error' :
 percentage >= 80 ?'bg-warning-dark' :'bg-success'}`}
 ...
 <p className={`font-medium capitalize ${subscription.status ==='active' ?'text-success' :
 subscription.status ==='trialing' ?'text-warning-dark' :'text-error'}`}>
 {subscription.status}
 </p>
 </div>

 {/* Trial Info */}
 {subscription.trialEndsAt && subscription.status ==='trialing' && (<div className="bg-yellow-100 /30 border border-yellow-400 rounded-lg p-3">
 ...
 {mounted && (<div className="bg-card rounded-lg p-6 shadow-sm">
 <h2 className="text-lg font-medium text-foreground mb-4">
 Biometric Authentication
 </h2>

 {!biometricSupported ? (<div className="bg-warning-light border border-warning-light rounded-lg p-4">
 ...
 <p className="text-sm text-muted-foreground">
 {biometricEnabled ?'You can sign in using your fingerprint or face recognition.' :'Use your fingerprint or face recognition for quick and secure sign-in.'}
 </p>

 <div className="pt-2">
 {biometricEnabled ? (<button
 onClick={handleDisableBiometrics}
 className="btn btn-secondary w-full"aria-label="Disable biometric authentication">
 🗑️ Remove Biometric Authentication
 </button>) : (<button
 onClick={handleEnableBiometrics}
 disabled={loading}
 className="btn btn-primary w-full"aria-label="Enable biometric authentication">
 ...
 {biometricSupported && (<div className="bg-indigo-100 /20 rounded-lg p-3">
 ...
 <div className="bg-error-light /20 border-2 border-error rounded-lg p-4 mb-3">
 ...
 className="btn btn-secondary w-full text-error border-error hover:bg-error-light /20 font-medium">
 {resetLoading ? (<span className="flex items-center justify-center space-x-2">
 <div className="animate-spin w-5 h-5 border-2 border-error border-t-transparent rounded-full"/>
 <span>Resetting...</span>
 </span>) : ('🔄 Reset All Data & Start Over')}
 </button>
 </div>
 </div>
 </div>
 ...
 {isTracking && (<span className="relative flex h-2 w-2">
 ...
 <input
 type="checkbox"className="sr-only peer"checked={stepTrackingEnabled}
 ...
 className={`btn btn-secondary w-full text-error border-error hover:bg-error-light /20 inline-flex items-center justify-center space-x-2 ${signOutLoading ?'cursor-wait' :''}`}
...
export default function ProfilePage() {return (<AuthGuard>

...
import {fetchRecentPhotos,
 groupPhotosByDate,
 searchPhotos,
 getGalleryStats,
 type PhotoGalleryItem} from'@/lib/photo-gallery-utils'
...
export default function GalleryPage() {const router = useRouter()
 ...
 // Auth check
 useEffect(() => {const unsubscribe = onAuthStateChanged(auth, (user) => {if (!user) {router.push('/auth/signin')}})
 return () => unsubscribe()}, [router])

 // Load photos
 useEffect(() => {console.log('[Gallery] useEffect triggered, timeRange:', timeRange)
 loadPhotos()
 // eslint-disable-next-line react-hooks/exhaustive-deps}, [timeRange])

 const loadPhotos = async () => {setLoading(true)
 try {console.log('[Gallery] Starting to fetch photos, timeRange:', timeRange)
 const fetchedPhotos = await fetchRecentPhotos(timeRange)
 console.log('[Gallery] Fetched photos:', fetchedPhotos.length)
 setPhotos(fetchedPhotos)
 setFilteredPhotos(fetchedPhotos)} catch (error) {console.error('[Gallery] Error loading photos:', error)
 logger.error('Error loading photos:', error as Error)
 toast.error('Failed to load photos')} finally {setLoading(false)}}

 // Apply filters
 useEffect(() => {let filtered = photos

 // Apply meal type filter
 if (mealTypeFilter !=='all') {filtered = filtered.filter(photo => photo.mealType === mealTypeFilter)}

 // Apply search filter
 if (searchTerm) {filtered = searchPhotos(filtered, searchTerm)}

 setFilteredPhotos(filtered)}, [photos, mealTypeFilter, searchTerm])

 // Modal handlers
 const handlePhotoClick = (photo: PhotoGalleryItem, index: number) => {setSelectedPhoto(photo)
 setSelectedIndex(index)}

 const handleNextPhoto = () => {if (selectedIndex < filteredPhotos.length - 1) {const newIndex = selectedIndex + 1
 setSelectedIndex(newIndex)
 setSelectedPhoto(filteredPhotos[newIndex])}}

 const handlePreviousPhoto = () => {if (selectedIndex > 0) {const newIndex = selectedIndex - 1
 setSelectedIndex(newIndex)
 setSelectedPhoto(filteredPhotos[newIndex])}}
 ...
 <ShareButton
 shareOptions={{type:'gallery',
 data: {photoCount: stats.totalPhotos}}}
 variant="default"size="md"className="bg-background/20 hover:bg-background/30"onShareModalOpen={() => setShowShareModal(true)}
 />
 <button
 ...
 <input
 type="text"placeholder="Search meals..."value={searchTerm}
 ...
 <svg
 className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground"fill="none"stroke="currentColor"viewBox="0 0 24 24">
 ...
 className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${viewMode ==='grouped'
 ?'bg-primary text-white'
 :'bg-muted text-foreground hover:bg-gray-200'}`}
 >
 By Date
 ...
 {(searchTerm || mealTypeFilter !=='all') && (<div className="flex items-center gap-2 mt-4 pt-4 border-t border-border">
 <span className="text-sm text-muted-foreground">Active filters:</span>
 {searchTerm && (<span className="inline-flex items-center px-3 py-1 bg-primary/10 text-primary rounded-full text-sm">
 Search:"{searchTerm}"<button
 onClick={() => setSearchTerm('')}
 className="ml-2 hover:text-primary-hover">
 ×
 </button>
 </span>)}
 {mealTypeFilter !=='all' && (<span className="inline-flex items-center px-3 py-1 bg-primary/10 text-primary rounded-full text-sm capitalize">
 {mealTypeFilter}
 <button
 ...
 {/* Motivational Banner */}
 {!loading && filteredPhotos.length > 0 && (<div className="mb-6 p-4 bg-gradient-to-r from-purple-50 to-pink-50 /20 /20 rounded-lg border-2 border-purple-200">
 <div className="flex items-start gap-3">
 <div className="text-3xl">🔥</div>
 <div>
 <h3 className="font-semibold text-foreground mb-1">
 {filteredPhotos.length >= 7
 ?`I've logged ${filteredPhotos.length} meals - that's ${filteredPhotos.length} wins! Time to show my progress. 🚀`:`Every meal I track is proof of my dedication. Keep going! 💪`}
 </h3>
 ...
 {/* Gallery Content */}
 {loading ? (<PhotoGridSkeleton count={8} />) : viewMode ==='grid' ? (<PhotoGalleryGrid

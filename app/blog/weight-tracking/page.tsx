...
import {
  ...
  SparklesIcon,
  UserCircleIcon,
  ScaleIcon, ChartBarIcon, FlagIcon, ArrowTrendingDownIcon, CalendarIcon, TrophyIcon
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl text-white/90 mb-8 leading-relaxed">
              Track weight loss progress with interactive charts, goal setting, BMI calculations, and trend analysis. Set custom weight check-in reminders and celebrate milestones as you reach your health goals.
            </p>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <section className="mb-20">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <FeatureCard
              icon={<ScaleIcon className="w-12 h-12 text-blue-600" />}
              title="Quick Weight Logging"
              description="Log weight in seconds with one-tap entry from dashboard"
            />
            <FeatureCard
              icon={<ChartBarIcon className="w-12 h-12 text-blue-600" />}
              title="Interactive Charts"
              description="Zoom, pan, and annotate weight trends over time"
            />
            <FeatureCard
              icon={<FlagIcon className="w-12 h-12 text-blue-600" />}
              title="Goal Setting"
              description="Set target weight, goal date, and track progress percentage"
            />
            <FeatureCard
              icon={<ArrowTrendingDownIcon className="w-12 h-12 text-blue-600" />}
              title="BMI Tracking"
              description="Automatic BMI calculation with healthy range indicators"
            />
            <FeatureCard
              icon={<CalendarIcon className="w-12 h-12 text-blue-600" />}
              title="Custom Check-In Frequency"
              description="Daily, weekly, or bi-weekly reminders based on your preference"
            />
            <FeatureCard
              icon={<TrophyIcon className="w-12 h-12 text-blue-600" />}
              title="Milestone Celebrations"
              description="Celebrate every 5, 10, 20 lbs lost with achievement badges"
            />
          </div>
        </section>

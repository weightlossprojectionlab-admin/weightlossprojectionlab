...
// Feature-flagged: NEXT_PUBLIC_PERKS_ENABLED'use client';
...
export default function PerksPage() {const {user} = useAuth();
 ...
 const handleRedeem = async (perkId: string) => {const perk = perks.find(p => p.perkId === perkId);
 if (perk) {setSelectedPerk(perk);
 setShowRedemptionForm(true);}};

 const handleRedeemSubmit = async (perkId: string, email: string) => {logger.debug('Redeem perk:', {perkId, email});
 ...
 const handleCancelRedemption = () => {setShowRedemptionForm(false);
 setSelectedPerk(null);};

 // Feature flag check
 if (!PERKS_ENABLED) {return (<div className="flex items-center justify-center min-h-screen">
 ...
 {showRedemptionForm && selectedPerk && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
 ...
 {/* Perks Grid */}
 {perks.length > 0 ? (<div className="space-y-6">
 {/* Available Perks */}
 {perks.some(p => !redeemedPerkIds.includes(p.perkId) && (!p.expiresAt || timestampToDate(p.expiresAt) > new Date())) && (<div>
 ...
 </div>) : (<div className="bg-card border border-border rounded-lg p-12 text-center">
 ...
 {/* Info Box */}
 <div className="mt-8 bg-purple-50 border border-purple-200 /20 rounded-lg p-6">

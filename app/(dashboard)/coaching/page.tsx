...
// TODO: Link to PRD v1.3.7 § coaching_readiness_system'use client';
...
export default function CoachingPage() {const {user} = useAuth();
 const coachingData = useCoaching(user?.uid);

 if (coachingData.loading) {return (<div className="flex items-center justify-center min-h-screen">
 ...
 if (coachingData.error) {return (<div className="p-6">
 ...
 if (!isEligibleForCoaching(coachingData)) {return (<div className="p-6 max-w-4xl mx-auto">
 ...
 if (hasActiveAICoach(coachingData)) {const {aiCoachPlan, progress, status} = coachingData;
 ...
 {coachingData.actions.length > 0 && (<div className="bg-card rounded-lg border border-border p-6 mt-6">
 <h2 className="text-xl font-semibold mb-4">Upcoming Reminders</h2>
 <div className="space-y-2">
 {coachingData.actions.map((nudge) => (<div key={nudge.nudgeId} className="p-3 bg-indigo-100 /20 border border-accent rounded-lg">

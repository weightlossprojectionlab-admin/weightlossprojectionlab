...
interface AdminUser {uid: string
 email: string
 displayName?: string
...
interface AuditLog {logId: string
 adminEmail: string
 action: string
 targetType: string
 targetId: string
...
export default function AdminSettingsPage() {const {isAdmin, role, isSuperAdmin} = useAdminAuth()
 ...
 useEffect(() => {if (isAdmin) {loadAdminUsers()
 loadAuditLogs()}}, [isAdmin])

 const loadAdminUsers = async () => {try {const response = await fetch('/api/admin/settings/admin-users')
 if (!response.ok) throw new Error('Failed to load admin users')
 const data = await response.json()
 setAdminUsers(data.admins || [])} catch (err) {logger.error('Error loading admin users:', err as Error)}}

 const loadAuditLogs = async () => {try {const response = await fetch('/api/admin/settings/audit-logs?limit=50')
 if (!response.ok) throw new Error('Failed to load audit logs')
 const data = await response.json()
 setAuditLogs(data.logs || [])
 setLoading(false)} catch (err) {logger.error('Error loading audit logs:', err as Error)
 setLoading(false)}}

 const handleGrantRole = async () => {if (!grantRoleEmail.trim()) {alert('Please enter an email address')
 return}

 if (!isSuperAdmin) {alert('Only super admins can grant roles')
 return}

 setActionLoading(true)
 try {const response = await fetch('/api/admin/grant-role', {method:'POST',
 headers: {'Content-Type':'application/json'},
 body: JSON.stringify({targetEmail: grantRoleEmail,
 role: grantRoleType,
 action:'grant',}),})
 ...
 await loadAuditLogs()} catch (err) {alert(err instanceof Error ? err.message :'Failed to grant role')} finally {setActionLoading(false)}}

 const handleRevokeRole = async (email: string) => {if (!isSuperAdmin) {alert('Only super admins can revoke roles')
 return}
 ...
 await loadAdminUsers()
 await loadAuditLogs()} catch (err) {alert(err instanceof Error ? err.message :'Failed to revoke role')} finally {setActionLoading(false)}}
 ...
 {adminUsers.length === 0 ? (<p className="text-muted-foreground text-center py-4">No admin users found</p>) : (adminUsers.map((admin) => (<div
 key={admin.uid}
 ...
 {isSuperAdmin && !['perriceconsulting@gmail.com','weigthlossprojectionlab@gmail.com'].includes(admin.email) && (<button
 ...
 {isSuperAdmin && (<div className="mt-4 p-4 bg-accent-light /20 border border-indigo-200 rounded-lg">
 ...
 {auditLogs.length === 0 ? (<p className="text-muted-foreground text-center py-4">No audit logs found</p>) : (auditLogs.map((log) => (<div
 key={log.logId}
 ...
 {log.reason && (<div className="text-muted-foreground text-xs mt-1">
 Reason: {log.reason}
 </div>)}
 </div>
 ...
 {/* Grant Role Modal */}
 {showGrantRoleModal && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
 ...
 <input
 id="email"type="email"value={grantRoleEmail}
 onChange={(e) => setGrantRoleEmail(e.target.value)}
 placeholder="user@example.com"className="w-full px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"/>
 </div>
 ...
 onClick={() => setShowGrantRoleModal(false)}
 disabled={actionLoading}
 className="flex-1 px-4 py-2 bg-gray-200 text-foreground rounded-lg hover:bg-gray-300">
 Cancel
 </button>
 </div>
 </div>

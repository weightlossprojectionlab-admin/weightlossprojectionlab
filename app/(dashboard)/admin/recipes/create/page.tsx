...
interface SelectedProduct {barcode: string
 productName: string
 brand: string
 imageUrl?: string
 quantity: number
...
export default function CreateRecipePage() {const router = useRouter()
 ...
 const calculateNutrition = () => {let totalCalories = 0
 let totalProtein = 0
 let totalCarbs = 0
 let totalFat = 0
 ...
 ingredients.forEach(ingredient => {if (ingredient.nutrition) {totalCalories += ingredient.nutrition.calories * ingredient.quantity
 ...
 return {calories: Math.round(totalCalories / servingSize),
 ...
 const handleAddProduct = (product: SelectedProduct) => {const newIngredient: RecipeIngredient = {productBarcode: product.barcode,
 ...
 unit: product.unit,
 nutrition: {calories: product.nutrition.calories,
 ...
 const handleAddTextIngredient = () => {if (!textIngredient.trim()) return

 const newIngredient: RecipeIngredient = {ingredientText: textIngredient,
 quantity: 1,
 unit:'serving'}
 ...
 const handleUpdateStep = (index: number, value: string) => {const newSteps = [...recipeSteps]
 newSteps[index] = value
 setRecipeSteps(newSteps)}

 const handleRemoveStep = (index: number) => {setRecipeSteps(recipeSteps.filter((_, i) => i !== index))}

 const handleAddTip = () => {setCookingTips([...cookingTips,''])}

 const handleUpdateTip = (index: number, value: string) => {const newTips = [...cookingTips]
 newTips[index] = value
 setCookingTips(newTips)}

 const handleRemoveTip = (index: number) => {setCookingTips(cookingTips.filter((_, i) => i !== index))}

 const toggleDietaryTag = (tag: DietaryTag) => {if (dietaryTags.includes(tag)) {setDietaryTags(dietaryTags.filter(t => t !== tag))} else {setDietaryTags([...dietaryTags, tag])}}

 const handleSave = async (status:'draft' |'published') => {// Validation
 if (!recipeName.trim()) {toast.error('Please enter a recipe name')
 return}

 if (ingredients.length === 0) {toast.error('Please add at least one ingredient')
 return}

 if (recipeSteps.filter(s => s.trim()).length === 0) {toast.error('Please add at least one recipe step')
 return}
 ...
 macros: {protein: nutrition.protein,
 ...
 const response = await fetch('/api/admin/recipes', {method:'POST',
 ...
 router.push('/admin/recipes')} catch (err) {logger.error('Save recipe error:', err as Error)
 toast.error(err instanceof Error ? err.message :'Failed to save recipe')} finally {setSaving(false)}}
 ...
 <div className="mb-8">
 <Link
 href="/admin/recipes"className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground mb-4">
 ...
 <input
 type="text"value={recipeName}
 onChange={(e) => setRecipeName(e.target.value)}
 placeholder="e.g., High Protein Egg Breakfast"className="w-full px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"/>
 </div>
 ...
 placeholder="Describe what makes this recipe special..."rows={3}
 ...
 <input
 type="number"min="1"value={servingSize}
 ...
 className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${dietaryTags.includes(tag)
 ...
 {ingredients.map((ingredient, index) => (<div key={index} className="flex items-start gap-3 p-3 border border-border rounded-lg">
 {ingredient.productImageUrl && (<img src={ingredient.productImageUrl} alt={ingredient.productName} className="w-12 h-12 object-cover rounded"/>)}
 <div className="flex-1">
 <p className="text-sm font-medium text-foreground">{ingredient.ingredientText}</p>
 {ingredient.productBarcode && (<p className="text-xs text-muted-foreground">{ingredient.productBrand} • {ingredient.productBarcode}</p>)}
 {ingredient.notes && (<p className="text-xs text-muted-foreground italic mt-1">{ingredient.notes}</p>)}
 {ingredient.nutrition && (<p className="text-xs text-muted-foreground mt-1">
 ...
 {ingredients.length === 0 && (<div className="text-center py-8 text-muted-foreground">
 ...
 <input
 type="text"value={textIngredient}
 onChange={(e) => setTextIngredient(e.target.value)}
 onKeyPress={(e) => e.key ==='Enter' && handleAddTextIngredient()}
 placeholder="Or add text ingredient (e.g.,'2 cups milk','salt to taste')"className="flex-1 px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm"/>
 <button
 ...
 {/* Nutrition Summary */}
 <div className="bg-gradient-to-r from-purple-50 to-blue-50 /20 /20 rounded-lg shadow p-6 mb-6">
 ...
 {ingredients.some(i => i.nutrition) && (<p className="text-xs text-muted-foreground mt-3 text-center">
 ...
 <div className="space-y-3">
 {recipeSteps.map((step, index) => (<div key={index} className="flex gap-3">
 ...
 placeholder="Enter step description..."rows={2}
 ...
 <div className="space-y-3">
 {cookingTips.map((tip, index) => (<div key={index} className="flex gap-3">
 <input
 type="text"value={tip}
 onChange={(e) => handleUpdateTip(index, e.target.value)}
 placeholder="Enter cooking tip..."className="flex-1 px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"/>
 <button
 ...
 {showProductSelector && (<ProductSelector

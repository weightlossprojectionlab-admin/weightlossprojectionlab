...
interface PublicRecipe {id: string
 title: string
 imageUrl: string
 createdBy: string
...
export default function RecipeModerationPage() {const router = useRouter()
 ...
 const fetchRecipes = async () => {setLoading(true)
 try {// TODO: Replace with real API call
 // const res = await fetch(`/api/admin/recipes?status=${filter}`)
 // const data = await res.json()
 // setRecipes(data.recipes)

 // Placeholder: No recipes yet
 setRecipes([])} catch (error) {logger.error('Error fetching recipes:', error as Error)
 toast.error('Failed to fetch recipes')} finally {setLoading(false)}}

 const handleApprove = async (recipeId: string, feature: boolean = false) => {if (!permissions.canModerateRecipes) {toast.error('You do not have permission to moderate recipes')
 return}

 setProcessing(true)
 try {const response = await fetch(`/api/admin/recipes/${recipeId}/moderate`, {method:'POST',
 ...
 setNotes('')
 fetchRecipes()} catch (error) {logger.error('Error approving recipe:', error as Error)
 toast.error('Failed to approve recipe')} finally {setProcessing(false)}}

 const handleReject = async (recipeId: string) => {if (!permissions.canModerateRecipes) {toast.error('You do not have permission to moderate recipes')
 return}

 if (!rejectionReason) {toast.error('Please select a rejection reason')
 return}
 ...
 setNotes('')
 setRejectionReason('')
 fetchRecipes()} catch (error) {logger.error('Error rejecting recipe:', error as Error)
 toast.error('Failed to reject recipe')} finally {setProcessing(false)}}

 const rejectionReasons: {value: RejectionReason; label: string}[] = [{value:'low_quality_photo', label:'Low quality photo'},
 ...
 if (!permissions.canModerateRecipes) {return (<div className="p-8">
 ...
 {(['pending','approved','rejected'] as const).map((status) => (<button
 key={status}
 ...
 >
 {status}
 {status ==='pending' && recipes.length > 0 && (<span className="ml-2 bg-primary-light /20 text-primary py-0.5 px-2 rounded-full text-xs">
 {recipes.length}
 </span>)}
 </button>))}
 </nav>
 ...
 </div>) : recipes.length === 0 ? (<div className="text-center py-12 bg-card rounded-lg shadow">
 ...
 key={recipe.id}
 className="bg-card rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden cursor-pointer"onClick={() => setSelectedRecipe(recipe)}
 >
 {/* Recipe Image */}
 ...
 alt={recipe.title}
 fill
 className="object-cover"/>) : (<div className="flex items-center justify-center h-full">
 ...
 {/* Recipe Detail Modal */}
 {selectedRecipe && (<div
 className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"onClick={() => setSelectedRecipe(null)}
 >
 <div
 className="bg-card rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto"onClick={(e) => e.stopPropagation()}
 >
 {/* Modal Content */}
 ...
 {/* Recipe Image */}
 {selectedRecipe.imageUrl && (<div className="relative h-64 rounded-lg overflow-hidden">
 <Image
 ...
 {selectedRecipe.ingredients.map((ingredient, index) => (<li key={index} className="text-foreground">{ingredient}</li>))}
 </ul>
 </div>
 ...
 {selectedRecipe.steps.map((step, index) => (<li key={index} className="text-foreground">{step}</li>))}
 </ol>
 </div>

 {/* Tags */}
 {selectedRecipe.tags.length > 0 && (<div>
 <h3 className="text-sm font-semibold text-foreground mb-2">Tags</h3>
 <div className="flex flex-wrap gap-2">
 {selectedRecipe.tags.map((tag, index) => (<span
 key={index}
 ...
 {selectedRecipe.moderationStatus ==='pending' && (<div className="border-t border-border pt-6">
 ...
 className="w-full border border-border bg-background text-foreground rounded-lg px-3 py-2">
 <option value="">Select a reason...</option>
 {rejectionReasons.map((reason) => (<option key={reason.value} value={reason.value}>
 {reason.label}
 </option>))}
 </select>
 </div>
 ...
 className="w-full border border-border bg-background text-foreground placeholder-gray-500 rounded-lg px-3 py-2"/>
 </div>
 ...
 {permissions.canFeatureRecipes && (<button

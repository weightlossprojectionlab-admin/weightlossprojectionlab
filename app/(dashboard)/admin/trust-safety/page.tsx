...
// PRD Reference: trust_safety_moderation (PRD v1.3.7)'use client';
...
export default function TrustSafetyAdminPage() {const {user} = useAuth();
 ...
 const handleViewCase = (caseId: string) => {setSelectedCaseId(caseId);};

 const handleAction = async (action: AdminAction, notes: string) => {logger.info('Admin action', {action, notes, caseId: selectedCaseId});
 ...
 if (!isAdmin || !permissions.canViewCases) {return (<div className="flex items-center justify-center min-h-screen">
 ...
 <div className="bg-error-light /20 border border-red-200 rounded-lg p-4">
 <h2 className="text-red-900 font-semibold mb-2">Error Loading Cases</h2>
 ...
 {/* Layout */}
 {!selectedCase ? (/* List View */
 <CaseList cases={cases} onViewCase={handleViewCase} />) : (/* Detail View */
 ...
 {/* Evidence */}
 {selectedCase.evidence && selectedCase.evidence.length > 0 && (<div>
 <div className="text-sm font-semibold text-muted-foreground mb-2">Evidence</div>
 <div className="space-y-2">
 {selectedCase.evidence.map((ev, index) => (<div key={index} className="bg-muted p-3 rounded">
 ...
 {/* Risk Assessment */}
 <RiskScoreDisplay
 assessment={{score: selectedCase.riskScore,
 ...
 {/* Info Box */}
 {!selectedCase && (<div className="mt-8 bg-indigo-100 /20 border border-accent rounded-lg p-6">

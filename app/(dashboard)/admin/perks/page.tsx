...
export default function PerksAdminPage() {const {isAdmin, role} = useAdminAuth()
 ...
 // Form state
 const [formData, setFormData] = useState({partnerName:'',
 title:'',
 description:'',
 value:'',
 xpRequired: 0,
 ...
 useEffect(() => {if (isAdmin) {loadPerks()}}, [isAdmin])

 const loadPerks = async () => {setLoading(true)
 try {const response = await fetch('/api/admin/perks')
 if (!response.ok) throw new Error('Failed to load perks')
 const data = await response.json()
 setPerks(data.perks || [])} catch (err) {logger.error('Error loading perks:', err as Error)} finally {setLoading(false)}}

 const handleOpenModal = (perk?: Perk) => {if (perk) {setEditingPerk(perk)
 setFormData({partnerName: perk.partnerName,
 title: perk.title,
 ...
 category: perk.category,})} else {setEditingPerk(null)
 setFormData({partnerName:'',
 title:'',
 description:'',
 value:'',
 xpRequired: 0,
 ...
 const handleSave = async () => {if (!formData.partnerName || !formData.title || !formData.value) {alert('Please fill in all required fields')
 return}

 setActionLoading(true)
 try {const method = editingPerk ?'PUT' :'POST'
 ...
 setShowModal(false)
 await loadPerks()} catch (err) {alert(err instanceof Error ? err.message :'Failed to save perk')} finally {setActionLoading(false)}}

 const handleDelete = async (perkId: string, title: string) => {if (!confirm(`Are you sure you want to delete"${title}"?`)) return

 setActionLoading(true)
 try {const response = await fetch(`/api/admin/perks?id=${perkId}`, {method:'DELETE',})

 if (!response.ok) throw new Error('Failed to delete perk')

 alert('Perk deleted successfully')
 await loadPerks()} catch (err) {alert(err instanceof Error ? err.message :'Failed to delete perk')} finally {setActionLoading(false)}}

 const handleToggleEnabled = async (perkId: string, enabled: boolean) => {setActionLoading(true)
 try {const response = await fetch(`/api/admin/perks?id=${perkId}`, {method:'PUT',
 headers: {'Content-Type':'application/json'},
 body: JSON.stringify({enabled: !enabled}),})

 if (!response.ok) throw new Error('Failed to toggle perk')

 await loadPerks()} catch (err) {alert(err instanceof Error ? err.message :'Failed to toggle perk')} finally {setActionLoading(false)}}
 ...
 </div>) : perks.length === 0 ? (<div className="text-center py-12">
 <GiftIcon className="h-12 w-12 text-muted-foreground mx-auto mb-4"/>
 <p className="text-muted-foreground">No perks created yet</p>
 <button
 onClick={() => handleOpenModal()}
 ...
 {perks.map((perk) => (<div key={perk.perkId} className="p-6 hover:bg-background">
 ...
 <span className="px-3 py-1 rounded-full text-xs font-medium bg-primary-light /20 text-purple-800">
 {perk.tier} • {perk.xpRequired} XP
 </span>
 <span className="px-3 py-1 rounded-full text-xs font-medium bg-green-100 /20 text-green-800">
 {perk.value}
 </span>
 ...
 {!perk.enabled && (<span className="px-3 py-1 rounded-full text-xs font-medium bg-red-100 /20 text-red-800">
 Disabled
 </span>)}
 </div>
 </div>
 ...
 className={`px-3 py-1 rounded text-xs font-medium ${perk.enabled
 ...
 disabled={actionLoading}
 className="p-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 /30"title="Edit perk">
 <PencilSquareIcon className="h-5 w-5"/>
 </button>
 <button
 onClick={() => handleDelete(perk.perkId, perk.title)}
 disabled={actionLoading}
 className="p-2 bg-red-100 /20 text-error-dark rounded-lg hover:bg-red-200 /30"title="Delete perk">
 ...
 <input
 type="text"value={formData.partnerName}
 ...
 <input
 type="text"value={formData.value}
 ...
 <input
 type="number"value={formData.xpRequired}
 ...
 {formData.redemptionType ==='code' && (<div>
 <label className="block text-sm font-medium text-foreground mb-2">
 Discount Code
 </label>
 <input
 type="text"value={formData.discountCode}
 onChange={(e) => setFormData({...formData, discountCode: e.target.value})}
 className="w-full px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"/>
 </div>)}

 {formData.redemptionType ==='link' && (<div>
 <label className="block text-sm font-medium text-foreground mb-2">
 Redemption URL
 </label>
 <input
 type="url"value={formData.redemptionUrl}
 ...
 <input
 type="number"value={formData.maxRedemptionsPerUser}
 ...
 <input
 type="number"value={formData.totalAvailable}
 ...
 <input
 type="checkbox"id="enabled"checked={formData.enabled}
 ...
 onClick={() => setShowModal(false)}
 disabled={actionLoading}
 className="flex-1 px-4 py-3 bg-gray-200 text-foreground rounded-lg hover:bg-gray-300">
 Cancel
 </button>
 </div>
 </div>

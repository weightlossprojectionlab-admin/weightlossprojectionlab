...
export default function AdminDashboardPage() {const {role, isSuperAdmin} = useAdminAuth()
 const {stats, loading, error} = useAdminStats()

 const statCards = [{title:'Total Users',
 ...
 {isSuperAdmin && (<span className="px-3 py-1 rounded-full text-sm font-bold bg-gradient-to-r from-yellow-400 to-orange-500 text-white shadow-lg">
 ⭐ Super Admin
 </span>)}
 </div>
 </div>
 ...
 {/* Error Display */}
 {error && (<div className="mb-8 bg-error-light /20 border border-red-200 rounded-lg p-4">
 <h3 className="text-red-900 font-semibold mb-1">Error Loading Stats</h3>
 ...
 {[1, 2, 3, 4, 5, 6].map((i) => (<div key={i} className="bg-card rounded-lg shadow p-6 animate-pulse">
 <div className="h-4 bg-gray-200 rounded w-1/2 mb-4"></div>
 <div className="h-8 bg-gray-200 rounded w-1/3"></div>
 </div>))}
 </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
 {statCards.map((card) => {const Icon = card.icon
 return (<Link key={card.title} href={card.href}>
 <div className="bg-card rounded-lg shadow hover:shadow-lg transition-shadow p-6 cursor-pointer">
 <div className="flex items-center justify-between mb-4">
 <div className={`${card.color} p-3 rounded-lg`}>
 <Icon className="h-6 w-6 text-white"/>
 </div>
 {card.badge && (<span className="bg-error-light0 text-white text-xs font-bold px-2 py-1 rounded-full">
 Action Required
 </span>)}
 </div>
 ...
 <Link
 href="/admin/users"className="bg-card rounded-lg shadow p-4 hover:shadow-lg transition-shadow">
 ...
 <Link
 href="/admin/recipes"className="bg-card rounded-lg shadow p-4 hover:shadow-lg transition-shadow">
 ...
 <Link
 href="/admin/trust-safety"className="bg-card rounded-lg shadow p-4 hover:shadow-lg transition-shadow">
 ...
 <Link
 href="/admin/ai-decisions"className="bg-card rounded-lg shadow p-4 hover:shadow-lg transition-shadow">
 ...
 <Link
 href="/admin/analytics"className="bg-card rounded-lg shadow p-4 hover:shadow-lg transition-shadow">
 ...
 <Link
 href="/admin/settings"className="bg-card rounded-lg shadow p-4 hover:shadow-lg transition-shadow">

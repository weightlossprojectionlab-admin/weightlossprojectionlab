...
interface ProductStats {totalProducts: number
 totalScans: number
 totalPurchases: number
 uniqueUsers: number}

export default function AdminProductsPage() {const {isAdmin, role} = useAdminAuth()
 ...
 const loadProducts = useCallback(async () => {setLoading(true)
 setError(null)
 ...
 const response = await fetch(`/api/admin/products?${params}`, {headers: {'Authorization':`Bearer ${token}`}})
 ...
 setCategoryBreakdown(data.categoryBreakdown || {})} catch (err) {logger.error('Load products error:', err as Error)
 setError(err instanceof Error ? err.message :'Failed to load products')} finally {setLoading(false)}}, [searchQuery, selectedCategory, sortBy])
 ...
 {loading && (<div className="flex items-center gap-2 text-sm text-primary">
 ...
 <input
 type="text"placeholder="Search by name, brand, or barcode..."value={searchQuery}
 ...
 {Object.keys(categoryBreakdown).sort().map(cat => (<option key={cat} value={cat} className="capitalize">
 ...
 {/* Loading State */}
 {loading && (<div className="bg-card rounded-lg shadow p-16">
 ...
 <h2 className="text-red-900 font-semibold mb-2">Error Loading Products</h2>
 <p className="text-error-dark">{error}</p>
 </div>)}

 {/* Products Table */}
 {!loading && !error && products.length > 0 && (<div className="bg-card rounded-lg shadow overflow-hidden">
 ...
 <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Stores</th>
 <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Status</th>
 <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">Actions</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-200">
 {products.map((product) => (<tr key={product.barcode} className="hover:bg-background">
 ...
 {/* Empty State */}
 {!loading && !error && products.length === 0 && (<div className="bg-background rounded-lg border-2 border-dashed border-border p-12 text-center">

...
interface AIDecision {id: string
...
interface AIStats {totalDecisions: number
 lowConfidenceCount: number
 avgConfidence: number
 reversalRate: number
 unreviewedCount: number}

export default function AIDecisionsPage() {const {isAdmin, role} = useAdminAuth()
 ...
 useEffect(() => {if (isAdmin) {loadDecisions()
 loadStats()}}, [isAdmin, filterReviewed, filterType, filterConfidence])

 const loadDecisions = async () => {setLoading(true)
 try {const token = await getAuthToken()
 const queryParams = new URLSearchParams({reviewed: filterReviewed,
 maxConfidence: filterConfidence.toString()})
 if (filterType !=='all') {queryParams.append('type', filterType)}
 const response = await fetch(`/api/admin/ai-decisions?${queryParams.toString()}`,
 {headers: {'Authorization':`Bearer ${token}`}})
 if (!response.ok) throw new Error('Failed to load AI decisions')
 const data = await response.json()
 setDecisions(data.decisions || [])} catch (err) {logger.error('Error loading AI decisions:', err as Error)} finally {setLoading(false)}}

 const loadStats = async () => {try {const token = await getAuthToken()
 const response = await fetch('/api/admin/ai-decisions/stats', {headers: {'Authorization':`Bearer ${token}`}})
 if (!response.ok) throw new Error('Failed to load stats')
 const data = await response.json()
 setStats(data.stats)} catch (err) {logger.error('Error loading stats:', err as Error)}}

 const handleReview = async (decisionId: string, action:'approve' |'reject' |'modify') => {if (!reviewNotes.trim() && action ==='reject') {alert('Please provide a reason for rejecting this decision')
 return}

 setActionLoading(true)
 try {const token = await getAuthToken()
 const response = await fetch(`/api/admin/ai-decisions/${decisionId}/review`, {method:'POST',
 ...
 setReviewNotes('')
 await loadDecisions()
 await loadStats()} catch (err) {alert(err instanceof Error ? err.message :'Failed to review decision')} finally {setActionLoading(false)}}

 const getConfidenceColor = (confidence: number) => {if (confidence >= 0.9) return'text-success'
 if (confidence >= 0.7) return'text-warning'
 return'text-error'}

 const getConfidenceBgColor = (confidence: number) => {if (confidence >= 0.9) return'bg-green-100 /20'
 if (confidence >= 0.7) return'bg-yellow-100'
 return'bg-red-100 /20'}

 const getDecisionTitle = (decision: AIDecision): string => {if (decision.type ==='health-profile') {const conditions = decision.payload?.restrictions ?
 Object.keys(decision.payload.restrictions).filter(k => decision.payload.restrictions[k]?.limit).join(',') :'No restrictions'
 return`Health Profile: ${conditions ||'General'}`} else if (decision.type ==='meal-safety') {return`Meal Safety Check: ${decision.payload?.safetyCheck?.severity ||'Unknown'}`}
 // Legacy or meal-analysis
 return decision.decision ||'AI Decision'}

 const getDecisionDescription = (decision: AIDecision): string => {if (decision.type ==='health-profile') {const restrictionsCount = decision.payload?.restrictions ?
 Object.keys(decision.payload.restrictions).filter(k => decision.payload.restrictions[k]?.limit).length :
 0
 return`Generated ${restrictionsCount} dietary restrictions based on health conditions`} else if (decision.type ==='meal-safety') {const warnings = decision.payload?.safetyCheck?.warnings || []
 return warnings.length > 0 ? warnings[0] :'Safety check completed'}
 return decision.rationale ||'No description available'}

 const getTypeIcon = (type: string) => {switch (type) {case'health-profile':
 return'🏥'
 case'meal-safety':
 return'⚠️'
 case'meal-analysis':
 ...
 {/* Stats */}
 {stats && (<div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
 ...
 {loading && (<div className="ml-auto flex items-center gap-2 text-sm text-primary">
 ...
 <input
 type="range"min="0"max="100"value={filterConfidence * 100}
 ...
 </div>) : decisions.length === 0 ? (<div className="text-center py-12">
 <CpuChipIcon className="h-12 w-12 text-muted-foreground mx-auto mb-4"/>
 <p className="text-muted-foreground">No AI decisions found matching filters</p>
 </div>) : (<div className="divide-y divide-gray-200">
 {decisions.map((decision) => (<div
 key={decision.id || decision.decisionId}
 className="p-6 hover:bg-background cursor-pointer"onClick={() => setSelectedDecision(decision)}
 >
 ...
 {decision.reviewedBy && (<span className="px-2 py-1 rounded-full text-xs font-medium bg-green-100 /20 text-green-800">
 Reviewed
 </span>)}
 </div>
 ...
 {selectedDecision && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
 ...
 {selectedDecision.reversalReason && (<div>
 <div className="text-sm font-semibold text-muted-foreground mb-1">Reversal Reason</div>
 <div className="text-sm text-foreground bg-error-light /20 p-3 rounded">
 ...
 placeholder="Add notes about your review (required for reversals)..."rows={3}

...
interface AnalysisStatus {running: boolean
 startedAt?: string
 startedBy?: string
 completedAt?: string
 failedAt?: string
...
interface AnalysisMetadata {lastAnalysisRun?: string
...
export default function MLAnalyticsPage() {const [status, setStatus] = useState<AnalysisStatus>({running: false})
 ...
 const interval = setInterval(() => {if (status.running || recipeStatus.running) {loadStatus()
 loadRecipeStatus()}}, 5000)

 return () => clearInterval(interval)}, [status.running, recipeStatus.running])

 const loadStatus = async () => {try {const token = await getAuthToken()
 const response = await fetch('/api/admin/ml/analyze-associations', {headers: {'Authorization':`Bearer ${token}`}})

 if (!response.ok) {throw new Error('Failed to load status')}

 const data = await response.json()
 setStatus(data.status || {running: false})
 setMetadata(data.metadata || {})} catch (err) {logger.error('Failed to load analysis status', err as Error)
 toast.error('Failed to load status')} finally {setLoading(false)}}

 const handleTriggerAnalysis = async () => {if (status.running) {toast.error('Analysis already in progress')
 return}

 setShowConfirmModal(true)}

 const handleConfirmAnalysis = async () => {setTriggering(true)

 try {const token = await getAuthToken()
 const response = await fetch('/api/admin/ml/analyze-associations', {method:'POST',
 ...
 setTimeout(() => loadStatus(), 1000)} catch (err) {logger.error('Failed to trigger analysis', err as Error)
 toast.error(err instanceof Error ? err.message :'Failed to start analysis')} finally {setTriggering(false)}}

 const loadRecipeStatus = async () => {try {const token = await getAuthToken()
 const response = await fetch('/api/admin/ml/generate-recipes', {headers: {'Authorization':`Bearer ${token}`}})

 if (!response.ok) {throw new Error('Failed to load recipe generation status')}

 const data = await response.json()
 setRecipeStatus(data.status || {running: false})

 // Merge recipe metadata into main metadata
 setMetadata(prev => ({...prev, ...data.metadata}))} catch (err) {logger.error('Failed to load recipe generation status', err as Error)
 toast.error('Failed to load recipe status')} finally {setRecipeLoading(false)}}

 const handleTriggerRecipeGeneration = async () => {if (recipeStatus.running) {toast.error('Recipe generation already in progress')
 return}

 // Check if associations exist
 if (!metadata.totalProductAssociations || metadata.totalProductAssociations === 0) {toast.error('No product associations found. Please run analysis first.')
 return}

 const confirmed = confirm('This will generate recipes from product associations discovered in your data.' +'Generated recipes will be saved as drafts for review. Continue?')
 ...
 try {const token = await getAuthToken()
 const response = await fetch('/api/admin/ml/generate-recipes', {method:'POST',
 ...
 // Refresh status after a short delay
 setTimeout(() => loadRecipeStatus(), 1000)} catch (err) {logger.error('Failed to trigger recipe generation', err as Error)
 toast.error(err instanceof Error ? err.message :'Failed to start recipe generation')} finally {setRecipeTriggering(false)}}

 const formatDate = (dateStr?: string) => {if (!dateStr) return'Never'
 ...
 {/* Running Status */}
 {status.running && (<div className="flex items-center gap-3 p-4 bg-secondary-light border border-secondary-light rounded-lg">
 ...
 {status.startedAt && (<p className="text-xs text-secondary mt-1">
 ...
 {/* Completed Status */}
 {!status.running && status.status ==='completed' && (<div className="flex items-center gap-3 p-4 bg-success-light /20 border border-green-200 rounded-lg">
 <SparklesIcon className="h-5 w-5 text-success"/>
 <div className="flex-1">
 <p className="font-medium text-green-900">Analysis Complete</p>
 {status.completedAt && (<p className="text-sm text-success-dark">
 ...
 {/* Failed Status */}
 {!status.running && status.status ==='failed' && (<div className="flex items-center gap-3 p-4 bg-error-light /20 border border-red-200 rounded-lg">
 <div className="flex-1">
 <p className="font-medium text-red-900">Analysis Failed</p>
 <p className="text-sm text-error-dark">{status.error ||'Unknown error'}</p>
 {status.failedAt && (<p className="text-xs text-error mt-1">
 ...
 {/* Idle Status */}
 {!status.running && !status.status && (<div className="flex items-center gap-3 p-4 bg-background border border-border rounded-lg">
 ...
 {metadata.lastAnalysisBy && (<p className="text-xs text-muted-foreground mt-1">by {metadata.lastAnalysisBy}</p>)}
 </div>
 ...
 <p className="text-xs text-muted-foreground mt-1">products with associations</p>
 </div>
 ...
 className="px-6 py-3 bg-primary hover:bg-primary-hover disabled:bg-gray-300 text-white rounded-lg font-medium transition-colors flex items-center gap-2 disabled:cursor-not-allowed">
 ...
 {recipeLoading ? (<div className="flex items-center gap-2 text-muted-foreground">
 <ArrowPathIcon className="h-5 w-5 animate-spin"/>
 <span>Loading status...</span>
 </div>) : (<div className="space-y-3">
 {/* Running Status */}
 {recipeStatus.running && (<div className="flex items-center gap-3 p-4 bg-secondary-light border border-secondary-light rounded-lg">
 <ArrowPathIcon className="h-5 w-5 text-secondary animate-spin"/>
 <div className="flex-1">
 ...
 {recipeStatus.startedAt && (<p className="text-xs text-secondary mt-1">
 ...
 {/* Completed Status */}
 {!recipeStatus.running && recipeStatus.status ==='completed' && (<div className="flex items-center gap-3 p-4 bg-success-light /20 border border-green-200 rounded-lg">
 <SparklesIcon className="h-5 w-5 text-success"/>
 <div className="flex-1">
 ...
 {recipeStatus.completedAt && (<p className="text-xs text-success mt-1">
 ...
 {/* Failed Status */}
 {!recipeStatus.running && recipeStatus.status ==='failed' && (<div className="flex items-center gap-3 p-4 bg-error-light /20 border border-red-200 rounded-lg">
 <div className="flex-1">
 <p className="font-medium text-red-900">Generation Failed</p>
 <p className="text-sm text-error-dark">{recipeStatus.error ||'Unknown error'}</p>
 {recipeStatus.failedAt && (<p className="text-xs text-error mt-1">
 ...
 {/* Idle Status */}
 {!recipeStatus.running && !recipeStatus.status && (<div className="flex items-center gap-3 p-4 bg-background border border-border rounded-lg">
 ...
 {metadata.lastRecipeGenerationBy && (<p className="text-xs text-muted-foreground mt-1">
 ...
 className="px-6 py-3 bg-success hover:bg-green-700 disabled:bg-gray-300 text-white rounded-lg font-medium transition-colors flex items-center gap-2 disabled:cursor-not-allowed">
 ...
 title="Start Market Basket Analysis"message="This will analyze all user shopping data to find product associations. The process may take several minutes depending on data volume. Continue?"confirmText="OK"cancelText="Cancel"variant="info"/>
 </div>)}

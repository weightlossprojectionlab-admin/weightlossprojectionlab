...
interface ProductEditData {barcode: string
 productName: string
 brand: string
 imageUrl: string
 category: string
...
export default function ProductEditPage() {const params = useParams()
 ...
 // Form state
 const [formData, setFormData] = useState<ProductEditData>({barcode:'',
 productName:'',
 brand:'',
 imageUrl:'',
 category:'other',
 ...
 useEffect(() => {if (isAdmin && barcode) {loadProduct()}}, [isAdmin, barcode])

 const loadProduct = async () => {setLoading(true)
 setError(null)
 ...
 const productData: ProductEditData = {barcode: data.product.barcode,
 ...
 setFormData(productData)} catch (err) {logger.error('Load product error:', err as Error, {barcode})
 ...
 try {const token = await getAuthToken()
 const response = await fetch(`/api/admin/products/${barcode}`, {method:'PUT',
 headers: {'Content-Type':'application/json','Authorization':`Bearer ${token}`},
 body: JSON.stringify(formData)})

 if (!response.ok) {const errorData = await response.json().catch(() => ({error:'Failed to save product'}))
 throw new Error(errorData.error ||'Failed to save product')}

 toast.success('Product updated successfully')
 router.push('/admin/barcodes')} catch (err) {logger.error('Save product error:', err as Error, {barcode})
 toast.error(err instanceof Error ? err.message :'Failed to save product')} finally {setSaving(false)}}

 const handleVerify = async () => {const updatedFormData = {...formData,
 quality: {...formData.quality,
 verified: true,
 confidence: 100}}
 setFormData(updatedFormData)}

 const handleFetchNutrition = async () => {setFetchingNutrition(true)

 try {const token = await getAuthToken()
 const response = await fetch(`/api/admin/products/${barcode}/fetch-nutrition`, {method:'POST',
 headers: {'Authorization':`Bearer ${token}`}})

 if (!response.ok) {const errorData = await response.json().catch(() => ({error:'Failed to fetch nutrition data'}))
 ...
 toast.success('Nutrition data updated from OpenFoodFacts')} catch (err) {logger.error('Fetch nutrition error:', err as Error, {barcode})
 toast.error(err instanceof Error ? err.message :'Failed to fetch nutrition data')} finally {setFetchingNutrition(false)}}
 ...
 if (error || !product) {return (<div className="p-8 max-w-4xl mx-auto">
 <div className="bg-error-light /20 border border-red-200 rounded-lg p-6">
 <h3 className="text-red-900 font-semibold mb-2">Error Loading Product</h3>
 ...
 <div className="mb-8">
 <Link
 href="/admin/barcodes"className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground mb-4">
 ...
 {/* Product Preview */}
 {formData.imageUrl && (<div className="bg-card rounded-lg shadow p-6 mb-6">
 ...
 <input
 type="number"value={formData.nutrition.calories ?? 0}
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.protein ?? 0}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.carbs ?? 0}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.fat ?? 0}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.saturatedFat ??''}
 onChange={(e) => setFormData({...formData,
 nutrition: {...formData.nutrition, saturatedFat: e.target.value ? parseFloat(e.target.value) : undefined}})}
 placeholder="Optional"className="w-full px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"/>
 </div>
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.transFat ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.fiber ?? 0}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.sugars ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.addedSugars ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"value={formData.nutrition.sodium ?? 0}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.cholesterol ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.vitaminD ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.calcium ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.iron ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="number"step="0.1"value={formData.nutrition.potassium ??''}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="text"value={formData.nutrition.servingSize ??''}
 onChange={(e) => setFormData({...formData,
 nutrition: {...formData.nutrition, servingSize: e.target.value}})}
 placeholder="e.g., 100g, 1 cup"className="w-full px-4 py-2 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"/>
 </div>
 </div>
 </div>
 ...
 <input
 type="checkbox"checked={formData.quality.verified}
 onChange={(e) => setFormData({...formData,
 ...
 <input
 type="range"min="0"max="100"value={formData.quality.confidence}
 onChange={(e) => setFormData({...formData,
 ...
 <Link
 href="/admin/barcodes"className="py-3 px-6 bg-gray-200 hover:bg-gray-300 text-foreground rounded-lg font-medium transition-colors">
 Cancel
 </Link>
 </div>
 </div>)}

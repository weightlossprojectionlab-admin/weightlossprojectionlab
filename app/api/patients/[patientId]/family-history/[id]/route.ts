 ...
 *
 * DELETE — remove a single family-history entry.
 *
 * Phase D of the medical-binder gap close. Mirrors the per-id
  ...
  try {
    ...
    if (!snapshot.exists) {
      return NextResponse.json(
        { success: false, error: 'Family history entry not found' },
        { status: 404 },
      )
    }
    ...
    return errorResponse(error, {
      route: '/api/patients/[patientId]/family-history/[id]',
      operation: 'delete',
    })
  }
}

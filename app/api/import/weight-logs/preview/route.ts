/**
 * Spreadsheet Import — Weight Logs Preview
 *
 * POST /api/import/weight-logs/preview
 ...
 *   }
 *
 * Same shape as /api/import/patients/preview, with one addition:
 * weight logs are per-patient, so the response includes the
 * household's existing patient list. The UI uses it to show
 * "Mike Doe → matches Mike Doe ✓ / no match ✗" alongside the row
 * preview — users see matching problems before they commit.
 */
...
import { suggestMapping } from '@/lib/import/weight-log-import-config'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!rateLimitResult.success) {
      logger.warn('[API /import/weight-logs/preview] Rate limit exceeded', { callerUserId })
    ...
    // Load the household's patients so the UI can preview which
    // CSV rows will match. Returns just the matchable fields —
    // no PHI beyond what's needed to render "matched: yes/no".
    ...
    return errorResponse(error, { route: '/api/import/weight-logs/preview', operation: 'preview' })
  }
}

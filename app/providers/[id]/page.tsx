...
export default function ProviderDetailPage() {return (<AuthGuard>
 <ProviderDetailContent />
 </AuthGuard>)}

function ProviderDetailContent() {const params = useParams()
 ...
 const handleDelete = async () => {if (!provider) return

 if (confirm(`Are you sure you want to delete ${provider.name}?`)) {try {await deleteProvider(providerId)
 toast.success('Provider deleted successfully')
 router.push('/providers')} catch (error) {toast.error('Failed to delete provider')}}}
 ...
 if (!provider) {return (<div className="min-h-screen bg-background">
 ...
 {provider.isPrimary && (<span className="px-3 py-1 bg-blue-100 /30 text-blue-700 rounded-full text-sm font-medium">
 Primary Provider
 </span>)}
 </div>
 </div>
 ...
 {/* Fax */}
 {provider.fax && (<div className="flex items-center gap-3">
 <PhoneIcon className="w-5 h-5 text-muted-foreground flex-shrink-0"/>
 ...
 {/* Email */}
 {provider.email && (<div className="flex items-center gap-3">
 ...
 {/* Website */}
 {provider.website && (<div className="flex items-center gap-3">
 ...
 {/* Additional Info */}
 {(provider.npi || provider.notes) && (<div className="space-y-4 pt-6 border-t border-border">
 <h3 className="text-lg font-semibold text-foreground">

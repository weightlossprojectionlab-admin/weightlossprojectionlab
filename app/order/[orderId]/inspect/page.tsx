...
import {getOrderForInspection,
 getOrderItems,
 startInspection,
 reportIssue,
 completeInspection} from'@/lib/inspection-operations'
// import type {ShopAndDeliverOrder} from'@/types/shopping'
type ShopAndDeliverOrder = any
import {ArrowLeftIcon} from'@heroicons/react/24/outline'
import toast from'react-hot-toast'

export default function OrderInspectionPage() {const params = useParams()
 ...
 useEffect(() => {if (!user || !orderId) return

 const loadOrderData = async () => {try {setLoading(true)
 setError('')
 ...
 if (orderData.userId !== user.uid) {setError('You do not have permission to view this order')
 return}

 // Verify order is delivered
 if (orderData.status !=='delivered') {setError('Order must be delivered before inspection')
 return}
 ...
 if (!orderData.inspectionCompleted) {await startInspection(orderId)}} catch (error: any) {console.error('[InspectionPage] Load error:', error)
 setError(error.message ||'Failed to load order')} finally {setLoading(false)}}

 loadOrderData()}, [user, orderId])

 // Handle report issue
 const handleReportIssue = (itemId: string) => {setSelectedItemId(itemId)
 setReportModalOpen(true)}

 // Handle submit issue report
 const handleSubmitIssue = async (issue: {issueType:'missing' |'damaged' |'wrong_item' |'expired' |'quality_issue'
 description: string
 photoBase64: string}) => {if (!selectedItemId) return

 try {await reportIssue(orderId, selectedItemId, issue)
 ...
 setSelectedItemId(null)} catch (error: any) {console.error('[InspectionPage] Report issue error:', error)
 toast.error(error.message ||'Failed to report issue')
 throw error}}

 // Handle confirm all correct
 const handleConfirmAllCorrect = async () => {try {await completeInspection(orderId, true)

 toast.success('Inspection completed - all items confirmed correct')

 // Reload order
 const updatedOrder = await getOrderForInspection(orderId)
 if (updatedOrder) {setOrder(updatedOrder)}} catch (error: any) {console.error('[InspectionPage] Confirm error:', error)
 ...
 // Error state
 if (error || !order) {return (<div className="min-h-screen bg-background flex items-center justify-center p-4">
 <div className="max-w-md w-full bg-card rounded-lg shadow-lg border border-border p-8 text-center">
 <div className="w-16 h-16 bg-error-light /20 rounded-full flex items-center justify-center mx-auto mb-4">
 ...
 <button
 onClick={() => router.back()}
 className="p-2 hover:bg-muted rounded-lg transition-colors"aria-label="Go back">
 ...
 {order.deliveredAt &&
 new Date((order.deliveredAt as any)?.toDate
 ...
 {/* Delivery Photo */}
 {order.deliveryPhotoUrl && (<div className="mt-6">
 <p className="text-sm font-medium text-foreground mb-2">
 Delivery Photo
 </p>
 <img
 src={order.deliveryPhotoUrl}
 alt="Delivery photo"className="w-full max-w-md h-64 object-cover rounded-lg border border-border"/>
 </div>)}
 </div>
 ...
 {order.requiresManualReview && (<div className="mt-6 bg-yellow-50 /20 border border-yellow-200 rounded-lg p-4">
 ...
 {/* Refund Summary */}
 {order.refundedCents && order.refundedCents > 0 && (<div className="mt-6 bg-success-light /20 border border-success-light0 rounded-lg p-6">

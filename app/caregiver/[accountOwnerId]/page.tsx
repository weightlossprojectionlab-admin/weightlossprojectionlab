...
interface CaregiverDashboardPageProps {params: Promise<{accountOwnerId: string}>}

export default function CaregiverDashboardPage({params}: CaregiverDashboardPageProps) {return (<AuthGuard>
 <CaregiverDashboardContent params={params} />
 </AuthGuard>)}

function CaregiverDashboardContent({params}: CaregiverDashboardPageProps) {const resolvedParams = use(params)
 ...
 const loadCaregiverData = async () => {try {setLoading(true)

 // Get user's profile to check caregiverOf and onboarding status
 const userDoc = await getDoc(doc(db,'users', user.uid))
 const userData = userDoc.data()

 if (!userData) {console.error('User data not found')
 router.push('/auth')
 return}
 ...
 const context = caregiverContexts.find((ctx: CaregiverContext) => ctx.accountOwnerId === accountOwnerId)

 if (!context) {console.error('Caregiver context not found for this account owner')
 ...
 const patientsSnapshot = await getDocs(collection(db,'users', accountOwnerId,'patients'))

 const allPatients: Patient[] = []
 patientsSnapshot.forEach((doc) => {const patientData = doc.data() as Patient
 // Only include patients this caregiver has access to
 if (context.patientsAccess.includes(doc.id)) {allPatients.push({...patientData, id: doc.id})}})

 setPatients(allPatients)} catch (error) {console.error('Error loading caregiver data:', error)} finally {setLoading(false)}}

 loadCaregiverData()}, [user, accountOwnerId, router])
 ...
 if (!caregiverContext) {return (<div className="min-h-screen flex items-center justify-center bg-background">
 <div className="text-center">
 ...
 .map(([permission, _]) => (<div key={permission} className="flex items-center gap-2 text-sm">
 ...
 {patients.length === 0 ? (<div className="bg-card rounded-lg border-2 border-border p-8 text-center">
 ...
 {patient.age && (<p className="text-sm text-muted-foreground mt-1">{patient.age} years old</p>)}
 </div>
 </div>
 ...
 {/* Help Section */}
 <div className="mt-8 bg-blue-50 /20 rounded-lg border-2 border-blue-200 p-6">
 <div className="flex items-start gap-3">
 <svg className="w-6 h-6 text-blue-600 flex-shrink-0 mt-0.5"fill="none"viewBox="0 0 24 24"stroke="currentColor">
 <path strokeLinecap="round"strokeLinejoin="round"strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
 </svg>
 <div>
 <h3 className="font-semibold text-blue-900 mb-2">About Caregiver Mode</h3>
 ...
 {!hasOwnAccount && (<p className="text-sm text-blue-800">

...
export default function RecipeIndexPage() {const searchParams = useSearchParams()
 ...
 useEffect(() => {if (!memberId) {setMemberProfile(null)
 setMemberRecipes([])
 return}

 const fetchMemberData = async () => {setLoadingMemberData(true)
 try {const user = auth.currentUser
 if (!user) {logger.warn('[Recipes] No authenticated user for member data')
 return}
 ...
 if (recipes.length > 0 && inventoryItems.length >= 0) {const suggestions = await getMemberRecipeSuggestions({patient: profile,
 medications: meds || [],
 recentVitals: vitals || [],
 ...
 logger.info('[Recipes] Generated member recipes', {memberId,
 count: suggestions.length,
 // conditions: profile.healthConditions?.length || 0 // healthConditions not in PatientProfile})}} catch (error) {logger.error('[Recipes] Error fetching member data', error as Error, {memberId})
 toast.error('Failed to load personalized recipes')} finally {setLoadingMemberData(false)}}

 fetchMemberData()}, [memberId, recipes, inventoryItems, selectedMealType])
 ...
 // Filter recipes
 const filteredRecipes = displayRecipes.filter(recipe => {// Filter by meal type
 if (selectedMealType !=='all' && recipe.mealType !== selectedMealType) {return false}
 ...
 const handleMediaUploadSuccess = () => {// No need to reload - useRecipes hook provides real-time updates via Firestore onSnapshot
 // The recipe list will automatically update when media is uploaded}

 return (<main className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-100 /20">
 {/* Marketing Banner */}
 {!memberId && (<div className="bg-gradient-to-r from-primary to-accent text-white py-3 px-4 text-center">
 ...
 {memberId && memberProfile && (<div className="bg-gradient-to-r from-green-600 to-blue-600 text-white py-4 px-4 border-b-2 border-green-700">
 ...
 <Link
 href="/recipes"className="text-white hover:underline text-sm font-medium">
 View All Recipes →
 </Link>
 </div>
 </div>)}
 ...
 {false && memberId && memberProfile && (<div className="bg-yellow-50 /20 border-b-2 border-yellow-300 py-3 px-4">
 ...
 {isAdminMode && (<div className="mt-4 space-y-3">
 <div className="bg-primary-light /20 border border-purple-300 text-purple-900 px-4 py-2 rounded-lg inline-block">
 <p className="text-sm font-medium">
 ...
 {/* Loading State */}
 {recipesLoading && (<div className="flex items-center justify-center py-12">
 <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
 <span className="ml-3 text-muted-foreground">Loading recipes...</span>
 </div>)}

 {/* Results Count */}
 {!recipesLoading && (<p className="text-muted-foreground mb-6">
 {filteredRecipes.length} recipe{filteredRecipes.length !== 1 ?'s' :''} found
 </p>)}

 {/* Recipe Grid */}
 {!recipesLoading && filteredRecipes.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
 {filteredRecipes.map(recipe => (<div
 key={recipe.id}
 ...
 {memberId &&'safetyResult' in recipe && recipe.safetyResult && (<div className="space-y-2 mb-4">
 ...
 {'inventoryAvailability' in recipe && recipe.inventoryAvailability && (<div className="flex items-center gap-2 text-xs">
 ...
 Edit Media
 {!recipe.imageUrls?.length && !recipe.videoUrl && (<span className="ml-2 bg-yellow-400 text-yellow-900 px-2 py-0.5 rounded text-xs">
 No media
 </span>)}
 {recipe.imageUrls && recipe.imageUrls.length > 0 && (<span className="ml-2 bg-green-400 text-green-900 px-2 py-0.5 rounded text-xs">
 {recipe.imageUrls.length} {recipe.imageUrls.length === 1 ?'image' :'images'}
 </span>)}
 </button>)}
 </div>
 </div>))}
 </div>) : !recipesLoading ? (<div className="bg-card rounded-lg shadow-lg p-12 text-center">
 ...
 <Link
 href="/auth"className="inline-block btn bg-background text-primary hover:bg-muted px-8 py-4 text-lg font-bold rounded-lg shadow-lg">
 Start Your Journey
 </Link>
 ...
 {/* Media Upload Modal */}
 {selectedRecipeForEdit && (<RecipeMediaUpload

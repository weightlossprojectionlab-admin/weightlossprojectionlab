 ...
 */
export default function RecipesLoading() {return (<main className="min-h-screen bg-gradient-to-br from-gray-50 to-purple-100 /20">
 ...
 {[1, 2, 3, 4, 5].map((i) => (<div key={i} className="h-10 w-24 bg-gray-200 rounded-lg"/>))}
 </div>
 </div>
 ...
 <div className="flex flex-wrap gap-2">
 {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (<div key={i} className="h-8 w-28 bg-gray-200 rounded-full"/>))}
 </div>
 </div>
 </div>
 ...
 {[1, 2, 3, 4, 5, 6].map((i) => (<div key={i} className="bg-card rounded-lg shadow-lg overflow-hidden animate-pulse">

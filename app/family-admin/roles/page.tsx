...
function ManageRolesContent() {const {user} = useAuth()
 ...
 const handleSaveMemberEdit = async (data: MemberUpdateData) => {if (!editingMember) return

 try {await updateMember(editingMember.userId, data)
 await refetch()
 setShowEditModal(false)
 setEditingMember(null)} catch (err) {console.error('Failed to update member:', err)
 throw err}}

 const toggleExpanded = (memberId: string) => {setExpandedMemberId(expandedMemberId === memberId ? null : memberId)}

 return (<div className="min-h-screen bg-background">
 <PageHeader
 title="Role Management"subtitle="Manage family member permissions and access"actions={<div className="flex gap-2">
 <Link
 href="/family-admin/dashboard"className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">
 <ArrowLeftIcon className="w-4 h-4"/>
 Back to Dashboard
 </Link>
 {currentUserRole ==='account_owner' && (<button
 ...
 </div>) : error ? (<div className="bg-error-light /20 border-2 border-red-200 rounded-lg p-6 text-center">
 <p className="text-error-dark font-medium">Error: {error}</p>
 </div>) : (<div className="space-y-8">
 {/* Role Hierarchy Info */}
 <div className="bg-blue-50 /20 rounded-lg p-6 border border-blue-200">
 ...
 {/* Co-Admins Section */}
 {coAdmins.length > 0 && (<section>
 <h2 className="text-lg font-bold text-foreground mb-3 flex items-center gap-2">
 <UserGroupIcon className="w-5 h-5 text-purple-500"/>
 Co-Admins ({coAdmins.length})
 </h2>
 <div className="space-y-3">
 {coAdmins.map(member => (<MemberCard
 key={member.userId}
 member={member}
 currentUserRole={currentUserRole}
 patients={patients || []}
 ...
 {/* Caregivers Section */}
 {caregivers.length > 0 && (<section>
 <h2 className="text-lg font-bold text-foreground mb-3 flex items-center gap-2">
 <UserGroupIcon className="w-5 h-5 text-blue-500"/>
 Caregivers ({caregivers.length})
 </h2>
 <div className="space-y-3">
 {caregivers.map(member => (<MemberCard
 key={member.userId}
 member={member}
 currentUserRole={currentUserRole}
 patients={patients || []}
 ...
 {/* Viewers Section */}
 {viewers.length > 0 && (<section>
 <h2 className="text-lg font-bold text-foreground mb-3 flex items-center gap-2">
 <EyeIcon className="w-5 h-5 text-gray-500"/>
 Viewers ({viewers.length})
 </h2>
 <div className="space-y-3">
 {viewers.map(member => (<MemberCard
 key={member.userId}
 member={member}
 currentUserRole={currentUserRole}
 patients={patients || []}
 ...
 {showEditModal && editingMember && (<EditMemberModal
...
interface MemberCardProps {member: FamilyMember
...
function MemberCard({member,
 ...
 canEdit,
 isExpanded,
 onToggleExpand}: MemberCardProps) {const roleCapabilitiesObj = ROLE_CAPABILITIES[member.familyRole] || {}

 // Convert capabilities object to array of readable strings
 const roleCapabilities = Object.entries(roleCapabilitiesObj)
 .filter(([_, value]) => value === true)
 .map(([key]) => {// Convert camelCase to readable text
 return key
 .replace(/([A-Z])/g,' $1')
 .replace(/^./, str => str.toUpperCase())
 .replace('Can','')})

 return (<div className="bg-card rounded-lg border-2 border-border p-5">
 <div className="flex items-start justify-between gap-4">
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-3 mb-2">
 <h3 className="font-bold text-foreground truncate">{member.name}</h3>
 {member.familyRole ==='account_owner' ? (<AccountOwnerBadge />) : (<RoleBadge role={member.familyRole} />)}
 </div>
 <p className="text-sm text-muted-foreground">{member.email}</p>

 {/* Patient Access */}
 {member.patientIds && member.patientIds.length > 0 && (<div className="mt-3">
 ...
 className="p-2 text-gray-600 hover:text-primary transition-colors"title="Edit member">
 <PencilSquareIcon className="w-5 h-5"/>
 </button>)}
 <button
 onClick={onToggleExpand}
 className="p-2 text-gray-600 hover:text-primary transition-colors"title={isExpanded ?'Collapse' :'Expand'}
 >
 ...
 {roleCapabilities.map((capability, index) => (<li key={index} className="flex items-start gap-2">

...
/**
 * Fetch ONLY the product image URL from OpenFoodFacts.
 *
 * Used by the hybrid lookup in lib/product-lookup-server.ts when USDA wins on
 * nutrition (USDA has no images) — we still want the OFF image when available.
 * Uses the v2 ?fields= filter to keep the response small.
 *
 * Returns the image URL string or null. Never throws.
 */
export async function fetchOpenFoodFactsImageOnly(barcode: string): Promise<string | null> {
  try {
    ...
    logger.warn('[OpenFoodFacts] image-only lookup failed', { barcode, error: (error as Error).message })
    return null
  }
}

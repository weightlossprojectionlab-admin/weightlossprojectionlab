...
interface WeightReminderResult {shouldShow: boolean
 daysSince: number
 ...
 */
export function shouldShowWeightReminder(lastWeightLog: WeightLog | null,
 frequency:'daily' |'weekly' |'biweekly' |'monthly' ='weekly',
 lastMealLogDate?: Date | null): WeightReminderResult {// If no weight logs exist, always show reminder
 if (!lastWeightLog) {return {shouldShow: true,
 daysSince: 0,
 ...
 if (lastMealLogDate) {daysSinceLastMealLog = Math.floor((now.getTime() - lastMealLogDate.getTime()) / (1000 * 60 * 60 * 24))
 ...
 if (isUserInactive) {return {shouldShow: true,
 ...
 const frequencyDays: Record<string, number> = {daily: 1,
 weekly: 7,
 biweekly: 14,
 monthly: 30}
 ...
 if (frequency ==='daily') {if (loggedToday) {// Already logged today - no reminder
 return {shouldShow: false,
 daysSince: 0,
 nextDueDate: new Date(todayDate.getTime() + 24 * 60 * 60 * 1000), // Tomorrow
 isOverdue: false,
 status:'on-track'}} else if (daysSinceLastLog === 1) {// Last log was yesterday - due today
 return {shouldShow: true,
 daysSince: 1,
 nextDueDate: todayDate,
 isOverdue: false,
 status:'due-soon'}} else {// Overdue (2+ days)
 return {shouldShow: true,
 daysSince: daysSinceLastLog,
 nextDueDate: todayDate,
 isOverdue: true,
 ...
 if (isOverdue) {status ='overdue'} else if (isDueSoon) {status ='due-soon'} else {status ='on-track'}
 ...
 */
export function getWeightReminderMessage(result: WeightReminderResult, frequency: string): string {if (result.daysSince === 0) {return`Log your first weight to start tracking progress!`}

 if (result.status ==='inactive') {return`Welcome back! It's been ${result.daysSinceLastMealLog || 0} days since your last meal log. Let's get back on track by logging your current weight!`}

 if (result.isOverdue) {const daysOverdue = result.daysSince - (frequency ==='daily' ? 1 : frequency ==='weekly' ? 7 : frequency ==='biweekly' ? 14 : 30)
 return`Time for your ${frequency} weigh-in! Last logged ${result.daysSince} days ago (${daysOverdue} days overdue).`}

 if (result.status ==='due-soon') {return`Your ${frequency} weigh-in is due tomorrow. Last logged ${result.daysSince} days ago.`}
 ...
 */
export function getWeightReminderColor(status:'on-track' |'due-soon' |'overdue' |'inactive'): {border: string
 bg: string
 text: string
 badge: string} {switch (status) {case'on-track':
 ...
 badge:'bg-yellow-100 text-warning-dark'}
 case'overdue':
 ...
 case'inactive':
 return {border:'border-secondary-light',
 bg:'bg-secondary-light',
 text:'text-blue-700',
 badge:'bg-blue-100 text-secondary-dark'}}}

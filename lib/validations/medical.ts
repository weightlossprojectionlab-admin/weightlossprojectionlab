...
export const patientProfileSchema = z.object({
  ...
  (data) => {
    // If type is 'human', gender should be provided
    if (data.type === 'human') {
      return !!data.gender
    }
    return true
  ...
  {
    message: 'Gender is recommended for humans',
    path: ['gender']
  }
)
...
export const familyInvitationSchema = z.object({
  ...
  patientsShared: z.array(z.string().uuid()).min(1, 'At least one patient must be shared'),
  permissions: familyMemberPermissionsSchema,
  message: z.string().max(500).optional(),
  createdAt: z.string().datetime(),

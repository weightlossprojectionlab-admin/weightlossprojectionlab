...
export interface GenericNotificationEmailParams {
  /** Already-built notification title (e.g. "💓 Blood pressure reminder for Baby 3") */
  title: string
  /** Already-built body text (e.g. "Time to log Baby 3's blood pressure.") */
  body: string
  /** Absolute URL the CTA button links to. Pass through buildVitalReminder's `link` */
  ctaUrl: string
  /** Optional CTA button label override (default: "Open in app") */
  ctaLabel?: string
  /** Optional preheader — the short preview text shown next to the subject in
   *  most inboxes. Defaults to the body. */
  preheader?: string
  /** Recipient's display name, when known (used in greeting). Defaults to
   *  no greeting (just the title) for shorter notifications. */
  recipientName?: string
}

/**
 * Generic notification email — used by lib/notifications/dispatch.ts when
 * fanning out to the email channel for any NotificationType that doesn't
 * have a dedicated template (vital reminders, duty assignments, etc).
 *
 * Matches the purple-gradient styling of the medication / vital / document
 * templates above so the user sees a consistent brand across notification
 * types. Plain-text alternative is generated by stripping markup so spam
 * filters favoring multipart/alternative get what they expect.
 */
export function generateGenericNotificationEmail(params: GenericNotificationEmailParams): { html: string; text: string; subject: string } {
  const { title, body, ctaUrl, ctaLabel = 'Open in app', preheader, recipientName } = params

  const greeting = recipientName ? `<h2 style="${emailStyles.contentTitle}">Hello ${recipientName},</h2>` : ''
  const previewText = preheader ?? body
  ...
  const html = `
    ...
    <body style="${emailStyles.body}">
      <!-- Preheader: hidden but pulled by inboxes for the preview line -->
      <div style="display:none; max-height:0; overflow:hidden; mso-hide:all;">${previewText}</div>

      <div style="${emailStyles.header}">
        <h1 style="${emailStyles.headerTitle}">${title}</h1>
        <p style="${emailStyles.headerSubtitle}">WPL Family Health</p>
      </div>
      ...
      <div style="${emailStyles.content}">
        ...
        <p style="color: #6b7280; font-size: 14px; margin: 0;">
          You're receiving this because notifications are enabled in your WPL Family Health profile.
          You can adjust which notifications you get under <strong>Profile → Notification Preferences</strong>.
        </p>
      </div>
      ...
      <div style="${emailStyles.footer}">
        ...
        <p style="margin: 10px 0 0 0;">
          This is an automated notification — replies aren't monitored.
        </p>
      </div>
    </body>
    </html>
...
---
You're receiving this because notifications are enabled in your WPL Family Health profile.
Adjust your preferences under Profile → Notification Preferences.
...
export const notificationEmailTemplates = {
  ...
  generic: generateGenericNotificationEmail,
}

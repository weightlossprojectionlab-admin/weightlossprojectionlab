/**
 * Health Report Generator — shared rule-based markdown report builder.
 *
 * Single source of truth for the clinical markdown report. Consumes the
 * `analyses` object from generateHealthSummary() and formats the full report
 * (executive summary, vitals, meds, doctor discussion points, caregiver
 * actions, shopping lists, etc.). Used by both the /ai-health-report and
 * /health-reports routes so they never diverge again.
 */

import { getCriticalAlerts, calculateHealthScore, getVitalStatus } from '@/lib/health-summary-generator'
...
export function generateHealthReport(data: any): string {
  const {
    patient,
    age,
    isPet,
  ...
  if (vitalsAnalysis.status === 'recorded' && vitalsAnalysis.recent.length > 0) {
    ...
    // Averages / range over the reporting window — the doctor-facing rollup.
    const averages = vitalsAnalysis.averages as Array<any> | undefined
    if (averages && averages.length > 0) {
      report += `### Averages (this period)\n\n`
      report += `| Vital | Average | Range (min–max) | Latest | Readings |\n`
      report += `|-------|---------|-----------------|--------|----------|\n`
      averages.forEach((a: any) => {
        if (a.type === 'blood_pressure') {
          const avg = `${a.averageSystolic}/${a.averageDiastolic} mmHg`
          const latest = a.latestSystolic != null ? `${a.latestSystolic}/${a.latestDiastolic} mmHg` : '—'
          report += `| ${a.label} | ${avg} | — | ${latest} | ${a.count} |\n`
        } else {
          const unit = a.unit ? ` ${a.unit}` : ''
          const range = `${a.min}–${a.max}${unit}`
          const latest = a.latest != null ? `${a.latest}${unit}` : '—'
          report += `| ${a.label} | ${a.average}${unit} | ${range} | ${latest} | ${a.count} |\n`
        }
      })
      report += `\n`
    }

 ...
 */
export function getDaysUntilExpiration(expiresAt: Date): number {const now = new Date()
 ...
 */
export function getExpirationSeverity(daysUntil: number):'info' |'warning' |'critical' |'expired' {if (daysUntil < 0) return'expired'
 ...
 */
export function createExpirationAlert(item: ShoppingItem): ExpirationAlert | null {if (!item.expiresAt || !item.inStock) return null
 ...
 */
export function getExpirationAlerts(items: ShoppingItem[],
 options: {includeExpired?: boolean
 daysAhead?: number
 sortBySeverity?: boolean} = {}): ExpirationAlert[] {const {includeExpired = true,
 daysAhead = 7,
 sortBySeverity = true} = options

 const alerts: ExpirationAlert[] = items
 .filter(item => item.expiresAt && item.inStock)
 .map(item => createExpirationAlert(item))
 .filter((alert): alert is ExpirationAlert => {if (!alert) return false
 if (!includeExpired && alert.daysUntilExpiration < 0) return false
 if (alert.daysUntilExpiration > daysAhead) return false
 return true})

 if (sortBySeverity) {const severityOrder = {expired: 0, critical: 1, warning: 2, info: 3}
 alerts.sort((a, b) => {const severityDiff = severityOrder[a.severity] - severityOrder[b.severity]
 ...
 */
export function getExpirationSummary(items: ShoppingItem[]): {total: number
 expiring3Days: number
 expiring7Days: number
 expired: number
 critical: number
 warning: number} {const now = new Date()
 ...
 */
export function calculateTypicalShelfLife(item: ShoppingItem): number | undefined {if (!item.purchaseHistory || item.purchaseHistory.length < 2) return undefined

 const shelfLives: number[] = item.purchaseHistory
 .filter(entry => entry.expiresAt)
 .map(entry => {const purchaseDate = entry.date
 ...
 */
export function calculateConsumptionRate(item: ShoppingItem): number | undefined {if (!item.purchaseHistory || item.purchaseHistory.length < 2) return undefined

 const sortedHistory = [...item.purchaseHistory].sort((a, b) => a.date.getTime() - b.date.getTime())

 const intervals: number[] = []
 for (let i = 1; i < sortedHistory.length; i++) {const prevDate = sortedHistory[i - 1].date
 ...
 */
export function predictRepurchaseDate(item: ShoppingItem): Date | undefined {if (!item.lastPurchased) return undefined
 ...
 */
export function shouldAutoAddToShoppingList(item: ShoppingItem): boolean {// If already needed, don't auto-add again
 ...
 const now = new Date()
 const daysUntilRepurchase = Math.ceil((predictedRepurchase.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
 ...
 */
export function getExpirationNotificationMessage(alert: ExpirationAlert): {title: string
 body: string
 emoji: string} {const {productName, daysUntilExpiration, severity} = alert

 switch (severity) {case'expired':
 return {title:'🚨 Food Expired',
 body:`${productName} expired ${Math.abs(daysUntilExpiration)} day${Math.abs(daysUntilExpiration) !== 1 ?'s' :''} ago`,
 emoji:'🚨'}

 case'critical':
 if (daysUntilExpiration === 0) {return {title:'⚠️ Expires Today',
 body:`${productName} expires today! Use it soon.`,
 emoji:'⚠️'}}
 return {title:'⚠️ Expiring Soon',
 ...
 */
export function sortByExpirationPriority(items: ShoppingItem[]): ShoppingItem[] {return [...items].sort((a, b) => {// Items without expiration go to end
 ...
 */
export function isExpired(item: ShoppingItem): boolean {if (!item.expiresAt) return false
 return getDaysUntilExpiration(item.expiresAt) < 0}

/**
 * Check if item is expiring soon (within 3 days)
 */
export function isExpiringSoon(item: ShoppingItem): boolean {if (!item.expiresAt) return false
 const daysUntil = getDaysUntilExpiration(item.expiresAt)
 ...
 */
export function getExpirationColor(severity:'info' |'warning' |'critical' |'expired'): {bg: string
 text: string
 border: string} {switch (severity) {case'expired':
 return {bg:'bg-error-light /20',
 text:'text-red-900',
 border:'border-red-500'}
 case'critical':
 return {bg:'bg-orange-50 /20',
 text:'text-orange-900',
 border:'border-orange-500'}
 case'warning':
 return {bg:'bg-warning-light',
 text:'text-yellow-900',
 border:'border-yellow-500'}
 case'info':
 return {bg:'bg-secondary-light',
 text:'text-blue-900',

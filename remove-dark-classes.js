...
async function removeDarkClasses() {// Find all TSX and JSX files in components directory
 ...
 for (const file of files) {let content = fs.readFileSync(file,'utf8');
 const originalContent = content;

 // Remove all dark: Tailwind classes
 // Pattern matches: including variants like /30
 content = content.replace(/\s*dark:[a-zA-Z0-9_\-:\/\[\]\.%#]+/g,'');

 if (content !== originalContent) {fs.writeFileSync(file, content,'utf8');
 ...
 if (modifiedFiles.length > 0) {console.log('\nModified files:');

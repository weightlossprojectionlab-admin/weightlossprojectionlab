...
const replacements = [// BACKGROUNDS - Most common patterns first
 ...
 {pattern: /bg-primary-light \/30/g, replacement:'bg-primary-light'},
 {pattern: /bg-primary-light(?!-)/g, replacement:'bg-primary-light'},
 {pattern: /bg-purple-50 \/20/g, replacement:'bg-primary-light'},
 ...
 {pattern: //g, replacement:''},
 {pattern: / \/30/g, replacement:''},
 {pattern: / \/50/g, replacement:''},
 ...
 {pattern: /bg-warning-light \/20/g, replacement:'bg-warning-light'},
 ...
 {pattern: /bg-secondary-light \/20/g, replacement:'bg-secondary-light'},
...
const hoverCleanup = [{pattern: /hover:text-foreground/g, replacement:'hover:text-foreground'},
...
async function processFile(filePath) {try {let content = fs.readFileSync(filePath,'utf8');
 let originalContent = content;
 let changeCount = 0;

 // Apply all replacements
 for (const {pattern, replacement} of allReplacements) {const beforeLength = content.length;
 content = content.replace(pattern, replacement);
 if (content.length !== beforeLength) {changeCount++;}}
 ...
 return 0;} catch (error) {console.error(`✗ Error processing ${filePath}:`, error.message);
 return 0;}}

// Recursive function to find all files
function findFiles(dir, fileList = []) {const files = fs.readdirSync(dir);

 files.forEach(file => {const filePath = path.join(dir, file);
 const stat = fs.statSync(filePath);

 if (stat.isDirectory()) {// Skip excluded directories
 if (!['node_modules','.next','dist','build','.git','coverage'].includes(file)) {findFiles(filePath, fileList);}} else if (/\.(tsx|jsx|ts|js)$/.test(file) && !file.includes('.test.') && !file.includes('.spec.')) {fileList.push(filePath);}});

 return fileList;}

async function main() {console.log('🎨 Starting CSS Color Unification...\n');
 ...
 for (const file of files) {const changes = await processFile(file);

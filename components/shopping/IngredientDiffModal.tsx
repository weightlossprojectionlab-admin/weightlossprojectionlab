...
interface IngredientDiffModalProps {recipeId: string
 recipeName: string
 ingredients: RecipeIngredient[]
 onClose: () => void}

export function IngredientDiffModal({recipeId,
 recipeName,
 ingredients,
 onClose,}: IngredientDiffModalProps) {const {allItems: inventoryItems, loading: inventoryLoading} = useInventory()
 ...
 useEffect(() => {const comparison = compareWithInventory(ingredients, inventoryItems)
 setDiffs(comparison)
 ...
 setSelectedItems(new Set(needIndices))}, [ingredients, inventoryItems])

 const toggleItem = (index: number) => {const newSelected = new Set(selectedItems)
 if (newSelected.has(index)) {newSelected.delete(index)} else {newSelected.add(index)}
 setSelectedItems(newSelected)}

 const handleAddToList = async () => {try {// Get selected diffs
 const selectedDiffs = diffs.filter((_, index) => selectedItems.has(index))
 const itemsToAdd = getItemsToAdd(selectedDiffs)

 if (itemsToAdd.length === 0) {toast.error('No items selected')
 return}

 await addFromRecipe(recipeId, itemsToAdd)

 toast.success(`Added ${itemsToAdd.length} items to shopping list`)
 onClose()} catch (error) {toast.error('Failed to add items to shopping list')}}
 ...
 <button
 onClick={onClose}
 className="text-muted-foreground hover:text-muted-foreground"aria-label="Close modal">
 ...
 {partialCount > 0 && (<div className="flex items-center gap-1">
 ...
 {inventoryLoading ? (<div className="flex justify-center py-8">
 <Spinner size="lg"/>
 </div>) : (<div className="space-y-2">
 {diffs.map((diff, index) => (<div
 key={index}
 className={`flex items-center gap-3 p-3 rounded-lg border ${diff.status ==='have'
 ?'bg-success-light /20 border-green-200'
 :'bg-background /50 border-border'}`}
 >
 {/* Checkbox (only for items that need to be added) */}
 {diff.status !=='have' && (<input
 type="checkbox"checked={selectedItems.has(index)}
 onChange={() => toggleItem(index)}
 className="h-4 w-4 text-secondary rounded"/>)}

 {/* Status Icon */}
 {diff.status ==='have' && (<CheckCircleIcon className="h-5 w-5 text-green-500 flex-shrink-0"/>)}
 {diff.status ==='partial' && (<ExclamationCircleIcon className="h-5 w-5 text-warning flex-shrink-0"/>)}
 {diff.status ==='need' && (<PlusCircleIcon className="h-5 w-5 text-blue-500 flex-shrink-0"/>)}

...
interface PurchaseResult {quantity: number
 unit?: QuantityUnit
 expirationDate?: Date
...
interface SequentialShoppingFlowProps {isOpen: boolean
 item: ShoppingItem
 onComplete: (result: PurchaseResult) => Promise<void>
 onCancel: () => void}

export function SequentialShoppingFlow({isOpen,
 item,
 onComplete,
 onCancel}: SequentialShoppingFlowProps) {// Flow state
 ...
 useEffect(() => {if (isOpen) {setCurrentStep('SCANNING')
 setScannedProduct(null)
 setReplacementProduct(null)
 setSelectedQuantity(item.quantity || 1)
 setSelectedCategory(item.category)
 setExpirationDate(undefined)
 setShowFamilyChat(false)}}, [isOpen, item])

 /**
 * Handle barcode scan
 */
 const handleBarcodeScan = async (barcode: string) => {try {toast.loading('Looking up product...', {id:'scan'})

 const response = await lookupBarcodeWithCache(barcode)
 const product = simplifyProduct(response)

 if (!product.found || !response.product) {toast.error('Product not found in database', {id:'scan'})
 ...
 if (user && response.product) {updateGlobalProductDatabase(barcode,
 response.product,
 user.uid,
 ...
 context:'shopping'}).catch(err => {// Silently log errors - don't block user flow
 logger.debug('[GlobalProduct] Failed to update database (non-critical)', err)})}

 // Store scanned product based on current step
 if (currentStep ==='SCANNING') {setScannedProduct(response.product)
 setCurrentStep('NUTRITION_REVIEW')} else if (currentStep ==='SCAN_REPLACEMENT') {setReplacementProduct(response.product)
 setCurrentStep('COMPARE_REPLACEMENT')}} catch (error) {logger.error('Error scanning barcode', error as Error)
 toast.error('Failed to scan product', {id:'scan'})}}

 /**
 * Handle nutrition review confirmation
 */
 const handleNutritionConfirm = () => {setCurrentStep('CATEGORY_CONFIRM')}

 /**
 * Handle category confirmation
 */
 const handleCategoryConfirm = (category: ProductCategory) => {setSelectedCategory(category)
 setCurrentStep('QUANTITY_ADJUST')}

 /**
 * Handle quantity confirmation
 */
 const handleQuantityConfirm = (quantity: number) => {setSelectedQuantity(quantity)

 // Check if item is perishable
 const categoryMeta = getCategoryMetadata(selectedCategory)
 if (categoryMeta.isPerishable) {setCurrentStep('EXPIRATION_PICKER')} else {// Non-perishable - complete immediately
 handleComplete()}}

 /**
 * Handle expiration date selection
 */
 const handleExpirationSelected = (date: Date) => {setExpirationDate(date)
 handleComplete()}

 /**
 * Handle skip expiration
 */
 const handleSkipExpiration = () => {setExpirationDate(undefined)
 handleComplete()}

 /**
 * Complete the purchase
 */
 const handleComplete = async () => {if (!scannedProduct) return

 setIsSubmitting(true)
 try {const result: PurchaseResult = {quantity: selectedQuantity,
 unit: item.unit,
 expirationDate,
 ...
 setTimeout(() => {onCancel()}, 1500)} catch (error) {logger.error('Error completing purchase', error as Error)
 toast.error('Failed to complete purchase')} finally {setIsSubmitting(false)}}

 /**
 * Handle scan replacement
 */
 const handleScanReplacement = () => {setCurrentStep('SCAN_REPLACEMENT')}

 /**
 * Handle replacement acceptance
 */
 const handleReplacementAccept = () => {setCurrentStep('NUTRITION_REVIEW')}

 /**
 * Handle replacement rejection
 */
 const handleReplacementReject = () => {setReplacementProduct(null)
 setCurrentStep('ITEM_NOT_FOUND')}

 /**
 * Handle skip item
 */
 const handleSkipItem = () => {toast('Item skipped', {icon:'⏭️'})
 onCancel()}

 if (!isOpen) return null

 // Get step info for progress indicator
 const steps = [{key:'SCANNING', label:'Scan'},
 ...
 key={step.key}
 className={`flex-1 h-1 rounded ${idx <= currentStepIndex ?'bg-primary' :'bg-gray-200'}`}
 />))}
 </div>
 ...
 <div className="p-6">
 {/* SCANNING */}
 {(currentStep ==='SCANNING' || currentStep ==='SCAN_REPLACEMENT') && (<BarcodeScanner
 isOpen={true}
 ...
 {/* NUTRITION REVIEW */}
 {currentStep ==='NUTRITION_REVIEW' && scannedProduct && (<NutritionReviewModal
 isOpen={true}
 onClose={onCancel}
 ...
 {/* CATEGORY CONFIRM */}
 {currentStep ==='CATEGORY_CONFIRM' && scannedProduct && (<CategoryConfirmModal
 isOpen={true}
 onClose={onCancel}
 ...
 {/* ITEM NOT FOUND */}
 {currentStep ==='ITEM_NOT_FOUND' && (<div className="text-center">
 ...
 className="w-full py-3 px-4 bg-gray-200 hover:bg-gray-300 text-foreground rounded-lg font-medium transition-colors">
 Skip This Item
 </button>
 </div>
 </div>)}
 ...
 {/* COMPARE REPLACEMENT */}
 {currentStep ==='COMPARE_REPLACEMENT' && replacementProduct && (<ReplacementCompareModal
 isOpen={true}
 onClose={onCancel}
 originalItem={item}
 ...
 {/* QUANTITY ADJUST */}
 {currentStep ==='QUANTITY_ADJUST' && (<QuantityAdjustModal
 isOpen={true}
 onClose={onCancel}
 ...
 {/* EXPIRATION PICKER */}
 {currentStep ==='EXPIRATION_PICKER' && scannedProduct && (<ExpirationPicker
 isOpen={true}
 onClose={onCancel}
 category={selectedCategory}
 productName={scannedProduct.product_name ||'Product'}
 ...
 {/* COMPLETE */}
 {currentStep ==='COMPLETE' && (<div className="text-center py-8">
 <div className="text-6xl mb-4">✅</div>
 ...
 {currentStep !=='COMPLETE' && (<button
 onClick={() => setShowFamilyChat(true)}
 className="fixed bottom-20 right-4 w-14 h-14 bg-primary hover:bg-primary-hover text-white rounded-full shadow-xl z-[60] flex items-center justify-center transition-colors"title="Ask Family">
 <span className="text-2xl">💬</span>
 </button>)}

 {/* Family Chat Drawer (Coming Soon) */}
 {showFamilyChat && (<div className="fixed inset-0 z-[70] bg-black/50 flex items-end justify-center">

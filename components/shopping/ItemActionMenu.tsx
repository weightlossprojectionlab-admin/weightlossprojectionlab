...
interface ItemActionMenuProps {isOpen: boolean
 onClose: () => void
 item: ShoppingItem
 onScan: () => void
...
export function ItemActionMenu({isOpen,
 onClose,
 item,
 onScan,
 onAdjustQuantity,
 onReplacement,
 onChat}: ItemActionMenuProps) {if (!isOpen) return null
 ...
 className="w-16 h-16 rounded-lg object-cover"/>) : (<div className="w-16 h-16 bg-muted rounded-lg flex items-center justify-center text-3xl">
 {categoryMeta.icon}
 </div>)}
 ...
 onClick={onChat}
 disabled
 className="w-full px-4 py-3 bg-gray-300 text-muted-foreground rounded-lg cursor-not-allowed flex items-center gap-3 relative opacity-60"title="Coming soon - collaborate with family on shopping">
 <svg className="w-6 h-6 flex-shrink-0"fill="none"stroke="currentColor"viewBox="0 0 24 24">
 ...
 <span className="absolute top-2 right-2 text-xs bg-gray-400 text-white px-2 py-0.5 rounded-full font-medium">
 Soon
 </span>
 </button>

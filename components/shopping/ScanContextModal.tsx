...
export interface ScanContextModalProps {isOpen: boolean
 onClose: () => void
 onSelectContext: (context: ScanContext) => void
 excludeContexts?: ScanContext[] // Optional: contexts to hide from the modal}

export function ScanContextModal({isOpen, onClose, onSelectContext, excludeContexts = []}: ScanContextModalProps) {if (!isOpen) return null

 const allOptions: {context: ScanContext; icon: string; title: string; description: string; color: string}[] = [{context:'purchase',
 icon:'🛒',
 title:'Just Bought This',
 description:'Add newly purchased items to inventory',
 color:'bg-success-light /20 border-green-200 hover:bg-green-100 /30'},
 {context:'consume',
 icon:'🗑️',
 title:'Used Up / Threw Away',
 description:'Mark items as consumed and add to shopping list',
 color:'bg-orange-50 /20 border-orange-200 hover:bg-orange-100 /30'},
 {context:'inventory',
 icon:'📦',
 title:'Check Inventory',
 description:'View or update quantity of items you have',
 color:'bg-secondary-light border-secondary-light hover:bg-blue-100 /30'},
 {context:'meal',
 icon:'🍽️',
 title:'Log a Meal',
 description:'Record food consumption for tracking',
 color:'bg-primary-light border-primary-light hover:bg-primary-light /30'}]
 ...
 type="button"key={option.context}

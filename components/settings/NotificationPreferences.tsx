...
interface NotificationCategory {title: string
 description?: string
 types: Array<{type: NotificationType
 label: string
 description: string}>}

const NOTIFICATION_CATEGORIES: NotificationCategory[] = [{title:'Medication & Health',
 description:'Medication changes and health monitoring',
 types: [{type:'medication_added',
 ...
 types: [{type:'appointment_scheduled',
 ...
 types: [{type:'family_member_invited',
...
export function NotificationPreferences({userId}: NotificationPreferencesProps) {const [preferences, setPreferences] = useState<NotificationPrefsType>(DEFAULT_NOTIFICATION_PREFERENCES)
 ...
 useEffect(() => {loadPreferences()}, [userId])

 const loadPreferences = async () => {try {setLoading(true)
 ...
 setPreferences(data.data)} catch (error) {logger.error('Error loading notification preferences', error as Error)
 toast.error('Failed to load notification preferences')} finally {setLoading(false)}}

 const handleSavePreferences = async () => {try {setSaving(true)

 const user = auth.currentUser
 if (!user) {throw new Error('User not authenticated')}

 const idToken = await user.getIdToken()
 const response = await fetch('/api/notifications/preferences', {method:'PATCH',
 headers: {'Authorization':`Bearer ${idToken}`,'Content-Type':'application/json'},
 ...
 setHasChanges(false)} catch (error) {logger.error('Error saving notification preferences', error as Error)
 toast.error('Failed to save notification preferences')} finally {setSaving(false)}}

 const handleToggleMaster = (enabled: boolean) => {setPreferences((prev) => ({...prev,
 globallyEnabled: enabled}))
 setHasChanges(true)}

 const handleToggleChannel = (type: NotificationType,
 channel:'email' |'push',
 enabled: boolean) => {setPreferences((prev) => ({...prev,
 [type]: {...prev[type],
 [channel]: enabled}}))
 setHasChanges(true)}

 const handleToggleQuietHours = (enabled: boolean) => {setPreferences((prev) => ({...prev,
 quietHours: {...prev.quietHours!,
 enabled}}))
 setHasChanges(true)}

 const handleQuietHourChange = (field:'startHour' |'endHour', value: number) => {setPreferences((prev) => ({...prev,
 quietHours: {...prev.quietHours!,
 [field]: value}}))
 setHasChanges(true)}

 const handleSendTestNotification = async () => {setSendingTest(true)
 ...
 const notification = new Notification('Test Notification', {body:'Your notification preferences are working! You will receive helpful updates based on your settings.',
 ...
 logger.info('[NotificationPreferences] Test notification sent successfully')}} catch (error) {logger.error('Error sending test notification:', error as Error)
 toast.error('Failed to send test notification:' + (error as Error).message)} finally {setSendingTest(false)}}

 if (loading) {return (<div className="bg-card rounded-lg p-6 shadow-sm">
 <div className="flex items-center justify-center py-8">
 <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
 </div>
 </div>)}

 return (<div className="bg-card rounded-lg p-6 shadow-sm space-y-6">
 {/* Header */}
 <div>
 ...
 {/* Master Toggle */}
 <div className="p-4 bg-accent-light /20 border border-indigo-200 rounded-lg">
 ...
 <input
 type="checkbox"className="sr-only peer"checked={preferences.globallyEnabled}
 ...
 {preferences.globallyEnabled && (<div className="space-y-6">
 {NOTIFICATION_CATEGORIES.map((category) => (<div key={category.title} className="border border-border rounded-lg p-4">
 <div className="mb-4">
 ...
 <label className="relative inline-flex items-center cursor-pointer">
 <input
 type="checkbox"className="sr-only peer"checked={preferences[item.type]?.email ?? true}
 onChange={(e) =>
 ...
 <label className="relative inline-flex items-center cursor-pointer">
 <input
 type="checkbox"className="sr-only peer"checked={preferences[item.type]?.push ?? true}
 onChange={(e) =>
 ...
 <label className="relative inline-flex items-center cursor-pointer">
 <input
 type="checkbox"className="sr-only peer"checked={preferences.quietHours?.enabled ?? true}
 ...
 {Array.from({length: 24}, (_, i) => (<option key={i} value={i}>
 {i === 0
 ?'12 AM'
 : i < 12
 ?`${i} AM`: i === 12
 ...
 {/* Save Button */}
 {hasChanges && (<div className="sticky bottom-4 p-4 bg-warning-light border-2 border-warning-dark rounded-lg shadow-lg">

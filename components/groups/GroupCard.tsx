...
// Displays individual group information'use client';
...
export default function GroupCard({group, isMember = false, onJoin, onLeave, onClick}: GroupCardProps) {const memberCount = group.memberIds?.length || 0;
 ...
 return (<div
 className="bg-card border border-border rounded-lg p-5 hover:shadow-lg transition-all cursor-pointer"onClick={() => onClick?.(groupIdToUse)}
 >
 {/* Header */}
 ...
 {isPrivate ? (<LockClosedIcon className="h-4 w-4 text-muted-foreground"title="Private Group"/>) : (<GlobeAltIcon className="h-4 w-4 text-muted-foreground"title="Public Group"/>)}
 </div>
 ...
 {group.createdAt && (<span className="text-xs">
 Created {formatTimestamp(group.createdAt)}
 </span>)}
 </div>

 {/* Mission Info */}
 {group.activeMissionId && (<div className="bg-indigo-100 /20 border border-accent rounded-lg p-3 mb-4">
 ...
 <div
 className={`h-full rounded-full transition-all ${isFull ?'bg-error-dark' :'bg-success'}`}

...
// Visual display of risk score and contributing signals'use client';
...
interface RiskScoreDisplayProps {assessment: RiskScoreResult;
 showDetails?: boolean;}

export default function RiskScoreDisplay({assessment, showDetails = true}: RiskScoreDisplayProps) {const {score, signals, recommendation, confidence} = assessment;

 const getRiskLevel = (score: number): {label: string; color: string; bgColor: string; icon: any} => {if (score >= 70) {return {label:'High Risk',
 color:'text-error-dark',
 bgColor:'bg-error-light border-error',
 icon: ExclamationTriangleIcon};} else if (score >= 40) {return {label:'Medium Risk',
 color:'text-warning',
 bgColor:'bg-orange-50 border-orange-200',
 icon: ShieldExclamationIcon};} else {return {label:'Low Risk',
 ...
 return (<div className={`border rounded-lg p-6 ${riskLevel.bgColor}`}>
 {/* Header with Score */}
 ...
 <div
 className={`h-full rounded-full transition-all duration-500 ${score >= 70 ?'bg-error-light0' :
 score >= 40 ?'bg-orange-500' :'bg-success'}`}
 ...
 {/* Recommendation */}
 {recommendation && (<div className="bg-card rounded-lg border border-border p-4 mb-4">
 ...
 {confidence !== undefined && (<div className="text-right ml-4">
 ...
 {showDetails && signals && signals.length > 0 && (<div>
 <h4 className="text-sm font-semibold text-foreground mb-3">Contributing Factors</h4>
 <div className="space-y-2">
 {signals
 .sort((a, b) => b.weight - a.weight)
 .map((signal, index) => (<div key={index} className="bg-card rounded-lg border border-border p-3">
 <div className="flex items-start justify-between mb-1">
 <span className="text-sm font-medium text-foreground capitalize">
 {signal.signal.replace(/_/g,'')}
 </span>
 <span className={`text-sm font-bold ${signal.weight >= 30 ?'text-error-dark' :
 signal.weight >= 15 ?'text-warning' :'text-warning'}`}>
 +{signal.weight}
 </span>
 </div>
 ...
 {confidence !== undefined && (<div className="mt-4 p-3 bg-indigo-100 /20 border border-accent rounded-lg">

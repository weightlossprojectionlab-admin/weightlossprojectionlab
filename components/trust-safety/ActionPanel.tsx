...
// Admin action buttons for moderation decisions'use client';
...
interface ActionPanelProps {case: DisputeCase;
 onAction: (action: AdminAction, notes: string) => Promise<void>;
 disabled?: boolean;}

export default function ActionPanel({case: disputeCase, onAction, disabled = false}: ActionPanelProps) {const [isLoading, setIsLoading] = useState(false);
 const [selectedAction, setSelectedAction] = useState<AdminAction | null>(null);
 const [notes, setNotes] = useState('');
 const [showConfirm, setShowConfirm] = useState(false);

 const handleActionClick = (action: AdminAction) => {setSelectedAction(action);
 setShowConfirm(true);};

 const handleConfirm = async () => {if (!selectedAction || isLoading) return;

 setIsLoading(true);
 try {await onAction(selectedAction, notes);
 setShowConfirm(false);
 setSelectedAction(null);
 setNotes('');} catch (error) {logger.error('Action failed:', error as Error);
 // Optionally show error toast} finally {setIsLoading(false);}};
 ...
 if (disputeCase.status ==='resolved') {return (<div className="bg-success-light border border-success rounded-lg p-6 text-center">
 ...
 {!showConfirm ? (<div className="space-y-3">
 {/* Approve Full Refund */}
 <button
 ...
 disabled={disabled}
 className="w-full flex items-center justify-between px-4 py-3 bg-indigo-100 /20 border border-accent rounded-lg hover:bg-indigo-100 /20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
 <div className="flex items-center space-x-3">
 ...
 {/* Recommendation Hint */}
 {disputeCase.recommendation && (<div className="mt-4 p-3 bg-indigo-100 /20 border border-accent rounded-lg">
 ...
 {disputeCase.confidence && (<span className="ml-2 text-xs">
 ...
 placeholder="Provide reasoning for this decision..."className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"disabled={isLoading}
 />
 </div>
 ...
 className={`flex-1 px-4 py-2 rounded-lg font-medium transition-colors ${isLoading || !notes.trim()

...
// Displays individual dispute/case information'use client';
...
interface CaseCardProps {case: DisputeCase;
 onClick?: (caseId: string) => void;}

export default function CaseCard({case: disputeCase, onClick}: CaseCardProps) {const getStatusColor = (status: string) => {switch (status) {case'pending':
 return'bg-yellow-100 text-warning-dark';
 case'under_review':
 return'bg-indigo-100 text-accent-dark /20';
 case'resolved':
 ...
 const getRiskLevelColor = (score: number) => {if (score >= 70) return'text-error-dark';
 ...
 return (<div
 className={`border rounded-lg p-5 hover:shadow-md transition-all cursor-pointer ${isHighPriority ?'border-error bg-error-light' :'bg-card border-border'}`}
 ...
 {isHighPriority && (<ExclamationTriangleIcon className="h-5 w-5 text-error-dark"title="High Priority"/>)}
 </div>
 ...
 {/* Confidence */}
 {disputeCase.confidence !== undefined && (<div>
 ...
 {/* Recommendation */}
 {disputeCase.recommendation && (<div className="bg-card border border-accent rounded-lg p-3 mb-4">
 ...
 {/* SLA Deadline */}
 {slaDeadline && (<div className={`flex items-center space-x-2 text-sm mb-3 ${isOverdue ?'text-error-dark font-semibold' :'text-muted-foreground'}`}>
 ...
 {disputeCase.assignedTo && (<div>Assigned: {disputeCase.assignedTo}</div>)}
 {disputeCase.resolvedAt && (<div className="flex items-center space-x-1 text-success-dark">

...
interface AppointmentWizardProps {isOpen: boolean
 onClose: () => void
 ...
 zipCode?: string
 phone?: string}>
 familyMembers: Array<{userId: string
 name: string
 email: string}>
 onSubmit: (appointmentData: AppointmentData) => Promise<void>}

export interface AppointmentData {providerId: string
 dateTime: Date
...
export default function AppointmentWizard({isOpen,
 onClose,
 familyMember,
 providers,
 familyMembers,
 onSubmit}: AppointmentWizardProps) {const [currentStep, setCurrentStep] = useState<WizardStep>('intro')
 const [isSubmitting, setIsSubmitting] = useState(false)

 const [appointmentData, setAppointmentData] = useState<Partial<AppointmentData>>({type:'routine-checkup',
 requiresDriver: false})

 const stepOrder: WizardStep[] = ['intro','provider','datetime','type','location','transportation','notes','review']

 const currentStepIndex = stepOrder.indexOf(currentStep)
 const progress = ((currentStepIndex + 1) / stepOrder.length) * 100

 const handleNext = () => {const nextIndex = currentStepIndex + 1
 if (nextIndex < stepOrder.length) {setCurrentStep(stepOrder[nextIndex])}}

 const handleBack = () => {const prevIndex = currentStepIndex - 1
 ...
 if (!appointmentData.providerId || !appointmentData.dateTime || !appointmentData.location) {alert('Please fill in all required fields')
 return}

 await onSubmit(appointmentData as AppointmentData)
 setCurrentStep('confirmation')} catch (error) {logger.error('[AppointmentWizard] Submit failed', error)
 alert(`Failed to schedule appointment: ${error instanceof Error ? error.message :'Unknown error'}`)} finally {setIsSubmitting(false)}}
 ...
 onChange={(providerId) => {// Auto-populate location from provider
 const provider = providers.find(p => p.id === providerId)
 let location =''
 if (provider?.address) {location = provider.address
 if (provider.city) location +=`\n${provider.city}`if (provider.state) location +=`, ${provider.state}`if (provider.zipCode) location +=`${provider.zipCode}`if (provider.phone) location +=`\n\nPhone: ${provider.phone}`}
 ...
 {/* Progress Bar */}
 {currentStep !=='confirmation' && (<div className="bg-gray-200 h-2 flex-shrink-0">
 <div
 className="bg-gradient-to-r from-blue-600 to-indigo-600 h-2 transition-all duration-300"style={{width:`${progress}%`}}
 />
 </div>)}
 ...
 {/* Footer Navigation */}
 {currentStep !=='confirmation' && currentStep !=='intro' && (<div className="px-6 py-4 bg-gray-50 border-t border-border flex items-center justify-between flex-shrink-0">
 <button
 onClick={handleBack}
...
function IntroStep({familyMember,
 onNext}: {familyMember: {name: string}
 onNext: () => void}) {return (<div className="space-y-6">
...
function ProviderStep({providers,
 selectedProviderId,
 onChange}: {providers: Array<{id: string; name: string; specialty?: string}>
 selectedProviderId?: string
 onChange: (providerId: string) => void}) {// Deduplicate providers by ID (DRY - prevent showing duplicates)
 const uniqueProviders = providers.reduce((acc, provider) => {if (!acc.find(p => p.id === provider.id)) {acc.push(provider)}
 return acc}, [] as typeof providers)
 ...
 {sortedProviders.map((provider) => (<button
 key={provider.id}
 onClick={() => onChange(provider.id)}
 className={`w-full text-left p-4 rounded-lg border-2 transition-all ${selectedProviderId === provider.id
 ...
 {provider.specialty && (<div className="text-sm text-muted-foreground">{provider.specialty}</div>)}
 </div>
 {selectedProviderId === provider.id && (<CheckCircleIcon className="w-5 h-5 text-blue-600"/>)}
 </div>
 </button>))}
 </div>

 {sortedProviders.length === 0 && (<div className="text-center py-8">
...
function DateTimeStep({dateTime,
 onChange}: {dateTime?: Date
 onChange: (dateTime: Date) => void}) {const [date, setDate] = useState(dateTime ? dateTime.toISOString().split('T')[0] :'')
 const [time, setTime] = useState(dateTime
 ? dateTime.toTimeString().slice(0, 5)
 :'')

 const handleUpdate = () => {if (date && time) {const combined = new Date(`${date}T${time}`)
 onChange(combined)}}
 ...
 if (time) {const combined = new Date(`${e.target.value}T${time}`)
 onChange(combined)}}}
 ...
 if (date) {const combined = new Date(`${date}T${e.target.value}`)
 onChange(combined)}}}
 className="w-full px-4 py-3 border border-border rounded-lg bg-card text-foreground focus:ring-2 focus:ring-blue-500"/>
 </div>
 </div>
...
function TypeStep({type,
 reason,
 onChange}: {type?: string
 reason?: string
 onChange: (type: AppointmentData['type'], reason: string) => void}) {const [showCustomType, setShowCustomType] = useState(false)
 const [customTypeName, setCustomTypeName] = useState('')

 const types: Array<{value: AppointmentData['type']; label: string; description: string; placeholder: string}> = [{value:'routine-checkup',
 ...
 const handleCreateCustomType = () => {if (customTypeName.trim()) {// Store custom type name in the reason field with a marker
 ...
 {types.map((t) => (<button
 key={t.value}
 onClick={() => {onChange(t.value, reason ||'')
 setShowCustomType(false)}}
 className={`w-full text-left p-3 rounded-lg border-2 transition-all ${type === t.value
 ?'border-blue-600 bg-blue-50 /20'
 :'border-gray-200 hover:border-blue-300'}`}
 >
 ...
 {/* Custom Type Option */}
 {!showCustomType ? (<button
 onClick={() => setShowCustomType(true)}
 className="w-full text-left p-3 rounded-lg border-2 border-dashed border-gray-300 hover:border-blue-400 transition-all text-muted-foreground hover:text-blue-600">
 <div className="flex items-center gap-2">
 <span className="text-lg">➕</span>
 <span className="font-medium">Create Custom Type</span>
 </div>
 </button>) : (<div className="p-4 rounded-lg border-2 border-blue-600 bg-blue-50 /20">
 <label className="block text-sm font-medium text-foreground mb-2">
 Custom Appointment Type
 </label>
 <input
 type="text"value={customTypeName}
 onChange={(e) => setCustomTypeName(e.target.value)}
 placeholder="e.g., Physical Therapy, Dental Cleaning, Vision Screening..."className="w-full px-3 py-2 border border-border rounded-lg bg-card text-foreground focus:ring-2 focus:ring-blue-500 mb-3"autoFocus
 />
...
function LocationStep({location,
 onChange}: {location?: string
 onChange: (location: string) => void}) {const isAutoPopulated = location && location.includes('\n')
 ...
 {isAutoPopulated && (<div className="flex items-start gap-2 p-3 bg-green-50 /20 border border-green-200 rounded-lg">
 ...
 placeholder="e.g., 123 Medical Plaza, Suite 200, City, State ZIP"rows={5}
 className="w-full px-4 py-3 border border-border rounded-lg bg-card text-foreground focus:ring-2 focus:ring-blue-500"/>
 ...
 <LocationMap
 address={location}
 title="Appointment Location"showDirections={true}
 />
 </div>)}
 </div>)}

function TransportationStep({requiresDriver,
 assignedDriverId,
 pickupTime,
 familyMembers,
 onChange}: {requiresDriver: boolean
 assignedDriverId?: string
 pickupTime?: Date
 familyMembers: Array<{userId: string; name: string; email: string}>
 onChange: (requiresDriver: boolean, assignedDriverId?: string, pickupTime?: Date) => void}) {return (<div className="space-y-4">
 ...
 className={`flex-1 p-4 rounded-lg border-2 transition-all ${!requiresDriver
 ?'border-blue-600 bg-blue-50 /20'
 :'border-gray-200'}`}
 >
 ...
 {requiresDriver && (<div className="space-y-4 mt-4 p-4 bg-gray-50 rounded-lg">
 <div>
 <label className="block text-sm font-medium text-foreground mb-2">
 Assign Driver
 </label>
 ...
 {familyMembers.map((member) => (<option key={member.userId} value={member.userId}>
 ...
 <input
 type="time"value={pickupTime ? pickupTime.toTimeString().slice(0, 5) :''}
 onChange={(e) => {const time = e.target.value
...
function NotesStep({notes,
 onChange}: {notes?: string
 onChange: (notes: string) => void}) {return (<div className="space-y-4">
 ...
 value={notes ||''}
 onChange={(e) => onChange(e.target.value)}
 placeholder="e.g., Bring insurance card, fasting required, prepare list of medications..."rows={5}
 className="w-full px-4 py-3 border border-border rounded-lg bg-card text-foreground focus:ring-2 focus:ring-blue-500"/>
 </div>
 </div>)}

function ReviewStep({appointmentData,
 familyMember,
 providers,
 familyMembers}: {appointmentData: Partial<AppointmentData>
 familyMember: {name: string}
 providers: Array<{id: string; name: string; specialty?: string}>
 familyMembers: Array<{userId: string; name: string; email: string}>}) {const provider = providers.find((p) => p.id === appointmentData.providerId)
 const driver = familyMembers.find((m) => m.userId === appointmentData.assignedDriverId)

 const appointmentTypes = {'routine-checkup':'Routine Checkup','follow-up':'Follow-up Visit',
 ...
 ? appointmentData.dateTime.toLocaleString('en-US', {weekday:'long',
 year:'numeric',
 month:'long',
 day:'numeric',
 hour:'numeric',
 ...
 {appointmentData.requiresDriver && (<div>
 <div className="text-xs text-muted-foreground uppercase mb-1">Transportation</div>
 <div className="font-semibold text-foreground">
 Driver: {driver?.name ||'Not assigned'}
 </div>
 {appointmentData.pickupTime && (<div className="text-sm text-muted-foreground">
 Pickup: {appointmentData.pickupTime.toLocaleTimeString('en-US', {hour:'numeric',
 minute:'2-digit'})}
 </div>)}
 </div>)}

 {appointmentData.notes && (<div>
...
function ConfirmationStep({familyMember,
 onClose}: {familyMember: {name: string}
 onClose: () => void}) {return (<div className="text-center py-8 space-y-6">
 <div className="w-20 h-20 bg-green-100 /30 rounded-full flex items-center justify-center mx-auto">

...
interface VitalsSummaryModalProps {vitals: VitalSign[]
 patientName: string
 isOpen: boolean
 onClose: () => void
 ...
 */
export default function VitalsSummaryModal({vitals,
 patientName,
 isOpen,
 onClose,
 onViewDashboard}: VitalsSummaryModalProps) {if (!isOpen) return null
 ...
 <div className="rounded-full bg-green-100 /30 p-3">
 ...
 {vitals.map((vital, index) => (<VitalCard key={`${vital.type}-${index}`} vital={vital} />))}
 </div>

 {/* Notes */}
 {vitals[0]?.notes && (<div className="mt-6 p-4 bg-blue-50 /20 rounded-lg border border-blue-200">
 <h4 className="text-sm font-semibold text-blue-900 mb-2 flex items-center gap-2">
 <Activity className="h-4 w-4"/>
 Notes
 </h4>
 <p className="text-sm text-blue-800">{vitals[0].notes}</p>
 </div>)}
 </div>

 {/* Actions */}
 <div className="p-6 border-t border-gray-200 flex flex-col sm:flex-row gap-3 justify-end">
 {onViewDashboard && (<button
 onClick={onViewDashboard}
 className="px-4 py-2 text-sm font-medium text-blue-600 hover:bg-blue-50 /30 rounded-lg transition-colors flex items-center justify-center gap-2">
 ...
 */
function VitalCard({vital}: {vital: VitalSign}) {const label = getVitalTypeLabel(vital.type)
 const displayValue = formatVitalForDisplay(vital.type, vital.value, vital.unit)

 // Color scheme based on vital type
 const colorSchemes: Record<string, {bg: string; text: string; icon: string}> = {blood_pressure: {bg:'bg-red-100 /30',
 text:'text-red-900',
 icon:'text-red-600'},
 temperature: {bg:'bg-orange-100 /30',
 text:'text-orange-900',
 icon:'text-orange-600'},
 pulse_oximeter: {bg:'bg-purple-100 /30',
 text:'text-purple-900',
 icon:'text-purple-600'},
 blood_sugar: {bg:'bg-green-100 /30',
 text:'text-green-900',
 icon:'text-green-600'}}

 const colors = colorSchemes[vital.type] || {bg:'bg-blue-100 /30',
 text:'text-blue-900',
 icon:'text-blue-600'}

 return (<div
 className={`${colors.bg} rounded-lg p-4 border border-gray-200 transition-all hover:shadow-md`}
 >
 ...
 {vital.method && (<p className="text-xs text-gray-600 mt-2 capitalize">
 {vital.method} entry
 </p>)}
 </div>)}

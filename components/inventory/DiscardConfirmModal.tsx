...
interface DiscardConfirmModalProps {items: ShoppingItem[]
 isOpen: boolean
 onConfirm: (options: {addToShoppingList: boolean
 reason:'expired' |'spoiled' |'moldy' |'other'
 notes?: string}) => Promise<void>
 onCancel: () => void}

export function DiscardConfirmModal({items,
 isOpen,
 onConfirm,
 onCancel}: DiscardConfirmModalProps) {const [addToShoppingList, setAddToShoppingList] = useState(true)
 const [reason, setReason] = useState<'expired' |'spoiled' |'moldy' |'other'>('expired')
 const [notes, setNotes] = useState('')
 const [confirming, setConfirming] = useState(false)

 const handleConfirm = useCallback(async () => {setConfirming(true)
 try {await onConfirm({addToShoppingList,
 reason,
 notes: notes.trim() || undefined})
 // Reset form
 setAddToShoppingList(true)
 setReason('expired')
 setNotes('')} finally {setConfirming(false)}}, [addToShoppingList, reason, notes, onConfirm])
 ...
 <div
 className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto"onClick={e => e.stopPropagation()}
 >
 {/* Header */}
 <div className="p-6 border-b border-gray-200">
 <div className="flex items-start gap-3">
 <div className="flex-shrink-0 w-10 h-10 rounded-full bg-red-100 /20 flex items-center justify-center">
 <svg
 ...
 {isBatch
 ?`You are about to discard ${items.length} items`:`You are about to discard ${items[0]?.productName}`}
 </p>
 </div>
 </div>
 </div>
 ...
 {isBatch && (<div className="mb-4">
 <p className="text-sm font-medium text-gray-700 mb-2">
 Items to discard:
 </p>
 <div className="max-h-32 overflow-y-auto bg-gray-50 /50 rounded-lg p-3 space-y-1">
 {items.map(item => (<div
 key={item.id}
 className="text-sm text-gray-600 flex items-center gap-2">
 <span className="w-1.5 h-1.5 bg-gray-400 rounded-full"/>
 {item.productName}
 {item.quantity > 1 && (<span className="text-xs text-gray-500">({item.quantity}x)</span>)}
 </div>))}
 </div>
 </div>)}
 ...
 className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"/>
 </div>

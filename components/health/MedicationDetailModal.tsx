...
interface MedicationDetailModalProps {medication: PatientMedication | ScannedMedication
 onClose: () => void
 patientId?: string // Optional - needed for adherence history}

export default function MedicationDetailModal({medication, onClose, patientId}: MedicationDetailModalProps) {// Type guard to check if medication is ScannedMedication
 const isScannedMedication = (med: PatientMedication | ScannedMedication): med is ScannedMedication => {return'patientName' in med}

 // Get patient name if available
 const patientName = isScannedMedication(medication) ? medication.patientName : undefined

 // Check expiration status
 const getExpirationStatus = (expirationDate?: string) => {if (!expirationDate) return null

 try {const expDate = new Date(expirationDate)
 const today = new Date()
 const daysUntilExpiration = Math.floor((expDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))

 if (daysUntilExpiration < 0) {return {status:'expired', message:'EXPIRED', color:'text-error', bgColor:'bg-error-light /20'}} else if (daysUntilExpiration <= 30) {return {status:'expiring-soon', message:`Expires in ${daysUntilExpiration} days`, color:'text-amber-600', bgColor:'bg-amber-50 /20'}}
 ...
 <div
 className="bg-card rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"onClick={(e) => e.stopPropagation()}
 >
 {/* Header */}
 <div className="flex items-center justify-between p-6 border-b border-border sticky top-0 bg-card z-10">
 <div className="flex items-center gap-3">
 <div className="w-12 h-12 rounded-full bg-purple-100 /30 flex items-center justify-center">
 ...
 {medication.brandName && (<p className="text-sm text-muted-foreground italic">
 ...
 {/* Medication Image */}
 {(medication.imageUrl || medication.photoUrl) && (<div>
 ...
 className="w-full max-w-md mx-auto rounded-lg border-2 border-border hover:border-primary transition-all cursor-pointer"onClick={() => window.open(medication.imageUrl || medication.photoUrl,'_blank')}
 />
 ...
 {/* Warning Banners */}
 {expirationStatus && (expirationStatus.status ==='expired' || expirationStatus.status ==='expiring-soon') && (<div className={`${expirationStatus.bgColor} rounded-lg px-4 py-3 flex items-center gap-3`}>
 <ExclamationTriangleIcon className={`w-6 h-6 ${expirationStatus.color}`} />
 <div>
 <p className={`font-bold ${expirationStatus.color}`}>
 {expirationStatus.message}
 </p>
 {medication.pharmacyPhone && (<a
 ...
 <p className="text-base font-medium text-purple-900">{medication.prescribedFor}</p>
 </div>
 </div>)}
 ...
 {medication.rxNumber && (<div className="flex items-start gap-2">
 ...
 {medication.ndc && (<div className="flex items-start gap-2">
 ...
 {medication.prescribingDoctor && (<div className="flex items-start gap-2 md:col-span-2">
 ...
 {medication.quantity && (<div className="flex items-start gap-2">
 ...
 {medication.refills && (<div className="flex items-start gap-2">
 ...
 {medication.drugClass && (<div className="flex items-start gap-2 md:col-span-2">
 ...
 {/* Dates */}
 {(medication.fillDate || medication.expirationDate) && (<div>
 <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3 flex items-center gap-2">
 <CalendarIcon className="w-4 h-4"/>
 Important Dates
 </h3>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
 {medication.fillDate && (<div className="bg-muted rounded-lg p-3">
 <div className="text-xs text-muted-foreground mb-1">Fill Date</div>
 <div className="text-sm font-medium text-foreground">
 {new Date(medication.fillDate).toLocaleDateString()}
 </div>
 </div>)}
 {medication.expirationDate && (<div className={`rounded-lg p-3 ${expirationStatus ? expirationStatus.bgColor :'bg-muted'}`}>
 ...
 {(medication.pharmacyName || medication.pharmacyPhone) && (<div>
 <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-2">Pharmacy</h3>
 <div className="bg-muted rounded-lg p-4">
 {medication.pharmacyName && (<p className="text-base font-medium text-foreground mb-1">{medication.pharmacyName}</p>)}
 {medication.pharmacyPhone && (<a
 href={`tel:${medication.pharmacyPhone}`}
 ...
 {/* Warnings */}
 {medication.warnings && medication.warnings.length > 0 && (<div>
 <h3 className="text-sm font-semibold text-amber-600 uppercase tracking-wide mb-2 flex items-center gap-2">
 <ExclamationTriangleIcon className="w-4 h-4"/>
 Important Warnings
 </h3>
 <div className="bg-amber-50 /20 border border-amber-200 rounded-lg p-4">
 <ul className="space-y-2">
 {medication.warnings.map((warning, idx) => (<li key={idx} className="flex items-start gap-2 text-sm text-amber-900">
 ...
 <p className="text-base font-medium text-primary-dark">{patientName}</p>
 </div>
 </div>)}

 {/* Adherence History (if patientId provided and medication is PatientMedication) */}
 {patientId && !isScannedMedication(medication) && (<div>

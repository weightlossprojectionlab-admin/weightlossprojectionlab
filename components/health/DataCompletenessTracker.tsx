...
export interface DataCompletenessTrackerProps {patientId?: string | null
 className?: string}

interface CompletenessMetric {label: string
 current: number
 target: number
 percentage: number
...
interface WeekData {weightLogs: number
 mealLogs: number
 stepLogs: number
 bloodPressureReadings: number
 bloodGlucoseReadings: number}

export function DataCompletenessTracker({patientId, className =''}: DataCompletenessTrackerProps) {const {user} = useAuth()
 const {profile} = useUserProfile()
 const [loading, setLoading] = useState(true)
 const [weekData, setWeekData] = useState<WeekData>({weightLogs: 0,
 mealLogs: 0,
 stepLogs: 0,
 ...
 useEffect(() => {const loadPatientProfile = async () => {if (!patientId) {setPatientProfile(null)
 return}

 try {const patient = await medicalOperations.patients.getPatient(patientId)
 setPatientProfile(patient)} catch (error) {logger.error('[DataCompleteness] Error loading patient profile', error as Error)}}

 if (patientId) {loadPatientProfile()}}, [patientId])

 // Fetch data for the current week
 useEffect(() => {const fetchWeekData = async () => {if (!effectiveUserId) return

 setLoading(true)

 try {// Calculate start of current week (7 days ago)
 const now = new Date()
 ...
 const stepQuery = query(stepLogsRef,
 where('loggedAt','>=', Timestamp.fromDate(weekStart)))
 ...
 if (patientId) {try {const vitalsRef = collection(db,'users', effectiveUserId,'patients', patientId,'vitals')
 const bpQuery = query(vitalsRef,
 ...
 const bgQuery = query(vitalsRef,
 where('type','==','blood_sugar'),
 where('recordedAt','>=', weekStart.toISOString()))
 const bgSnapshot = await getDocs(bgQuery)
 bgCount = bgSnapshot.size} catch (error) {logger.debug('[DataCompleteness] Vitals not available for this user', {error: error instanceof Error ? error.message : String(error)})}}

 setWeekData({weightLogs: weightCount,
 mealLogs: mealCount,
 stepLogs: stepCount,
 bloodPressureReadings: bpCount,
 bloodGlucoseReadings: bgCount})} catch (error) {logger.error('[DataCompleteness] Error fetching week data', error as Error)} finally {setLoading(false)}}

 fetchWeekData()}, [effectiveUserId, patientId])
 ...
 const hasDiabetes = activeProfile?.profile?.healthConditions?.some((condition: string) => condition.toLowerCase().includes('diabetes')) || false

 // Check if step tracking is enabled (from localStorage)
 const [isStepTrackingEnabled, setIsStepTrackingEnabled] = useState(false)
 useEffect(() => {if (typeof window !=='undefined') {const enabled = localStorage.getItem('step-tracking-enabled')
 setIsStepTrackingEnabled(enabled ==='true')}}, [])

 // Calculate metrics
 const metrics: CompletenessMetric[] = [{label:'Weight logged',
 ...
 const calculateStreak = (): number => {// This is a simplified streak calculation
 ...
 if (loading) {return (<div className={`bg-card rounded-lg shadow-sm p-6 ${className}`}>
 ...
 return (<div className={`bg-card rounded-lg shadow-sm p-6 ${className}`}>
 <div className="flex items-center justify-between mb-4">
 <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
 <span>📊</span>
 <span>Your Data This Week</span>
 </h2>
 {streak > 0 && (<div className="flex items-center gap-2 bg-success-light /30 px-3 py-1 rounded-full">
 ...
 {enabledMetrics.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
 <p className="text-sm">Complete your health profile to see tracking recommendations</p>
 <Link
 href="/profile"className="mt-4 inline-block px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover transition-colors text-sm font-medium">
 Update Profile
 </Link>
 </div>) : (<div className="space-y-3">
 {enabledMetrics.map((metric, index) => (<div key={index} className="space-y-2">
 ...
 {metric.message && (<p className="text-xs text-muted-foreground">{metric.message}</p>)}
 </div>
 </div>
 {metric.status !=='complete' && metric.actionUrl && (<Link
 ...
 <div
 className={`absolute h-full rounded-full transition-all duration-500 ${metric.status ==='complete'
 ?'bg-success'
 ...
 {enabledMetrics.length > 0 && (<div className="mt-6 pt-4 border-t border-border">
 <div className="flex items-start gap-3 p-3 bg-secondary-light /20 rounded-lg">

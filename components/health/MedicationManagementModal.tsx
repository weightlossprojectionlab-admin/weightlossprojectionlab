...
interface MedicationManagementModalProps {isOpen: boolean
 onClose: () => void
...
export function MedicationManagementModal({isOpen,
 onClose,
 medications: initialMedications,
 onSave,
 prescribedFor}: MedicationManagementModalProps) {// Filter medications by condition if prescribedFor is specified
 ...
 useEffect(() => {if (isOpen) {const filtered = prescribedFor
 ? initialMedications.filter(med => med.prescribedFor === prescribedFor)
 : initialMedications
 setMedications(filtered)}}, [isOpen, prescribedFor, initialMedications])

 const handleSave = async () => {setSaving(true)
 try {// If filtering by condition, merge with medications from other conditions
 ...
 ?`Medications for ${prescribedFor} updated successfully`:'Medications updated successfully')
 onClose()} catch (error) {logger.error('Error saving medications', error as Error)
 toast.error('Failed to save medications')} finally {setSaving(false)}}

 const handleCancel = () => {setMedications(initialMedications)
 onClose()}
 ...
 {prescribedFor && (<p className="text-sm text-muted-foreground mt-1">
 ...
 ?`Scan or add medications for ${prescribedFor}. New medications will be automatically tagged for this condition.`:"Scan prescription labels with NDC barcode or OCR. Add, edit, or remove your medications."}
 />
 </div>

...
interface RecipeViewProps {patientId: string
 patientName: string}

/**
 * Get current meal type based on time of day
 */
function getCurrentMealType(): MealType {const hour = new Date().getHours()
 if (hour < 11) return'breakfast'
 if (hour < 15) return'lunch'
 if (hour < 21) return'dinner'
 return'snack'}

export function RecipeView({patientId, patientName}: RecipeViewProps) {const {recipes, loading: recipesLoading} = useRecipes()
 ...
 useEffect(() => {const fetchMemberData = async () => {setLoadingMemberData(true)
 try {const user = auth.currentUser
 if (!user) {logger.warn('[RecipeView] No authenticated user for member data')
 return}

 // Fetch patient profile
 const profile = await medicalOperations.patients.getPatient(patientId)
 if (!profile) {toast.error('Member profile not found')
 return}
 ...
 console.log('[RecipeView] Inventory loaded:', {householdId,
 ...
 if (recipes.length > 0) {const suggestions = await getMemberRecipeSuggestions({patient: profile,
 medications: meds || [],
 recentVitals: vitals || [],
 ...
 logger.info('[RecipeView] Generated member recipes', {memberId: patientId,
 count: suggestions.length,
 inventoryItems: household?.length || 0})}} catch (error) {logger.error('[RecipeView] Error fetching member data', error as Error, {memberId: patientId})
 toast.error('Failed to load personalized recipes')} finally {setLoadingMemberData(false)}}

 fetchMemberData()}, [patientId, recipes, selectedMealType])

 // Auto-set filter to"Can Make Now"when inventory loads
 useEffect(() => {if (!hasSetInitialFilter && inventoryItems.length > 0 && memberRecipes.length > 0) {// Check if we have any recipes with 100% availability
 const canMakeCount = memberRecipes.filter(r =>'inventoryAvailability' in r && r.inventoryAvailability?.percentage === 100).length

 // If we have recipes we can make, default to showing only those
 if (canMakeCount > 0) {setMinInventoryFilter(100)}
 setHasSetInitialFilter(true)}}, [inventoryItems, memberRecipes, hasSetInitialFilter])
 ...
 // Filter recipes
 const filteredRecipes = displayRecipes.filter(recipe => {if (selectedMealType !=='all' && recipe.mealType !== selectedMealType) {return false}

 if (selectedDietaryTags.length > 0) {const hasAllTags = selectedDietaryTags.every(tag =>
 ...
 if (hasInventory && memberRecipes.length > 0) {// We have inventory and member recipes - filter by availability
 if ('inventoryAvailability' in recipe && recipe.inventoryAvailability) {// Hide recipes with 0% availability (no ingredients on hand)
 if (recipe.inventoryAvailability.percentage === 0) {return false}
 // Apply additional filter if set
 if (minInventoryFilter > 0 && recipe.inventoryAvailability.percentage < minInventoryFilter) {return false}}} else if (hasInventory && memberRecipes.length === 0 && minInventoryFilter > 0) {// We have inventory but member recipes haven't loaded yet
 ...
 const sortedRecipes = [...filteredRecipes].sort((a, b) => {const aAvail ='inventoryAvailability' in a && a.inventoryAvailability ? a.inventoryAvailability.percentage : 0
 ...
 const toggleDietaryTag = (tag: DietaryTag) => {setSelectedDietaryTags(prev =>
 prev.includes(tag)
 ? prev.filter(t => t !== tag)
 : [...prev, tag])}

 // Check if we have expiring items
 const hasExpiringItems = inventoryItems.some(item => {if (!item.inStock || !item.expiresAt) return false
 const expiryDate = new Date(item.expiresAt)
 const sevenDaysFromNow = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
 return expiryDate <= sevenDaysFromNow})

 // Generate recipe from inventory using our inventory data as the"AI"const handleGenerateInventoryRecipe = () => {try {setGeneratingRecipe(true)

 const newRecipe = generateRecipeFromInventory(inventoryItems, {mealType: selectedMealType ==='all' ? getCurrentMealType() : selectedMealType,
 minIngredients: 3,
 maxIngredients: 6,
 preferExpiring: true})

 if (!newRecipe) {toast.error('No ingredients available to generate recipe')
 return}

 // Add to the recipe list temporarily
 setSelectedRecipe(newRecipe)
 toast.success(`Created: ${newRecipe.name}! 🎉`)
 logger.info('[RecipeView] Generated recipe from inventory', {recipeName: newRecipe.name,
 ingredientCount: newRecipe.ingredients.length})} catch (error) {logger.error('[RecipeView] Error generating recipe from inventory', error as Error)
 toast.error('Failed to generate recipe')} finally {setGeneratingRecipe(false)}}

 return (<div className="space-y-6">
 {/* Expiring Items Recipe Suggestions - DRY from RecipeSuggestions component */}
 {inventoryItems.length > 0 && hasExpiringItems && (<RecipeSuggestions
 items={inventoryItems}
 recipes={memberRecipes.length > 0 ? memberRecipes : recipes}
 />)}

 {/* Inventory Recipe Generator - No External AI! */}
 {memberProfile && inventoryItems.filter(i => i.inStock).length >= 3 && (<div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-6 px-6 rounded-lg shadow-lg">
 ...
 {memberProfile && (<div className="bg-gradient-to-r from-green-600 to-blue-600 text-white py-4 px-6 rounded-lg">
 ...
 <div className="mb-6">
 <input
 type="text"placeholder="Search recipes, ingredients..."value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 className="w-full px-4 py-3 border border-border bg-background text-foreground placeholder-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"/>
 </div>

 {/* Inventory Filter - Only show if we have inventory AND member recipes loaded */}
 {hasInventory && memberRecipes.length > 0 && (<div className="mb-6">
 ...
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${minInventoryFilter === 0
 ...
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${minInventoryFilter === 50
 ?'bg-primary text-white'
 :'bg-muted text-foreground hover:bg-gray-200'}`}
 >
 50%+ On Hand
 ...
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${minInventoryFilter === 80
 ?'bg-primary text-white'
 :'bg-muted text-foreground hover:bg-gray-200'}`}
 >
 80%+ On Hand
 ...
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${minInventoryFilter === 100
 ?'bg-primary text-white'
 :'bg-muted text-foreground hover:bg-gray-200'}`}
 >
 🔥 Can Make Now
 ...
 {!hasInventory && (<div className="mb-6 bg-blue-50 /20 border border-blue-300 rounded-lg py-3 px-4">
 <div className="flex items-center gap-3">
 ...
 {hasInventory && memberRecipes.length === 0 && !loadingMemberData && (<div className="mb-6 bg-yellow-50 /20 border border-yellow-300 rounded-lg py-3 px-4">
 <div className="flex items-center gap-3">
 <span className="text-2xl">🔄</span>
 <div>
 ...
 <div className="flex flex-wrap gap-2">
 {(['all','breakfast','lunch','dinner','snack'] as const).map(type => (<button
 key={type}
 onClick={() => setSelectedMealType(type)}
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${selectedMealType === type
 ?'bg-primary text-white'
 :'bg-muted text-foreground hover:bg-gray-200'}`}
 >
 ...
 <div className="flex flex-wrap gap-2">
 {(['vegan','vegetarian','keto','paleo','gluten-free','dairy-free','high-protein','low-carb'] as DietaryTag[]).map(tag => (<button
 key={tag}
 onClick={() => toggleDietaryTag(tag)}
 className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${selectedDietaryTags.includes(tag)
 ?'bg-primary text-white'
 :'bg-muted text-foreground hover:bg-gray-200'}`}
 >
 {tag}
 ...
 {/* Loading State */}
 {(recipesLoading || loadingMemberData) && (<div className="flex items-center justify-center py-12">
 <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
 <span className="ml-3 text-muted-foreground">Loading recipes...</span>
 </div>)}

 {/* Results Count */}
 {!recipesLoading && !loadingMemberData && (<div className="flex items-center justify-between">
 <p className="text-muted-foreground">
 {sortedRecipes.length} recipe{sortedRecipes.length !== 1 ?'s' :''} found
 {hasInventory && memberRecipes.length > 0 && sortedRecipes.length > 0 && (<span className="text-green-600 font-medium ml-2">
 (matched to your inventory)
 </span>)}
 {hasInventory && memberRecipes.length > 0 && minInventoryFilter > 0 &&`• ${minInventoryFilter}%+ ingredients on hand`}
 </p>
 {hasInventory && memberRecipes.length > 0 && (<div className="flex items-center gap-4 text-sm">
 ...
 {/* Recipe Grid */}
 {!recipesLoading && !loadingMemberData && sortedRecipes.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
 {sortedRecipes.map(recipe => (<div
 key={recipe.id}
 ...
 />
 {recipe.videoUrl && (<div className="absolute top-2 right-2 bg-black bg-opacity-75 text-white px-2 py-1 rounded-lg flex items-center gap-1 text-xs">
 ...
 {hasInventory && memberRecipes.length > 0 &&'inventoryAvailability' in recipe && recipe.inventoryAvailability && (<div className="mb-3">
 {recipe.inventoryAvailability.percentage === 100 ? (<div className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-3 py-2 rounded-lg font-bold text-sm shadow-md">
 <span className="text-lg">✓</span>
 <span>{requiresCooking(recipe) ?'READY TO COOK NOW' :'READY TO MAKE NOW'}</span>
 </div>) : recipe.inventoryAvailability.percentage >= 80 ? (<div className="flex items-center justify-between bg-yellow-50 /30 border-2 border-yellow-400 text-yellow-900 px-3 py-2 rounded-lg font-semibold text-xs">
 <span>Missing {recipe.inventoryAvailability.missingCount || 0} ingredient{(recipe.inventoryAvailability.missingCount || 0) !== 1 ?'s' :''}</span>
 <span className="text-yellow-600">{recipe.inventoryAvailability.percentage}%</span>
 </div>) : recipe.inventoryAvailability.percentage >= 50 ? (<div className="flex items-center justify-between bg-orange-50 /30 border border-orange-300 text-orange-900 px-3 py-1.5 rounded-lg font-medium text-xs">
 <span>Need {recipe.inventoryAvailability.missingCount || 0} more items</span>
 <span className="text-orange-600">{recipe.inventoryAvailability.percentage}%</span>
 </div>) : (<div className="flex items-center justify-between bg-gray-50 /30 border border-gray-300 text-gray-700 px-3 py-1.5 rounded-lg text-xs">
 ...
 {/* Dietary Tags */}
 {recipe.dietaryTags.length > 0 && (<div className="flex flex-wrap gap-1 mb-4">
 {recipe.dietaryTags.slice(0, 3).map(tag => (<span
 key={tag}
 className="text-xs bg-primary-light /20 text-primary px-2 py-1 rounded">
 {tag}
 </span>))}
 {recipe.dietaryTags.length > 3 && (<span className="text-xs text-muted-foreground px-2 py-1">
 +{recipe.dietaryTags.length - 3} more
 </span>)}
 </div>)}

 {/* No Inventory Alert - Show on every card when inventory is empty */}
 {!hasInventory && (<div className="mb-4 bg-gradient-to-r from-orange-100 to-yellow-100 /30 /30 border-2 border-orange-400 rounded-lg p-3 shadow-sm">
 ...
 <Link
 href="/shopping"onClick={(e) => e.stopPropagation()}
 ...
 {'safetyResult' in recipe && recipe.safetyResult && (<div className="space-y-2 mb-4">
 {/* Medical Safety Badges */}
 {recipe.safetyResult.badges && recipe.safetyResult.badges.length > 0 && (<div className="flex flex-wrap gap-1">
 {recipe.safetyResult.badges.map((badge: any, idx: number) => (<span
 key={idx}
 className={`text-xs px-2 py-1 rounded-full font-medium ${badge.type ==='safe'
 ...
 {hasInventory && memberRecipes.length > 0 &&'inventoryAvailability' in recipe && recipe.inventoryAvailability && (<div className="flex items-center gap-2 text-xs">
 <div className="flex-1 bg-gray-200 rounded-full h-1.5">
 <div
 className={`h-1.5 rounded-full ${recipe.inventoryAvailability.percentage >= 80
 ?'bg-green-500'
 ...
 {'inventoryAvailability' in recipe && recipe.inventoryAvailability?.expiringIngredients && recipe.inventoryAvailability.expiringIngredients.length > 0 && (<div className="bg-orange-50 /20 border border-orange-200 rounded px-2 py-1">
 ...
 {/* Safety Warnings */}
 {recipe.safetyResult.warnings && recipe.safetyResult.warnings.length > 0 && (<div className="bg-yellow-50 /20 border border-yellow-200 rounded px-2 py-1.5">
 {recipe.safetyResult.warnings.slice(0, 2).map((warning: string, idx: number) => (<p key={idx} className="text-xs text-yellow-800">
 ⚠️ {warning}
 </p>))}
 </div>)}
 </div>)}
 ...
 </div>) : !recipesLoading && !loadingMemberData ? (<div className="bg-card rounded-lg shadow-sm border border-border p-12 text-center">

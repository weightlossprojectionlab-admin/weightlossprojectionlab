...
interface MedicationListProps {patientId: string
 patientOwnerId?: string
 medications: PatientMedication[]
 loading: boolean
 onMedicationUpdated: () => void}

export function MedicationList({patientId, patientOwnerId, medications, loading, onMedicationUpdated}: MedicationListProps) {const [deletingMedicationId, setDeletingMedicationId] = useState<string | null>(null)
 ...
 const handleDeleteClick = (medication: PatientMedication) => {setMedicationToDelete(medication)}

 const handleConfirmDelete = async () => {if (!medicationToDelete) return

 try {setDeletingMedicationId(medicationToDelete.id)
 await medicalOperations.medications.deleteMedication(patientId, medicationToDelete.id)
 toast.success('Medication deleted')
 onMedicationUpdated() // Refresh the parent's medication list} catch (error: any) {logger.error('[MedicationList] Error deleting medication', error)
 toast.error('Failed to delete medication')} finally {setDeletingMedicationId(null)
 setMedicationToDelete(null)}}

 const handleMarkAsTaken = async (medication: PatientMedication) => {try {setLoggingDoseId(medication.id)
 await medicalOperations.medications.logDose(patientId, medication.id, {takenAt: new Date().toISOString()})
 toast.success(`${medication.name} marked as taken`)
 onMedicationUpdated() // Refresh the parent's medication list} catch (error: any) {logger.error('[MedicationList] Error logging dose', error)
 toast.error('Failed to log medication dose')} finally {setLoggingDoseId(null)}}

 const handleEditClick = (medication: PatientMedication, e: React.MouseEvent) => {e.stopPropagation()
 setEditingMedicationId(medication.id)
 setEditForm({name: medication.name,
 ...
 const handleCancelEdit = () => {setEditingMedicationId(null)
 setEditForm({})}

 const handleSaveEdit = async (medicationId: string) => {try {await medicalOperations.medications.updateMedication(patientId, medicationId, editForm)
 toast.success('Medication updated successfully')
 setEditingMedicationId(null)
 setEditForm({})
 onMedicationUpdated() // Refresh the parent's medication list} catch (error: any) {logger.error('[MedicationList] Error updating medication', error)
 toast.error('Failed to update medication')}}

 // Calculate if refill is needed
 const getRefillStatus = (medication: PatientMedication) => {if (!medication.quantityRemaining || !medication.quantity) return null

 const remaining = medication.quantityRemaining
 const total = parseInt(medication.quantity) || 0

 if (remaining <= 0) {return {status:'empty', message:'Out of medication', color:'text-error'}} else if (remaining <= 7) {return {status:'low', message:`Only ${remaining} doses left`, color:'text-amber-600'}} else if (remaining <= 14) {return {status:'moderate', message:`${remaining} doses remaining`, color:'text-blue-600'}}
 return null}
 ...
 if (medications.length === 0) {return (<div className="text-center py-8 text-muted-foreground">
 <p>No medications added yet</p>
 <p className="text-sm mt-2">Add medications using the form above</p>
 </div>)}

 return (<div className="space-y-3">
 {medications.map((med) => {const refillStatus = getRefillStatus(med)
 const isEditing = editingMedicationId === med.id

 return (<div
 key={med.id}
 className="bg-background rounded-lg p-4 border-2 border-border hover:border-primary/30 transition-colors"onClick={() => !isEditing && setSelectedMedication(med)}
 >
 {isEditing ? (// Edit Mode
 ...
 <input
 type="text"value={editForm.strength ||''}
 ...
 <input
 type="text"value={editForm.dosageForm ||''}
 ...
 <input
 type="text"value={editForm.frequency ||''}
 ...
 <input
 type="text"value={editForm.prescribedFor ||''}
 ...
 <input
 type="text"value={editForm.prescribingDoctor ||''}
 ...
 <input
 type="text"value={editForm.rxNumber ||''}
 ...
 <input
 type="text"value={editForm.quantity ||''}
 ...
 <input
 type="text"value={editForm.refills ||''}
 ...
 className="w-full mt-1 px-3 py-2 border border-border rounded bg-background text-foreground"rows={2}
 />
 </div>
 </div>
 </div>) : (// View Mode
 ...
 {med.name}
 {med.brandName && (<span className="text-sm font-normal text-muted-foreground ml-2">
 ({med.brandName})
 </span>)}
 </h3>
 ...
 {refillStatus && (<p className={`text-xs font-medium mt-1 ${refillStatus.color}`}>
 ...
 {med.frequency && (<div className="mt-2 p-2 bg-primary-light/50 rounded text-sm">
 <span className="font-medium">Dosage:</span> {med.frequency}
 </div>)}

 {med.prescribedFor && (<p className="text-sm text-muted-foreground mt-2">
 <span className="font-medium">For:</span> {med.prescribedFor}
 </p>)}

 {med.rxNumber && (<p className="text-xs text-muted-foreground mt-2">
 Rx #{med.rxNumber}
 </p>)}

 {med.warnings && med.warnings.length > 0 && (<div className="mt-2 p-2 bg-warning-light/20 rounded text-xs text-warning-dark">
 <span className="font-medium">⚠️ Warnings:</span>
 <ul className="list-disc list-inside mt-1">
 {med.warnings.map((warning, idx) => (<li key={idx}>{warning}</li>))}
 </ul>
 </div>)}

 {med.notes && (<p className="text-xs text-muted-foreground mt-2 italic">
 {med.notes}
 </p>)}
 ...
 <div className="flex-1">
 {med.lastTaken && (<p className="text-xs text-muted-foreground">
 Last taken: {new Date(med.lastTaken).toLocaleString()}
 </p>)}
 {med.adherenceRate !== undefined && (<p className="text-xs text-muted-foreground mt-1">
 ...
 title="Delete Medication Record"message={`Are you sure you want to delete ${medicationToDelete?.name}?\n\n⚠️ This is a MEDICAL RECORD and this action cannot be undone.\n\nThe deletion will be logged for accountability and compliance purposes.`}
 confirmText="Delete Medication"cancelText="Cancel"variant="danger"iconSize="large"customIcon={<div className="relative">

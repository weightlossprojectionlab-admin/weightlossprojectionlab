...
interface VitalsQuickViewModalProps {isOpen: boolean
 onClose: () => void
 vitals: VitalSign[]
 patientName: string
 onOpenWizard: () => void
...
export default function VitalsQuickViewModal({isOpen,
 onClose,
 vitals,
 patientName,
 onOpenWizard,
 loading = false}: VitalsQuickViewModalProps) {// Filter vitals from today
 ...
 const handleLogVitals = () => {onClose() // Close this modal
 ...
 ?`${todayVitals.length} reading${todayVitals.length !== 1 ?'s' :''} today`:'No vitals recorded today'}
 </p>
 </div>
 <button
 onClick={onClose}
 ...
 </div>) : todayVitals.length > 0 ? (<DailyVitalsSummary vitals={vitals} patientName={patientName} />) : (<div className="text-center py-12">
 ...
 {!loading && todayVitals.length > 0 && (<div className="px-6 py-4 bg-gray-50 border-t border-border flex items-center justify-between flex-shrink-0">

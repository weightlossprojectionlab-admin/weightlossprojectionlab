...
export default function CoachingStatus({ status, isEligible, hasActiveCoach }: CoachingStatusProps) {
  ...
  return (
    <div className={`border rounded-lg p-6 ${
      hasActiveCoach ? 'bg-success-light border-success' : 'bg-accent-light border-accent'
    }`}>
        ...
        <div className="mt-4 pt-4 border-t border-border">
          ...
          <div className="flex flex-wrap gap-2">
            {status.readinessSignals.map((signal, index) => (
              <span
                key={index}
                className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-accent-light text-accent-dark"
              >
                {signal}
              </span>
            ))}

...
interface VitalsFormData {heightFeet: string
 heightInches: string
 heightCm: string
...
interface VitalsFormSectionProps {data: VitalsFormData
 onChange: (updates: Partial<VitalsFormData>) => void
 required?: boolean
 showGoals?: boolean}

export function VitalsFormSection({data,
 onChange,
 required = false,
 showGoals = false}: VitalsFormSectionProps) {// Calculate BMI and healthy weight range
 function calculateBMI() {if (!data.currentWeight) return null
 ...
 function getTargetWeightSuggestion() {if (!bmiData) return null

 const currentWeight = parseFloat(data.currentWeight)
 const {bmi, minWeight, maxWeight} = bmiData

 if (bmi < 18.5) {// Underweight - suggest gaining to minimum healthy weight
 return {target: Math.round(minWeight * 10) / 10,
 reason:`Your BMI (${bmi.toFixed(1)}) indicates underweight. Aim for ${Math.round(minWeight)}-${Math.round(maxWeight)} ${data.weightUnit} for optimal health.`}} else if (bmi >= 25 && bmi < 30) {// Overweight - suggest midpoint of healthy range
 const target = (minWeight + maxWeight) / 2
 return {target: Math.round(target * 10) / 10,
 reason:`Your BMI (${bmi.toFixed(1)}) indicates overweight. Aim for ${Math.round(minWeight)}-${Math.round(maxWeight)} ${data.weightUnit} for optimal health.`}} else if (bmi >= 30) {// Obese - suggest 10% weight loss initially
 const target = currentWeight * 0.9
 return {target: Math.round(target * 10) / 10,
 reason:`Your BMI (${bmi.toFixed(1)}) indicates obesity. Start with a 10% weight loss goal to ${Math.round(target)} ${data.weightUnit}, then reassess.`}} else {// Healthy weight - suggest maintaining
 ...
 <button
 type="button"onClick={() => onChange({heightUnit:'imperial', weightUnit:'lbs'})}
 className={`flex-1 px-4 py-2 rounded-lg border-2 font-medium transition-colors ${data.heightUnit ==='imperial'
 ...
 <button
 type="button"onClick={() => onChange({heightUnit:'metric', weightUnit:'kg'})}
 className={`flex-1 px-4 py-2 rounded-lg border-2 font-medium transition-colors ${data.heightUnit ==='metric'
 ?'border-primary bg-primary-light text-primary-dark'
 :'border-border hover:border-border'}`}
 >
 Metric (cm/kg)
 ...
 {data.heightUnit ==='imperial' ? (<div className="grid grid-cols-2 gap-3">
 <div>
 <label className="block text-xs text-muted-foreground mb-1">Feet</label>
 <input
 type="number"min="0"max="8"value={data.heightFeet}
 ...
 placeholder="8"className="w-full px-4 py-2 border-2 border-border rounded-lg bg-background text-foreground focus:border-primary focus:ring-2 focus:ring-purple-600/20"/>
 </div>
 </div>) : (<div>
 <input
 type="number"min="50"max="250"step="0.1"value={data.heightCm}
 onChange={(e) => onChange({heightCm: e.target.value})}
 placeholder="170"required={required}
 className="w-full px-4 py-2 border-2 border-border rounded-lg bg-background text-foreground focus:border-primary focus:ring-2 focus:ring-purple-600/20"/>
 ...
 <div className="flex gap-3">
 <input
 type="number"min="0"step="0.1"value={data.currentWeight}
 ...
 {/* AI Health Analysis */}
 {suggestion && (<div className="mt-3 p-4 rounded-lg bg-primary/10 border-2 border-primary/30">
 ...
 {suggestion.target !== parseFloat(data.currentWeight) && (<button
 type="button"onClick={() => onChange({targetWeight: suggestion.target.toString(),
 ...
 {/* Target Weight */}
 {data.weightGoal && data.weightGoal !=='maintain-weight' && (<div>
 <label className="block text-sm font-medium text-foreground mb-2">
 Target Weight
 </label>
 <div className="flex gap-3">
 <input
 type="number"min="0"step="0.1"value={data.targetWeight}

...
interface FamilyMembersTabProps {members: FamilyMember[]
...
export function FamilyMembersTab({members,
 patients,
 currentUserRole,
 onEditPermissions,
 onRemoveMember,
 onUpdateRole,
 loading = false}: FamilyMembersTabProps) {const [updatingRoleFor, setUpdatingRoleFor] = useState<string | null>(null)
 ...
 const getAccessiblePatients = (member: FamilyMember): string[] => {return patients
 .filter(p => member.patientsAccess.includes(p.id))
 .map(p => p.name)}

 const handleRoleChange = async (memberId: string, newRole: FamilyRole) => {if (!onUpdateRole) return

 setUpdatingRoleFor(memberId)
 try {await onUpdateRole(memberId, newRole)} finally {setUpdatingRoleFor(null)}}
 ...
 if (activeMembers.length === 0) {return (<div className="text-center py-12 bg-card rounded-lg border-2 border-border">
 ...
 {activeMembers.map(member => {const accessiblePatients = getAccessiblePatients(member)
 const canEdit = canEditMember(currentUserRole, member.familyRole ||'caregiver')
 const isUpdatingRole = updatingRoleFor === member.id

 return (<div key={member.id} className="space-y-4">
 {/* Enhanced Member Card */}
 <div
 className={`bg-card rounded-lg border-2 p-6 transition-colors ${member.familyRole ==='account_owner'
 ?'border-amber-400'
 ...
 src={member.photo}
 alt={member.name}
 className="w-12 h-12 rounded-full object-cover"/>) : (<div className="w-12 h-12 rounded-full bg-primary-light /20 flex items-center justify-center">
 ...
 <span
 className={`px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-success-dark /20`}
 >
 Active
 </span>
 </div>
 ...
 {canEdit && onUpdateRole ? (<div className="mb-4">
 ...
 {isUpdatingRole && (<p className="text-xs text-muted-foreground mt-1">Updating role...</p>)}
 ...
 {/* Relationship */}
 {member.relationship && (<div className="mb-3">
 ...
 {accessiblePatients.length > 0 ? (<div className="flex flex-wrap gap-2">
 {accessiblePatients.slice(0, 3).map(patientName => (<span
 key={patientName}
 className="px-2 py-1 bg-secondary-light text-blue-700 /20 rounded text-xs">
 {patientName}
 </span>))}
 {accessiblePatients.length > 3 && (<span className="px-2 py-1 bg-muted text-muted-foreground rounded text-xs">
 +{accessiblePatients.length - 3} more
 </span>)}
 </div>) : (<p className="text-xs text-muted-foreground italic">No patients assigned</p>)}
 </div>

 {/* Last Active */}
 {member.lastActive && (<p className="text-xs text-muted-foreground mb-4">
 Last active: {new Date(member.lastActive).toLocaleDateString()}
 </p>)}

 {/* Actions */}
 {canEdit && (<div className="flex items-center gap-2 pt-4 border-t border-border">
 <button
 ...
 {member.familyRole !=='account_owner' && (<button

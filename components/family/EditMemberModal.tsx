...
interface EditMemberModalProps {isOpen: boolean
 onClose: () => void
...
export interface MemberUpdateData {memberId: string
 role?: FamilyRole
 patientsAccess: string[]
 patientPermissions: {[patientId: string]: FamilyMemberPermissions}}

const DEFAULT_PERMISSIONS: FamilyMemberPermissions = {viewPatientProfile: false,
...
export function EditMemberModal({isOpen,
 onClose,
 member,
 currentUserRole,
 patients,
 onSave}: EditMemberModalProps) {const [selectedRole, setSelectedRole] = useState<FamilyRole>('caregiver')
 const [selectedPatients, setSelectedPatients] = useState<Set<string>>(new Set())
 const [patientPermissions, setPatientPermissions] = useState<{[patientId: string]: FamilyMemberPermissions}>({})
 const [expandedPatient, setExpandedPatient] = useState<string | null>(null)
 const [saving, setSaving] = useState(false)

 // Initialize state when member changes
 useEffect(() => {if (member) {setSelectedRole(member.familyRole ||'caregiver')
 ...
 member.patientsAccess.forEach(patientId => {permissions[patientId] = {...member.permissions}})
 setPatientPermissions(permissions)}}, [member])

 if (!isOpen || !member) return null

 const handleRoleChange = (newRole: FamilyRole) => {setSelectedRole(newRole)}

 const handlePatientToggle = (patientId: string) => {const newSelected = new Set(selectedPatients)
 ...
 setPatientPermissions(newPermissions)} else {newSelected.add(patientId)
 // Add default permissions for this patient
 setPatientPermissions({...patientPermissions,
 [patientId]: {...DEFAULT_PERMISSIONS}})}
 setSelectedPatients(newSelected)}

 const handlePermissionToggle = (patientId: string, permission: keyof FamilyMemberPermissions) => {setPatientPermissions({...patientPermissions,
 [patientId]: {...patientPermissions[patientId],
 [permission]: !patientPermissions[patientId]?.[permission]}})}

 const handleSelectAllPermissions = (patientId: string) => {const allSelected = Object.values(patientPermissions[patientId] || {}).every(v => v)
 const newPermissions = {...DEFAULT_PERMISSIONS}
 Object.keys(newPermissions).forEach(key => {newPermissions[key as keyof FamilyMemberPermissions] = !allSelected})
 setPatientPermissions({...patientPermissions,
 [patientId]: newPermissions})}

 const handleSave = async () => {setSaving(true)
 try {await onSave({memberId: member.id,
 role: selectedRole,
 patientsAccess: Array.from(selectedPatients),
 patientPermissions})
 onClose()} catch (error) {console.error('Failed to save member updates:', error)} finally {setSaving(false)}}

 const permissionLabels: {[K in keyof FamilyMemberPermissions]: string} = {viewPatientProfile:'View Patient Profile',
 ...
 {/* Backdrop */}
 <div
 className="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75"onClick={onClose}
 />
 ...
 {patients.length === 0 ? (<div className="text-center py-8 bg-muted/50 rounded-lg">
 <p className="text-sm text-muted-foreground">No patients available</p>
 </div>) : (<div className="space-y-3">
 {patients.map(patient => {const hasAccess = selectedPatients.has(patient.id)
 ...
 <input
 type="checkbox"checked={hasAccess}
 ...
 {hasAccess && (<div className="flex items-center gap-2">
 ...
 {hasAccess && isExpanded && (<div className="border-t-2 border-border bg-muted/30 p-4">
 ...
 {(Object.keys(permissions) as Array<keyof FamilyMemberPermissions>).map(permission => (<label
 key={permission}
 className="flex items-center gap-2 p-2 hover:bg-muted rounded cursor-pointer">
 <input
 type="checkbox"checked={permissions[permission]}

...
interface ReviewStepProps {data: Partial<CaregiverProfile>
 onEdit: (step: number) => void}

export function ReviewStep({data, onEdit}: ReviewStepProps) {const {displayName,
 phoneNumber,
 photoUrl,
 isProfessional,
 ...
 {photoUrl ? (<div className="w-20 h-20 rounded-full overflow-hidden border-2 border-primary">
 <img src={photoUrl} alt="Profile"className="w-full h-full object-cover"/>
 </div>) : (<div className="w-20 h-20 rounded-full bg-muted flex items-center justify-center">
 ...
 {relationshipToPatients && Object.keys(relationshipToPatients).length > 0 && (<div>
 <div className="text-sm text-muted-foreground mb-2">Patient Relationships:</div>
 <div className="space-y-2">
 {Object.entries(relationshipToPatients).map(([patientId, relationship]) => (<div key={patientId} className="flex items-center gap-2 text-sm">
 ...
 {isProfessional && professionalInfo && (<div className="p-6 rounded-xl border-2 border-border bg-card space-y-4">
 <div className="flex items-center justify-between">
 <h3 className="text-lg font-semibold text-foreground">Professional Information</h3>
 <button
 type="button"onClick={() => onEdit(3)}
 className="text-sm text-primary hover:underline">
 Edit
 </button>
 </div>
 ...
 <div className="space-y-3">
 {professionalInfo.title && (<div>
 <span className="text-sm text-muted-foreground">Title:</span>
 <div className="font-medium text-foreground">{professionalInfo.title}</div>
 </div>)}

 {professionalInfo.credentials.length > 0 && (<div>
 <span className="text-sm text-muted-foreground">Credentials:</span>
 <div className="flex flex-wrap gap-2 mt-1">
 {professionalInfo.credentials.map((cred) => (<span key={cred} className="px-2 py-1 rounded-full bg-primary/10 text-primary text-sm">
 {cred}
 </span>))}
 </div>
 </div>)}

 {professionalInfo.certifications.length > 0 && (<div>
 <span className="text-sm text-muted-foreground">Certifications:</span>
 <div className="flex flex-wrap gap-2 mt-1">
 {professionalInfo.certifications.map((cert) => (<span key={cert} className="px-2 py-1 rounded-full bg-muted text-foreground text-sm">
 {cert}
 </span>))}
 </div>
 </div>)}

 {professionalInfo.yearsOfExperience && (<div>
 ...
 {availability && (<div className="p-6 rounded-xl border-2 border-border bg-card space-y-4">
 <div className="flex items-center justify-between">
 <h3 className="text-lg font-semibold text-foreground">Availability</h3>
 <button
 type="button"onClick={() => onEdit(4)}
 className="text-sm text-primary hover:underline">
 Edit
 </button>
 </div>
 ...
 .map(([day, dayData]) => (<span key={day} className="px-3 py-1 rounded-full bg-primary/10 text-primary text-sm">
 ...
 <span className={`px-2 py-1 rounded-full text-sm ${availability.isAvailableForEmergencies
 ?'bg-green-100 text-green-700 /20'
 ...
 {/* Preferences Section */}
 {preferences && (<div className="p-6 rounded-xl border-2 border-border bg-card space-y-4">
 <div className="flex items-center justify-between">
 <h3 className="text-lg font-semibold text-foreground">Communication Preferences</h3>
 <button
 type="button"onClick={() => onEdit(5)}
 className="text-sm text-primary hover:underline">
 Edit
 </button>
 </div>
 ...
 <div className="flex flex-wrap gap-2 mt-1">
 {preferences.languagesSpoken.map((lang) => (<span key={lang} className="px-2 py-1 rounded-full bg-muted text-foreground text-sm">
 {lang}
 </span>))}
 </div>
 </div>

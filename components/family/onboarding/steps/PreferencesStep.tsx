...
interface PreferencesStepProps {data: CaregiverPreferences | undefined
 onChange: (data: CaregiverPreferences) => void}

const CONTACT_METHODS: {value: ContactMethod; label: string; icon: string; description: string}[] = [{value:'app',
...
export function PreferencesStep({data, onChange}: PreferencesStepProps) {const preferencesData: CaregiverPreferences = data || {languagesSpoken: ['English'],
 ...
 const handleChange = (updates: Partial<CaregiverPreferences>) => {onChange({...preferencesData, ...updates})}

 const handleToggleLanguage = (language: string) => {const updated = preferencesData.languagesSpoken.includes(language)
 ? preferencesData.languagesSpoken.filter(l => l !== language)
 : [...preferencesData.languagesSpoken, language]

 // Ensure at least one language is selected
 if (updated.length > 0) {handleChange({languagesSpoken: updated})}}

 const handleToggleAlternateMethod = (method: ContactMethod) => {const current = preferencesData.alternateContactMethods || []
 ...
 {COMMON_LANGUAGES.map((language) => {const isSelected = preferencesData.languagesSpoken.includes(language)
 return (<button
 key={language}
 type="button"onClick={() => handleToggleLanguage(language)}
 ...
 {CONTACT_METHODS.map((method) => {const isSelected = preferencesData.preferredContactMethod === method.value
 return (<button
 key={method.value}
 type="button"onClick={() => handleChange({preferredContactMethod: method.value})}
 ...
 {CONTACT_METHODS.filter(m => m.value !== preferencesData.preferredContactMethod).map((method) => {const isSelected = preferencesData.alternateContactMethods?.includes(method.value)
 return (<button
 key={method.value}
 type="button"onClick={() => handleToggleAlternateMethod(method.value)}
 className={`px-4 py-3 rounded-xl text-left font-medium transition-all
 ${isSelected
 ...
 <input
 type="checkbox"checked={preferencesData.notifications?.email ?? true}
 onChange={(e) =>
 handleChange({notifications: {...preferencesData.notifications!,
 ...
 <label className="flex items-center gap-3 cursor-pointer p-4 rounded-lg border-2 border-border bg-card">
 <input
 type="checkbox"checked={preferencesData.notifications?.sms ?? false}
 onChange={(e) =>
 handleChange({notifications: {...preferencesData.notifications!,
 ...
 <label className="flex items-center gap-3 cursor-pointer p-4 rounded-lg border-2 border-border bg-card">
 <input
 type="checkbox"checked={preferencesData.notifications?.push ?? true}
 onChange={(e) =>
 handleChange({notifications: {...preferencesData.notifications!,
 ...
 <label className="flex items-center gap-3 cursor-pointer p-4 rounded-lg border-2 border-amber-500 bg-amber-50 /10">
 <input
 type="checkbox"checked={preferencesData.notifications?.emergencyOnly ?? false}
 onChange={(e) =>
 handleChange({notifications: {...preferencesData.notifications!,
 ...
 <input
 type="time"value={preferencesData.quietHours?.startTime ||'22:00'}
 onChange={(e) =>
 handleChange({quietHours: {...preferencesData.quietHours!,
 ...
 <input
 type="time"value={preferencesData.quietHours?.endTime ||'08:00'}
 onChange={(e) =>
 handleChange({quietHours: {...preferencesData.quietHours!,
 ...
 placeholder="e.g.,'I prefer detailed updates,''Keep it brief,''I work night shifts, so mornings are better'"rows={4}
 className="w-full px-4 py-3 rounded-xl border-2 border-border bg-background focus:border-primary focus:outline-none transition-colors resize-none"maxLength={500}
 />

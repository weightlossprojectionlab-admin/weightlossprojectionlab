...
export function FamilyMemberCard({member,
 onEdit,
 onRemove,
 showActions = true}: FamilyMemberCardProps) {const getStatusColor = (status: string) => {switch (status) {case'accepted':
 ...
 const getPermissionLevel = (permissions: typeof member.permissions): string => {const grantedCount = Object.values(permissions).filter(Boolean).length
 ...
 const getGrantedPermissions = () => {return Object.entries(member.permissions)
 ...
 return (<div className={`bg-card rounded-lg border-2 p-6 transition-colors ${borderClass}`}>
 ...
 {!isAccountOwner && member.managedBy && (<div className="mb-3">
 ...
 {grantedPermissions.slice(0, 3).map(permission => (<span
 key={permission}
 className="px-2 py-1 bg-primary-light text-primary-dark rounded text-xs">
 {permission}
 </span>))}
 {grantedPermissions.length > 3 && (<span className="px-2 py-1 bg-muted text-muted-foreground rounded text-xs">

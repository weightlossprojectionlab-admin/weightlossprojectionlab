...
interface UpgradeModalProps {isOpen: boolean
 onClose: () => void
 currentPlan?: SubscriptionPlan
 currentBillingInterval?: BillingInterval
 suggestedPlan?: SubscriptionPlan}

export function UpgradeModal({isOpen,
 onClose,
 currentPlan ='free',
 currentBillingInterval ='monthly',
 suggestedPlan}: UpgradeModalProps) {const [billingInterval, setBillingInterval] = useState<BillingInterval>(currentBillingInterval)

 if (!isOpen) return null

 const formatPrice = (cents: number): string => {return`$${(cents / 100).toFixed(2)}`}

 const calculateYearlySavings = (monthly: number, yearly: number): number => {const monthlyCost = monthly * 12
 const savings = monthlyCost - yearly
 return Math.round((savings / monthlyCost) * 100)}

 const plans = [{id:'single' as SubscriptionPlan,
 name:'Single User',
 icon:'👤',
 ...
 features: ['1 family member seat','2 external caregivers','Basic health tracking','Meal logging & recipes','Weight & step tracking'],
 ...
 features: ['5 family member seats','5 external caregivers','Track humans & pets','Family health dashboard','Shared meal planning'],
 ...
 features: ['10 family member seats','10 external caregivers','Advanced analytics & insights','Enhanced AI coaching','Priority support'],
 ...
 features: ['Unlimited family members','Unlimited external caregivers','All premium features','White-glove support','Early access to features'],
 highlighted: suggestedPlan ==='family_premium',
 badge:'Best Value'}]

 return (<div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
 ...
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${billingInterval ==='monthly'
 ...
 className={`px-4 py-2 rounded-lg font-medium transition-colors relative ${billingInterval ==='yearly'
 ?'bg-primary text-white'
 :'bg-muted text-muted-foreground hover:bg-muted/80'}`}
 >
 Yearly
 ...
 {plans.map((plan) => (<div
 key={plan.id}
 className={`rounded-lg border-2 p-6 relative ${plan.highlighted
 ...
 {billingInterval ==='yearly' && (<div className="text-xs text-success mt-1">
 ...
 onClick={() => {// Close modal and show a toast notification
 onClose()
 // TODO: Implement actual payment/upgrade flow
 if (typeof window !=='undefined') {const toast = (window as any).toast
 if (toast) {toast.success(`Upgrade to ${plan.name} coming soon!`)}}}}
 >

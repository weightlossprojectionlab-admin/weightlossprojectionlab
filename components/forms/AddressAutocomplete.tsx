...
interface AddressAutocompleteProps {value: string
 ...
 onPlaceSelected?: (place: {address: string
 city: string
 state: string
 zipCode: string
 ...
 * value={address}
 * onChange={setAddress}
 * onPlaceSelected={(place) => {* setCity(place.city)
...
export default function AddressAutocomplete({value,
 ...
 label ='Address',
 required = false,
 className =''}: AddressAutocompleteProps) {const inputRef = useRef<HTMLInputElement>(null)
 const {isLoaded, error, initializeAutocomplete} = useGooglePlaces()

 useEffect(() => {if (!isLoaded || !inputRef.current || !onPlaceSelected) return

 const cleanup = initializeAutocomplete(inputRef.current, (place) => {// Update address field
 onChange(place.address)

 // Notify parent of full place details
 onPlaceSelected({address: place.address,
 city: place.city,
 state: place.state,
 zipCode: place.zipCode,
 coordinates: place.coordinates})})

 return cleanup}, [isLoaded, initializeAutocomplete, onChange, onPlaceSelected])

 return (<div className={className}>
 {label && (<label className="block text-sm font-medium text-foreground mb-2">
 ...
 {isLoaded && (<div className="absolute right-3 top-1/2 -translate-y-1/2">
 ...
 {error && (<p className="text-xs text-yellow-600 mt-1">

...
interface RecipeImportModalProps {isOpen: boolean
 onClose: () => void
 onSuccess?: () => void}

export function RecipeImportModal({isOpen, onClose, onSuccess}: RecipeImportModalProps) {const [url, setUrl] = useState('')
 ...
 const handleImport = async () => {if (!url.trim()) {toast.error('Please enter a recipe URL')
 return}

 setLoading(true)
 try {// Get auth token
 const user = auth.currentUser
 if (!user) {toast.error('You must be logged in to import recipes')
 return}

 const idToken = await user.getIdToken()

 // Call API to import recipe
 const response = await fetch('/api/recipes/import', {method:'POST',
 ...
 setStep('preview')
 toast.success('Recipe imported successfully!')} catch (error: any) {logger.error('Import error:', error as Error)
 toast.error(error.message ||'Failed to import recipe')} finally {setLoading(false)}}

 const handleSave = async () => {if (!importedRecipe) return

 setStep('saving')
 setLoading(true)

 try {// Convert ImportedRecipe to MealSuggestion format
 ...
 const recipe: Partial<MealSuggestion> = {name: importedRecipe.name,
 ...
 macros: {protein: perServingProtein,
 ...
 onSuccess?.()
 handleClose()} catch (error: any) {logger.error('Save error:', error as Error)
 toast.error(error.message ||'Failed to save recipe')} finally {setLoading(false)
 setStep('url')}}
 ...
 <div className="mb-6">
 <label htmlFor="recipe-url"className="block text-sm font-medium text-foreground mb-2">
 Recipe URL
 </label>
 <input
 id="recipe-url"type="url"value={url}
 onChange={(e) => setUrl(e.target.value)}
 placeholder="https://www.example.com/recipe/..."className="w-full px-4 py-3 border border-border bg-background text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"disabled={loading}
 />
 </div>
 ...
 onClick={handleClose}
 disabled={loading}
 className="px-6 py-3 border border-border text-foreground rounded-lg hover:bg-background transition-colors font-medium">
 Cancel
 </button>
 </div>

 {/* Example URLs */}
 <div className="mt-6 p-4 bg-secondary-light rounded-lg border border-secondary-light">
 <p className="text-sm font-medium text-blue-900 mb-2">✨ Supported Sites:</p>
 ...
 {step ==='preview' && importedRecipe && (<div>
 <div className="mb-6">
 {importedRecipe.imageUrl && (<img
 ...
 {importedRecipe.prepTime && (<span>⏱️ Prep: {importedRecipe.prepTime} min</span>)}
 {importedRecipe.cookTime && (<span>🔥 Cook: {importedRecipe.cookTime} min</span>)}
 {importedRecipe.servings && (<span>🍽️ Servings: {importedRecipe.servings}</span>)}
 {importedRecipe.author && (<span>👨‍🍳 By {importedRecipe.author}</span>)}
 </div>
 ...
 {importedRecipe.fiber !== undefined && (<div className="bg-muted rounded-lg p-3 text-center">
 ...
 {(['breakfast','lunch','dinner','snack'] as MealType[]).map(type => (<button
 key={type}
 onClick={() => setMealType(type)}
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${mealType === type
 ...
 onClick={handleSave}
 disabled={loading}
 className="flex-1 px-6 py-3 bg-success text-white rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors font-medium flex items-center justify-center gap-2">
 {loading ? (<>
 <Spinner size="sm"/>
 Saving...
 </>) : (<>

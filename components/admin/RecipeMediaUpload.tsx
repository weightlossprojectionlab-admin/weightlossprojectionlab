...
export function RecipeMediaUpload({ recipe, onClose, onSuccess }: RecipeMediaUploadProps) {
  ...
  const initializeImageSlots = (): ImageSlot[] => {
    ...
    for (let i = 0; i < 4; i++) {
      slots.push({
        file: null,
        preview: recipe.imageUrls?.[i] || recipe.imageUrl && i === 0 ? recipe.imageUrl : null
      })
    }
    return slots
  }
  ...
  const handleDeleteImage = (slotIndex: number) => {
    setImageSlots(prev => {
      ...
      newSlots[slotIndex] = {
        file: null,
        preview: recipe.imageUrls?.[slotIndex] || recipe.imageUrl && slotIndex === 0 ? recipe.imageUrl : null
      }
      return newSlots
    })

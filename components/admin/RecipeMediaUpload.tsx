...
interface RecipeMediaUploadProps {recipe: MealSuggestion
 onClose: () => void
 onSuccess: () => void}

interface ImageSlot {file: File | null
 preview: string | null}

export function RecipeMediaUpload({recipe, onClose, onSuccess}: RecipeMediaUploadProps) {const {user} = useAuth()

 // Initialize 4 image slots with existing images if available
 const initializeImageSlots = (): ImageSlot[] => {const slots: ImageSlot[] = []
 for (let i = 0; i < 4; i++) {slots.push({file: null,
 ...
 const handleImageSelect = (slotIndex: number) => (e: React.ChangeEvent<HTMLInputElement>) => {const file = e.target.files?.[0]
 if (!file) return
 ...
 if (file.size > 5 * 1024 * 1024) {toast.error('Image must be less than 5MB')
 return}

 // Create preview
 const reader = new FileReader()
 reader.onloadend = () => {setImageSlots(prev => {const newSlots = [...prev]
 newSlots[slotIndex] = {file,
 preview: reader.result as string}
 return newSlots})}
 reader.readAsDataURL(file)}

 const handleBulkImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {const files = Array.from(e.target.files || [])
 if (files.length === 0) return

 // Validate that we have between 1-4 files
 if (files.length > 4) {toast.error('You can upload a maximum of 4 images')
 return}

 // Validate all files
 const invalidFiles = files.filter(file => !file.type.startsWith('image/'))
 if (invalidFiles.length > 0) {toast.error('All files must be valid images')
 return}

 const oversizedFiles = files.filter(file => file.size > 5 * 1024 * 1024)
 if (oversizedFiles.length > 0) {toast.error('All images must be less than 5MB')
 return}
 ...
 files.forEach((file, index) => {if (index >= 4) return // Safety check

 const reader = new FileReader()
 reader.onloadend = () => {newSlots[index] = {file,
 preview: reader.result as string}
 processedCount++

 // Update state once all files are processed
 if (processedCount === files.length) {setImageSlots(newSlots)
 toast.success(`${files.length} image${files.length > 1 ?'s' :''} selected`)}}
 reader.readAsDataURL(file)})}

 const handleDeleteImage = (slotIndex: number) => {setImageSlots(prev => {const newSlots = [...prev]
 ...
 const handleVideoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {const file = e.target.files?.[0]
 if (!file) return

 // Validate file type
 if (!file.type.startsWith('video/')) {toast.error('Please select a valid video file')
 return}

 // Validate file size (max 20MB)
 if (file.size > 20 * 1024 * 1024) {toast.error('Video must be less than 20MB')
 return}
 ...
 if (duration < 5 || duration > 7) {toast.error('Video must be 5-7 seconds long')
 return}
 setVideoFile(file)
 setVideoPreview(URL.createObjectURL(file))}
 video.src = URL.createObjectURL(file)}

 const handleUpload = async () => {const hasAnyImage = imageSlots.some(slot => slot.file !== null)

 if (!hasAnyImage && !videoFile) {toast.error('Please select at least one file to upload')
 return}

 if (!user) {toast.error('You must be logged in to upload media')
 return}

 setUploading(true)
 setUploadProgress(0)

 try {// Collect image files
 const imageFiles = imageSlots
 .filter(slot => slot.file !== null)
 .map(slot => slot.file!)

 // Upload to Firebase Storage (client-side) with SEO-friendly filenames
 const uploadResult = await uploadRecipeMedia(recipe.id,
 ...
 await setDoc(recipeRef, {imageUrls: uploadResult.imageUrls,
 imageStoragePaths: uploadResult.imageStoragePaths,
 ...(uploadResult.videoUrl && {videoUrl: uploadResult.videoUrl,
 videoStoragePath: uploadResult.videoStoragePath,}),
 mediaUploadedAt: Timestamp.now(),
 mediaUploadedBy: user.uid,}, {merge: true})

 toast.success('Media uploaded successfully!')
 onSuccess()
 onClose()} catch (error) {logger.error('Upload error:', error as Error)
 const errorMessage = error instanceof Error ? error.message :'Failed to upload media'
 toast.error(errorMessage, {duration: 5000})} finally {setUploading(false)
 setUploadProgress(0)}}

 const handleDeleteVideo = () => {setVideoFile(null)
 ...
 >
 <div
 className="bg-card rounded-lg max-w-6xl w-full max-h-[90vh] overflow-y-auto"onClick={(e) => e.stopPropagation()}
 >
 {/* Header */}
 ...
 <div
 className={`mb-4 relative transition-all duration-200 ${isDragging
 ?'border-4 border-solid border-purple-500 bg-primary-light'
 :'border-2 border-dashed border-border hover:border-purple-400'}`}
 ...
 <PhotoIcon className={`h-12 w-12 ${isDragging ?'text-primary animate-bounce' :'text-muted-foreground'}`} />
 <div>
 ...
 <span className="px-2 bg-card text-muted-foreground">or upload individually</span>
 </div>
 </div>

 <div className="grid grid-cols-2 gap-4">
 {imageSlots.map((slot, index) => (<div
 key={index}
 className="border-2 border-dashed border-border rounded-lg p-4 relative">
 {/* Primary Badge */}
 {index === 0 && (<div className="absolute -top-2 -right-2 bg-yellow-400 text-yellow-900 px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1 shadow-md z-10">
 <StarIcon className="h-3 w-3"/>
 Primary
 </div>)}

 {slot.preview ? (<div className="relative">
 ...
 type="file"accept="image/*"onChange={handleImageSelect(index)}
 className="hidden"/>
 </div>))}
 </div>
 </div>
 ...
 <input
 ref={videoInputRef}
 type="file"accept="video/*"onChange={handleVideoSelect}
 className="hidden"/>
 </div>
 </div>

 {/* Upload Progress */}
 {uploading && (<div className="bg-secondary-light border border-secondary-light rounded-lg p-4">
 ...
 <div
 className="bg-secondary h-2 rounded-full transition-all"style={{width:`${uploadProgress}%`}}
 />
 </div>
 </div>)}
 ...
 onClick={onClose}
 disabled={uploading}
 className="flex-1 bg-gray-200 text-foreground px-6 py-3 rounded-lg font-medium hover:bg-gray-300 disabled:opacity-50">
 Cancel
 </button>
 <button

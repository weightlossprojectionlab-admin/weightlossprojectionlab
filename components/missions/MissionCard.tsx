...
export default function MissionCard({ mission, onComplete }: MissionCardProps) {
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      ...
      case 'medium':
        return 'bg-yellow-100 text-warning-dark dark:text-yellow-300';
      case 'hard':
  ...
  return (
    <div className={`border rounded-lg p-5 transition-all hover:shadow-md ${
      ...
      <div className="flex items-center space-x-3 mb-4">
        ...
        {mission.xpReward && (
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-light text-purple-800 dark:bg-purple-900/20">

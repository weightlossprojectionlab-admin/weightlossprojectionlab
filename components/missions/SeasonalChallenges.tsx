...
// Displays seasonal challenges and events'use client';
...
interface SeasonalChallengesProps {challenges: SeasonalChallenge[];
 onJoinChallenge?: (challengeId: string) => void;}

export default function SeasonalChallenges({challenges, onJoinChallenge}: SeasonalChallengesProps) {if (challenges.length === 0) {return (<div className="bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-lg p-8 text-center">
 ...
 {challenges.map((challenge) => {const startDate = timestampToDate(challenge.startDate);
 ...
 return (<div
 key={challengeId}
 className={`border rounded-lg p-6 ${isActive
 ?'bg-gradient-to-br from-purple-50 to-pink-50 border-purple-200'
 : isUpcoming
 ?'bg-indigo-100 /20 border-accent'
 ...
 {isActive && (<span className="font-medium text-primary-dark">
 {daysRemaining} day{daysRemaining !== 1 ?'s' :''} left
 </span>)}
 </div>

 {/* Rewards */}
 {challenge.rewards && (<div className="bg-card rounded-lg p-3 mb-4">
 <div className="text-sm font-medium text-foreground mb-2">Rewards:</div>
 <div className="flex flex-wrap gap-2">
 {challenge.rewards.xp && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary-light /20 text-purple-800">
 {challenge.rewards.xp} XP
 </span>)}
 {challenge.rewards.badge && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-warning-dark">
 ...
 {/* Participants */}
 {challenge.participantCount !== undefined && (<div className="flex items-center space-x-2 text-sm text-muted-foreground mb-4">
 <UsersIcon className="h-4 w-4"/>
 <span>{challenge.participantCount.toLocaleString()} participants</span>
 </div>)}

 {/* Action Button */}
 {isActive && onJoinChallenge && (<button
 ...
 {isUpcoming && (<div className="text-center text-sm text-accent-dark font-medium py-2">
 Coming Soon!
 </div>)}

 {isEnded && (<div className="text-center text-sm text-muted-foreground font-medium py-2">
 Challenge Ended
 </div>)}
 </div>);})}
 </div>

...
export function RecipeCard({ recipe, onSave, onView, isSaved = false, priority = false }: RecipeCardProps) {
  ...
  return (
    <Link
      ...
      className="group block bg-background border border-border rounded-lg overflow-hidden hover:shadow-lg transition-all duration-200"
      ...
      <div className="relative w-full aspect-[3/4] overflow-hidden">
        ...
        <div className={`absolute inset-0 bg-black/40 flex items-center justify-center space-x-3 transition-opacity duration-200 ${showActions ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <button
            onClick={handleShare}
            disabled={sharing}
            className="flex items-center space-x-2 px-4 py-2 bg-white text-foreground rounded-full font-medium hover:bg-gray-100 transition-all"

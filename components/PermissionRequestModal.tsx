...
interface PermissionRequestModalProps {type:'camera' |'biometric'
 isOpen: boolean
 onClose: () => void
...
export default function PermissionRequestModal({type,
 isOpen,
 onClose,
 onRequest,
 onUseAlternative,
 alternativeLabel ='Use alternative method'}: PermissionRequestModalProps) {const [isRequesting, setIsRequesting] = useState(false)
 ...
 const handleRequest = async () => {setIsRequesting(true)
 try {await onRequest()
 onClose()} catch (error) {logger.error('Permission request failed:', error as Error)
 setShowInstructions(true)} finally {setIsRequesting(false)}}

 const handleUseAlternative = () => {onUseAlternative?.()
 onClose()}

 const getPermissionRationale = () => {if (type ==='camera') {return {title:'Camera Access Required',
 description:'We need access to your camera to take photos of your meals for AI analysis.',
 benefits: ['Automatically identify foods in your meals','Get instant calorie and nutrition estimates','Track your meals with visual records','Improve accuracy of your food log']}} else {return {title:'Biometric Authentication',
 description:`Set up ${isIOS() ?'Face ID or Touch ID' : isAndroid() ?'Fingerprint' :'biometric authentication'} for quick and secure sign-in.`,
 benefits: ['Sign in quickly without typing passwords','Enhanced security with device-level protection','Your biometric data never leaves your device','Optional - you can always use your password']}}}
 ...
 {rationale.benefits.map((benefit, index) => (<li key={index} className="flex items-start space-x-2 text-sm text-foreground">
 ...
 {/* Privacy Note */}
 {type ==='biometric' && (<div className="bg-indigo-100 /20 rounded-lg p-4">
 ...
 {/* Mobile Reminder */}
 {mobile && (<div className="bg-indigo-100 /20 border border-accent rounded-lg p-4">
 <p className="text-xs text-accent-dark">
 ...
 className="btn btn-primary w-full"aria-label={`Grant ${typeName} permission`}
 >
 {isRequesting ? (<span className="flex items-center justify-center space-x-2">
 <div className="animate-spin w-5 h-5 border-2 border-white border-t-transparent rounded-full"/>
 <span>Requesting...</span>
 </span>) : (<span>Grant {typeName} Access</span>)}
 </button>

 {onUseAlternative && (<button
 onClick={handleUseAlternative}
 disabled={isRequesting}
 className="btn btn-secondary w-full"aria-label={alternativeLabel}
 >
 {alternativeLabel}
 </button>)}
 ...
 onClick={onClose}
 disabled={isRequesting}
 className="w-full text-sm text-muted-foreground hover:text-foreground py-2"aria-label="Close dialog">
 Maybe later
 </button>
 </div>
 </>) : (<>
 ...
 <div>
 <h3 className="text-sm font-semibold text-warning-dark mb-1">Permission Denied</h3>
 ...
 <button
 onClick={handleRequest}
 className="btn btn-primary w-full"aria-label="Try again">
 Try Again
 </button>

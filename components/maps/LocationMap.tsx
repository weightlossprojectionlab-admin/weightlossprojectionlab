...
interface LocationMapProps {address: string
...
export default function LocationMap({address,
 coordinates,
 title,
 showDirections = true}: LocationMapProps) {const [showMap, setShowMap] = useState(false)

 const handleGetDirections = () => {const url = getDirectionsUrl({destination: address,
 mode:'driving'})
 window.open(url,'_blank')}

 const mapUrl = showMap
 ? getStaticMapUrl({address,
 coordinates,
 width: 600,
 height: 400,
 zoom: 15})
 ...
 {showMap && mapUrl && (<div className="rounded-lg overflow-hidden border border-border">
 <img
 src={mapUrl}
 alt="Location map"loading="lazy"className="w-full h-auto"onError={(e) => {// Handle map load error
 e.currentTarget.style.display ='none'
 console.error('Failed to load map image')}}
 />
 </div>)}

 {showMap && !mapUrl && (<div className="p-4 bg-yellow-50 /20 border border-yellow-200 rounded-lg text-sm text-yellow-800">

  ...
  if (variant === 'icon') {
    return (
      <button
        onClick={handleShare}
        disabled={sharing}
        className={`p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
        aria-label="Share"
      >
        {sharing ? (
          <div className={`animate-spin rounded-full border-2 border-gray-300 border-t-primary ${iconSizes[size]}`} />
        ) : (
          <svg className={`${iconSizes[size]} text-gray-600 dark:text-gray-400`} fill="none" stroke="currentColor" viewBox="0 0 24 24">

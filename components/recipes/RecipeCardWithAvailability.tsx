...
interface RecipeCardWithAvailabilityProps {recipe: MealSuggestion
...
export function RecipeCardWithAvailability({recipe,
 availabilityScore = 0,
 ingredientsFound = 0,
 totalIngredients = 0,
 estimatedCostMin,
 ...
 onAddToCart}: RecipeCardWithAvailabilityProps) {const [addingToCart, setAddingToCart] = useState(false)

 const handleAddToCart = async (e: React.MouseEvent) => {e.preventDefault()
 e.stopPropagation()

 const user = auth.currentUser
 if (!user) {toast.error('Please sign in to add items to your shopping list')
 return}

 setAddingToCart(true)
 try {// Add all ingredients to shopping list
 const promises = recipe.ingredients.map(ingredient =>
 addManualShoppingItem(user.uid, ingredient, {recipeId: recipe.id,
 quantity: 1}))

 await Promise.all(promises)
 toast.success(`Added ${recipe.ingredients.length} ingredients to shopping list`)
 onAddToCart?.(recipe.id)} catch (error) {logger.error('Error adding recipe to cart', error as Error, {recipeId: recipe.id})
 toast.error('Failed to add ingredients to shopping list')} finally {setAddingToCart(false)}}
 ...
 {/* Availability Badges */}
 {showAvailability && availabilityScore > 0 && (<div className="px-4 py-3 bg-background/50 border-b border-border">
 ...
 {/* Dietary Tags */}
 {recipe.dietaryTags.length > 0 && (<div className="flex flex-wrap gap-1">
 {recipe.dietaryTags.slice(0, 3).map(tag => (<span key={tag} className="text-xs bg-primary-light text-primary px-2 py-0.5 rounded">
 {tag}
 </span>))}
 {recipe.dietaryTags.length > 3 && (<span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded">
 ...
 className="flex-1 py-2 px-4 bg-success hover:bg-green-700 disabled:bg-gray-400 text-white rounded-lg font-medium transition-colors text-sm flex items-center justify-center gap-2">
 {addingToCart ? (<>

...
interface RecipeWithProductMatchingProps {recipe: MealSuggestion
 onClose?: () => void}

export function RecipeWithProductMatching({recipe, onClose}: RecipeWithProductMatchingProps) {const [showProductMatches, setShowProductMatches] = useState(false)
 const [addingToCart, setAddingToCart] = useState(false)

 // Parse ingredients for product matching
 const ingredients = recipe.ingredients.map(ing => {// Simple parsing - could be enhanced
 ...
 const handleAddToCart = async (barcode: string, ingredientName: string) => {const user = auth.currentUser
 if (!user) {toast.error('Please sign in to add items to your shopping list')
 return}

 setAddingToCart(true)
 try {await addManualShoppingItem(user.uid, ingredientName, {recipeId: recipe.id,
 quantity: 1})

 toast.success(`Added ${ingredientName} to shopping list`)} catch (error) {toast.error('Failed to add to shopping list')} finally {setAddingToCart(false)}}

 const handleAddAllToCart = async () => {if (!result) return
 ...
 setAddingToCart(true)
 try {const promises = result.matches
 .filter(match => match.matches.length > 0)
 .map(match => {return addManualShoppingItem(user.uid, match.ingredient, {recipeId: recipe.id,
 quantity: match.quantity || 1})})

 await Promise.all(promises)
 toast.success(`Added ${promises.length} items to shopping list`)} catch (error) {toast.error('Failed to add items to shopping list')} finally {setAddingToCart(false)}}

 return (<div className="space-y-4">
 {/* Product Matching Toggle Button */}
 {!showProductMatches ? (<button
 ...
 {/* Error State */}
 {error && (<div className="bg-error-light /20 border border-red-200 rounded-lg p-4">
 ...
 className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-foreground rounded-lg font-medium transition-colors">
 Hide
 </button>
 </div>

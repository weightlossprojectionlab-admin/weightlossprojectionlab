...
interface OrderInspectionProps {order: ShopAndDeliverOrder
 items: any[]
 onReportIssue: (itemId: string) => void
 onConfirmAllCorrect: () => Promise<void>}

export function OrderInspection({order,
 items,
 onReportIssue,
 onConfirmAllCorrect}: OrderInspectionProps) {const [remainingHours, setRemainingHours] = useState<number>(0)
 const [loading, setLoading] = useState(false)

 // Update countdown timer
 useEffect(() => {if (!order.deliveredAt) return

 const deliveredAt = (order.deliveredAt as any)?.toDate
 ? (order.deliveredAt as any).toDate()
 : new Date(order.deliveredAt)

 const updateTimer = () => {const remaining = getRemainingInspectionTime(deliveredAt)
 setRemainingHours(remaining)}

 updateTimer()
 const interval = setInterval(updateTimer, 60000) // Update every minute

 return () => clearInterval(interval)}, [order.deliveredAt])
 ...
 const getItemIssues = (itemId: string) => {return order.reportedIssues?.filter((issue: any) => issue.itemId === itemId) || []}

 // Handle confirm all correct
 const handleConfirmAllCorrect = async () => {setLoading(true)
 try {await onConfirmAllCorrect()} catch (error) {console.error('[OrderInspection] Error confirming all correct:', error)} finally {setLoading(false)}}

 // Format remaining time
 const formatRemainingTime = () => {if (remainingHours <= 0) return'Expired'
 ...
 const getTimeColor = () => {if (remainingHours <= 0) return'text-error'
 ...
 {order.inspectionCompleted ? (<div className="flex items-center gap-2 text-success-light0">
 <CheckCircleSolid className="h-6 w-6"/>
 <span className="font-semibold">Inspection Complete</span>
 </div>) : (<div className={`flex items-center gap-2 ${getTimeColor()}`}>
 <ClockIcon className="h-6 w-6"/>
 <span className="font-semibold">{formatRemainingTime()}</span>
 </div>)}
 </div>

 {!withinDeadline && !order.inspectionCompleted && (<div className="bg-error-light /20 border border-error rounded-lg p-4 flex items-start gap-3">
 ...
 {withinDeadline && !order.inspectionCompleted && (<p className="text-muted-foreground">
 ...
 {items.map((item) => {const issues = getItemIssues(item.id)
 const hasIssues = issues.length > 0

 return (<div
 key={item.id}
 className={`p-4 hover:bg-muted/50 transition-colors ${hasIssues ?'bg-error-light/10 /10' :''}`}
 >
 ...
 className="w-20 h-20 object-cover rounded-lg flex-shrink-0"/>) : (<div className="w-20 h-20 bg-gray-200 rounded-lg flex items-center justify-center flex-shrink-0">
 ...
 {item.brand && (<p className="text-sm text-muted-foreground mb-2">
 {item.brand}
 </p>)}
 ...
 {item.scannedPriceCents && (<span>
 ${(item.scannedPriceCents / 100).toFixed(2)}
 </span>)}
 </div>

 {/* Issues */}
 {hasIssues && (<div className="space-y-2 mb-3">
 {issues.map((issue: any, idx: number) => (<div
 key={idx}
 className="bg-error-light /20 border border-error/30 rounded-lg p-3">
 ...
 {issue.resolution && (<p className="text-xs text-muted-foreground">
 ...
 {/* Action Button */}
 {withinDeadline && !order.inspectionCompleted && (<button
 onClick={() => onReportIssue(item.id)}
 className={`px-4 py-2 rounded-lg font-medium transition-colors ${hasIssues
 ...
 {hasIssues ? (<ExclamationTriangleIcon className="h-8 w-8 text-error"/>) : (<CheckCircleIcon className="h-8 w-8 text-muted-foreground"/>)}
 </div>
 </div>
 </div>)})}
 </div>
 ...
 {withinDeadline && !order.inspectionCompleted && (<div className="bg-card rounded-lg shadow-sm border border-border p-6">
 ...
 {order.inspectionCompleted && (<div className="bg-success-light /20 border border-success-light0 rounded-lg p-6">
 ...
 {order.inspectionCompletedAt && (<p className="text-xs text-muted-foreground mt-2">
 Completed on:{''}
 {new Date((order.inspectionCompletedAt as any)?.toDate
 ? (order.inspectionCompletedAt as any).toDate()
 : order.inspectionCompletedAt).toLocaleString()}
 </p>)}
 </div>
 </div>
 </div>)}

...
export function UrgentRecommendationsWidget() {const router = useRouter()
 const {recommendations, loading} = useRecommendations()

 // Filter for urgent and high severity
 const urgentRecs = recommendations.filter(r => r.urgency ==='urgent' || r.severity ==='high')

 if (loading) {return (<div className="bg-card rounded-lg shadow-sm p-5">
 ...
 if (urgentRecs.length === 0) {return null // Don't show widget if no urgent recommendations}

 return (<div className="bg-gradient-to-br from-orange-50 to-red-50 /10 /10 border-2 border-orange-200 rounded-lg shadow-sm p-5">
 {/* Header */}
 ...
 {urgentRecs.slice(0, 2).map(rec => (<div
 key={rec.id}
 className="bg-background rounded-lg p-3 border border-border">
 <div className="flex items-start gap-2">
 <span className={`px-2 py-0.5 rounded text-xs font-bold ${rec.urgency ==='urgent'
 ?'bg-red-100 /30 text-error-dark'
 :'bg-orange-100 /30 text-orange-700'}`}>
 ...
 {urgentRecs.length > 2 && (<p className="text-xs text-muted-foreground pl-3">

...
interface RecommendationCardProps {recommendation: AppointmentRecommendation
 onSchedule: (recommendation: AppointmentRecommendation) => void
 onDismiss: (recommendationId: string) => void}

export function RecommendationCard({recommendation,
 onSchedule,
 onDismiss}: RecommendationCardProps) {const getSeverityColor = (severity: string) => {switch (severity) {case'urgent':
 return {border:'border-red-500',
 bg:'bg-error-light /10',
 text:'text-error-dark',
 badge:'bg-red-100 /30 text-error-dark',
 ...
 badge:'bg-orange-100 /30 text-orange-700',
 ...
 badge:'bg-yellow-100 /30 text-yellow-700',
 ...
 text:'text-blue-700',
 badge:'bg-blue-100 /30 text-blue-700',
 button:'bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800'}}}

 const getUrgencyLabel = (urgency: string) => {switch (urgency) {case'urgent':
 ...
 const getTypeLabel = (type: string) => {switch (type) {case'nutritionist':
 ...
 const getProviderTypeLabel = (type: string) => {switch (type) {case'nutritionist':
 return'Nutritionist'
 case'doctor':
 case'doctor-checkup':
 return'Doctor'
 ...
 const getCtaText = (severity: string, providerType: string) => {const provider = getProviderTypeLabel(providerType)

 switch (severity) {case'urgent':
 return`Schedule with ${provider} Now`case'high':
 return`Schedule with ${provider} Soon`default:
 return`Schedule with ${provider}`}}

 const formatLastVisit = (daysSinceLastVisit?: number | null) => {if (!daysSinceLastVisit) return null

 const weeks = Math.floor(daysSinceLastVisit / 7)
 if (weeks === 0) return`${daysSinceLastVisit} days ago`if (weeks === 1) return'1 week ago'
 if (weeks < 4) return`${weeks} weeks ago`const months = Math.floor(weeks / 4)
 ...
 return (<div className={`border-l-4 ${colors.border} ${colors.bg} rounded-r-lg p-5 relative`}>
 {/* Dismiss button */}
 <button
 onClick={() => onDismiss(recommendation.id)}
 className="absolute top-3 right-3 p-1 text-muted-foreground hover:text-foreground transition-colors"title="Dismiss this recommendation">
 ...
 {recommendation.severity ==='urgent' ? (<ExclamationTriangleIcon className="w-5 h-5"/>) : (<ClockIcon className="w-5 h-5"/>)}
 </div>
 ...
 {isOverdue && (<span className="text-xs px-2 py-0.5 rounded-full font-medium bg-red-100 /40 text-red-700 flex items-center gap-1">
 ...
 {/* Last Visit Info */}
 {lastVisitText && (<div className="mb-3 text-xs text-muted-foreground flex items-center gap-1.5">
 ...
 {/* Trigger Metrics */}
 {recommendation.triggerMetrics && (<div className="mb-4 text-xs text-muted-foreground space-y-1">
 {recommendation.triggerMetrics.weightLossStalled && (<div className="flex items-center gap-2">
 <span className="w-1.5 h-1.5 bg-gray-400 rounded-full"></span>
 <span>Weight loss stalled for 3+ weeks</span>
 </div>)}
 {recommendation.triggerMetrics.paceTooSlow && (<div className="flex items-center gap-2">
 <span className="w-1.5 h-1.5 bg-gray-400 rounded-full"></span>
 <span>Weight loss pace below target ({recommendation.triggerMetrics.weeklyChangeRate?.toFixed(1)} lbs/week)</span>
 </div>)}
 {recommendation.triggerMetrics.paceTooFast && (<div className="flex items-center gap-2">
 <span className="w-1.5 h-1.5 bg-gray-400 rounded-full"></span>
 <span>Rapid weight loss detected ({recommendation.triggerMetrics.weeklyChangeRate?.toFixed(1)} lbs/week)</span>
 </div>)}
 {recommendation.triggerMetrics.vitalAlert && (<div className="flex items-center gap-2">
 ...
 {recommendation.triggerMetrics.overdueAppointment && recommendation.triggerMetrics.daysSinceLastVisit && (<div className="flex items-center gap-2">
 <span className="w-1.5 h-1.5 bg-gray-400 rounded-full"></span>
 ...
 <button
 onClick={() => onDismiss(recommendation.id)}
 className="sm:w-auto px-4 py-2 text-sm text-muted-foreground hover:text-foreground border border-gray-300 hover:border-gray-400 rounded-lg transition-colors min-h-[44px]"title="Dismiss this recommendation">
 Dismiss
 </button>
 </div>
 </div>)}

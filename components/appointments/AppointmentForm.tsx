...
interface AppointmentFormProps {preSelectedPatientId?: string
 onSuccess?: (appointmentId: string) => void
 onCancel?: () => void}

export function AppointmentForm({preSelectedPatientId,
 onSuccess,
 onCancel}: AppointmentFormProps) {const {user} = useAuth()
 ...
 const {familyMembers} = useFamilyMembers({patientId: selectedPatientId,
 autoFetch: !!selectedPatientId})

 const [formData, setFormData] = useState({patientId: preSelectedPatientId ||'',
 providerId:'',
 dateTime:'',
 ...
 useEffect(() => {setSelectedPatientId(formData.patientId)}, [formData.patientId])

 // Get selected patient for pronoun
 const selectedPatient = patients.find(p => p.id === formData.patientId)
 const getPatientPronoun = () => {if (!selectedPatient) return'them'
 ...
 try {const patient = patients.find(p => p.id === formData.patientId)
 ...
 const appointmentData = {userId: user?.uid ||'',
 ...
 if (formData.requiresDriver && assignedDriver) {toast.success(`Driver request sent to ${assignedDriver.name}`)}

 // Reset form
 setFormData({patientId: preSelectedPatientId ||'',
 providerId:'',
 dateTime:'',
 ...
 onSuccess?.(newAppointment.id)} catch (error: any) {alert(error.message ||'Failed to schedule appointment. Please check all required fields and try again.')} finally {setLoading(false)}}

 return (<form onSubmit={handleSubmit} className="space-y-6">
 {/* Patient Selection - only show if not pre-selected */}
 {!preSelectedPatientId && (<div>
 ...
 {patients.length === 0 && (<p className="text-sm text-muted-foreground mt-1">
 ...
 {providers.length === 0 && (<p className="text-sm text-muted-foreground mt-1">
 ...
 placeholder="e.g., Annual checkup, Follow-up, Sick visit"required
 className="w-full px-4 py-2 border-2 border-border rounded-lg bg-background text-foreground focus:border-primary focus:ring-2 focus:ring-purple-600/20"/>
 </div>
 ...
 placeholder="Office address or telehealth"className="w-full px-4 py-2 border-2 border-border rounded-lg bg-background text-foreground focus:border-primary focus:ring-2 focus:ring-purple-600/20"/>
 </div>
 ...
 <button
 type="button"onClick={() => setFormData({...formData, requiresDriver: false, assignedDriverId:'', pickupTime:''})}
 className={`flex-1 px-4 py-3 rounded-lg border-2 font-medium transition-colors ${!formData.requiresDriver
 ...
 <button
 type="button"onClick={() => setFormData({...formData, requiresDriver: true})}
 className={`flex-1 px-4 py-3 rounded-lg border-2 font-medium transition-colors ${formData.requiresDriver
 ?'border-primary bg-primary-light text-primary-dark'
 :'border-border hover:border-border text-foreground'}`}
 >
 ...
 {formData.requiresDriver && (<div className="bg-secondary-light /10 border border-secondary-light rounded-lg p-4 space-y-4">
 <div className="flex items-center gap-2 text-blue-700 font-medium">
 <UserIcon className="w-5 h-5"/>
 <span>Assign a Family Member</span>
 </div>

 {familyMembers.length === 0 ? (<p className="text-sm text-muted-foreground">
 ...
 {familyMembers.map(member => (<option key={member.id} value={member.userId}>
 ...
 <input
 type="datetime-local"value={formData.pickupTime}
 ...
 rows={4}
 placeholder="Any additional information or preparation needed..."className="w-full px-4 py-2 border-2 border-border rounded-lg bg-background text-foreground focus:border-primary focus:ring-2 focus:ring-purple-600/20"/>
 </div>

 {/* Submit */}
 <div className="flex items-center gap-3 pt-4">
 <button
 type="submit"disabled={loading || patients.length === 0 || providers.length === 0}

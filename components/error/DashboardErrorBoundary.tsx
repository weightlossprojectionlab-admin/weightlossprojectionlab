 ...
 */
export class DashboardErrorBoundary extends Component<Props, State> {constructor(props: Props) {super(props)
 this.state = {hasError: false}}

 static getDerivedStateFromError(error: Error): State {return {hasError: true, error}}

 componentDidCatch(error: Error, errorInfo: any) {// Log error to console in development
 ...
 {this.state.error.stack && (<pre className="overflow-x-auto whitespace-pre-wrap mt-2 p-2 bg-background rounded">
 ...
 className="w-full px-6 py-3 bg-gray-200 text-foreground rounded-lg hover:bg-gray-300 transition-colors font-medium">
 Try Again
 </button>

 <a
 href="/"className="block w-full px-6 py-3 text-center text-sm text-muted-foreground hover:text-foreground transition-colors">
 Go to Home
 </a>
 </div>

...
export function HouseholdManager() {const {households, loading, error: fetchError} = useHouseholds()
 ...
 const handleDelete = async (householdId: string, householdName: string) => {const user = auth.currentUser
 if (!user) return

 if (!confirm(`Are you sure you want to delete"${householdName}"? All patients will be removed from this household.`)) {return}

 try {const token = await user.getIdToken()
 const response = await fetch(`/api/households/${householdId}`, {method:'DELETE',
 headers: {'Authorization':`Bearer ${token}`}})

 if (!response.ok) {throw new Error('Failed to delete household')}

 toast.success('Household deleted successfully')
 // Real-time listener will auto-update the list} catch (error) {logger.error('Failed to delete household', error as Error)
 toast.error('Failed to delete household')}}

 const handleEdit = (household: Household) => {setEditingHousehold(household)
 setIsFormOpen(true)}

 const handleFormSuccess = () => {setEditingHousehold(undefined)
 // Real-time listener will auto-update the list}

 const handleFormClose = () => {setIsFormOpen(false)
 setEditingHousehold(undefined)}

 const formatAddress = (address: Household['address']) => {const parts = [address.street,
 address.city,
 address.state,
 address.zipCode].filter(Boolean)
 return parts.join(',')}

 if (loading) {return (<div className="bg-card rounded-lg shadow p-6">
 ...
 if (fetchError) {return (<div className="bg-card rounded-lg shadow p-6">
 <h2 className="text-lg font-semibold text-foreground mb-4">Households</h2>
 <div className="bg-error-light /20 border-2 border-red-200 rounded-lg p-4">
 ...
 {households.length === 0 ? (<div className="text-center py-12 border-2 border-dashed border-border rounded-lg">
 ...
 {household.nickname && (<span className="text-sm text-muted-foreground">
 ({household.nickname})
 </span>)}
 </div>
 ...
 {household.kitchenConfig?.hasSharedInventory && (<span className="ml-2 px-2 py-0.5 bg-primary/10 text-primary rounded text-xs font-medium">
 Shared Kitchen
 </span>)}
 </div>
 </div>
 ...
 className="p-2 text-muted-foreground hover:text-primary hover:bg-primary-light rounded transition-colors"title="Edit household">
 <PencilIcon className="w-4 h-4"/>
 </button>
 <button
 onClick={() => handleDelete(household.id, household.name)}
 className="p-2 text-muted-foreground hover:text-red-600 hover:bg-red-50 /20 rounded transition-colors"title="Delete household">
 ...
 <button
 onClick={() => setExpandedHouseholdId(expandedHouseholdId === household.id ? null : household.id)}
 ...
 {expandedHouseholdId === household.id && (<div className="px-4 pb-4 pt-2">
 ...
 {/* Info Box */}
 <div className="mt-6 p-4 bg-blue-50 /20 border border-blue-200 rounded-lg">
 ...
 */
function HouseholdMembersList({householdId}: {householdId: string}) {const {members, loading, error} = useHouseholdMembers(householdId)

 if (loading) {return <div className="text-sm text-muted-foreground">Loading members...</div>}

 if (error) {return <div className="text-sm text-red-600">{error}</div>}

 if (members.length === 0) {return <div className="text-sm text-muted-foreground">No members assigned yet</div>}

 return (<div className="space-y-2">
 {members.map((patient) => (<div
 key={patient.id}

...
interface UnifiedCardViewProps {document: PatientDocument}

export default function UnifiedCardView({document}: UnifiedCardViewProps) {const [copiedField, setCopiedField] = useState<string | null>(null)
 const [showImageModal, setShowImageModal] = useState(false)
 const [currentImageIndex, setCurrentImageIndex] = useState(0)
 const [isFlipped, setIsFlipped] = useState(false)
 const [zoomLevel, setZoomLevel] = useState(1)

 const copyToClipboard = async (text: string, fieldName: string) => {try {await navigator.clipboard.writeText(text)
 setCopiedField(fieldName)
 toast.success(`${fieldName} copied!`)
 setTimeout(() => setCopiedField(null), 2000)} catch (error) {toast.error('Failed to copy')}}
 ...
 if (hasIdData) {bgGradient ='from-green-500 to-green-700'
 borderColor ='border-green-300'
 cardType ='ID Card'} else if (!hasInsuranceData) {bgGradient ='from-purple-500 to-purple-700'
 ...
 className="w-full h-auto cursor-pointer"onClick={() => setShowImageModal(true)}
 />
 </div>
 ...
 {/* Zoom Overlay */}
 <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer pointer-events-none"onClick={() => setShowImageModal(true)}
 >
 ...
 {/* Zoom Overlay */}
 <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer"onClick={() => setShowImageModal(true)}
 >
 ...
 {(metadata?.policyNumber || metadata?.memberName || metadata?.provider || metadata?.groupNumber) && (<div className="space-y-3">
 <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">
 Insurance Information
 </h4>

 {metadata?.provider && (<DataField
 label="Provider"value={metadata.provider}
 onCopy={() => copyToClipboard(metadata.provider!,'Provider')}
 copied={copiedField ==='Provider'}
 />)}

 {metadata?.memberName && (<DataField
 label="Member Name"value={metadata.memberName}
 onCopy={() => copyToClipboard(metadata.memberName!,'Member Name')}
 copied={copiedField ==='Member Name'}
 />)}

 {metadata?.policyNumber && (<DataField
 label="Policy Number"value={metadata.policyNumber}
 onCopy={() => copyToClipboard(metadata.policyNumber!,'Policy Number')}
 copied={copiedField ==='Policy Number'}
 mono
 />)}

 {metadata?.groupNumber && (<DataField
 label="Group Number"value={metadata.groupNumber}
 ...
 {/* ID Information */}
 {(metadata?.licenseNumber || metadata?.idNumber || metadata?.stateIssued) && (<div className="space-y-3 pt-4 border-t border-border">
 <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">
 ID Information
 </h4>

 {metadata?.licenseNumber && (<DataField
 label="License Number"value={metadata.licenseNumber}
 onCopy={() => copyToClipboard(metadata.licenseNumber!,'License Number')}
 copied={copiedField ==='License Number'}
 mono
 />)}

 {metadata?.idNumber && (<DataField
 label="ID Number"value={metadata.idNumber}
 onCopy={() => copyToClipboard(metadata.idNumber!,'ID Number')}
 copied={copiedField ==='ID Number'}
 mono
 />)}

 {metadata?.stateIssued && (<DataField
 label="State"value={metadata.stateIssued}
 ...
 {/* Additional Details */}
 {(metadata?.dates?.length || metadata?.phoneNumbers?.length || metadata?.addresses?.length || metadata?.names?.length || metadata?.amounts?.length) && (<div className="space-y-3 pt-4 border-t border-border">
 <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">
 Additional Details
 </h4>

 {metadata?.names && metadata.names.length > 0 && (<DataField
 label="Names"value={metadata.names.join(',')}
 onCopy={() => copyToClipboard(metadata.names!.join(','),'Names')}
 copied={copiedField ==='Names'}
 />)}

 {metadata?.dates && metadata.dates.length > 0 && (<DataField
 label="Dates"value={metadata.dates.join(',')}
 onCopy={() => copyToClipboard(metadata.dates!.join(','),'Dates')}
 copied={copiedField ==='Dates'}
 />)}

 {metadata?.phoneNumbers && metadata.phoneNumbers.length > 0 && (<DataField
 label="Phone Numbers"value={metadata.phoneNumbers.join(',')}
 onCopy={() => copyToClipboard(metadata.phoneNumbers!.join(','),'Phone Numbers')}
 copied={copiedField ==='Phone Numbers'}
 />)}

 {metadata?.addresses && metadata.addresses.length > 0 && (<DataField
 label="Addresses"value={metadata.addresses.join(' •')}
 onCopy={() => copyToClipboard(metadata.addresses!.join(' •'),'Addresses')}
 copied={copiedField ==='Addresses'}
 />)}

 {metadata?.amounts && metadata.amounts.length > 0 && (<DataField
 label="Amounts"value={metadata.amounts.join(',')}
 ...
 {/* Full Extracted Text */}
 {document.extractedText && (<div className="pt-4 border-t border-border">
 ...
 {metadata?.confidenceScore && (<span className="ml-3">
 ...
 {showImageModal && (<div className="fixed inset-0 bg-black/90 z-[9999] flex items-center justify-center p-4"onClick={closeModal}>
 <button
 onClick={closeModal}
 ...
 {/* Zoom Controls */}
 {document.fileType ==='image' && (<div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
 <button
 onClick={(e) => {e.stopPropagation()
 zoomIn()}}
 disabled={zoomLevel >= 3}
 className="p-3 bg-background/10 hover:bg-background/20 backdrop-blur-sm rounded-full text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"title="Zoom In">
 <svg className="w-6 h-6"fill="none"stroke="currentColor"viewBox="0 0 24 24">
 ...
 className="p-3 bg-background/10 hover:bg-background/20 backdrop-blur-sm rounded-full text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"title="Zoom Out">
 <svg className="w-6 h-6"fill="none"stroke="currentColor"viewBox="0 0 24 24">
 ...
 onClick={(e) => {e.stopPropagation()
 resetZoom()}}
 className="p-3 bg-background/10 hover:bg-background/20 backdrop-blur-sm rounded-full text-white transition-colors"title="Reset Zoom">
 <svg className="w-6 h-6"fill="none"stroke="currentColor"viewBox="0 0 24 24">
 <path strokeLinecap="round"strokeLinejoin="round"strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
 </svg>
 </button>)}
 ...
 {document.fileType ==='image' ? (hasFrontAndBack ? (/* Fullscreen 3D Flip for Front/Back Cards */
 <div className="relative max-w-4xl w-full overflow-auto"style={{perspective:'1500px'}}>
 <div
 className="relative w-full transition-all duration-600 ease-in-out"style={{transformStyle:'preserve-3d',
 ...
 src={images[0].url}
 alt={images[0].label ||'Front'}
 className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl mx-auto"style={{transformOrigin:'center center'}}
 />
 </div>
 ...
 </div>) : (/* Fullscreen Carousel for Multi-Page or Single Image */
 <div className="relative overflow-auto max-w-full max-h-full">
 <img
 src={images[currentImageIndex].url}
 alt={images[currentImageIndex].label || document.name}
 className="max-w-full max-h-full object-contain rounded-lg shadow-2xl transition-transform duration-300"style={{transform:`scale(${zoomLevel})`,
 ...
 className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${idx === currentImageIndex
 ...
 </div>)) : (<div className="bg-card rounded-lg shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col">
 ...
 className="w-full h-full"title={document.name}
 />
 </div>
 <div className="p-4 border-t border-border bg-background">
 <a
 href={document.originalUrl}
 target="_blank"rel="noopener noreferrer"className="text-primary hover:text-primary-dark font-medium text-sm">
 Open in new tab →
 </a>
 </div>
 </div>)}
...
function DataField({label,
 value,
 onCopy,
 copied,
 mono = false}: {label: string
 value: string
 onCopy: () => void
 copied: boolean
 mono?: boolean}) {return (<div className="flex items-center justify-between bg-background rounded-lg p-3 hover:bg-muted transition-colors group">
 ...
 <button
 onClick={onCopy}
 className="flex-shrink-0 p-2 text-muted-foreground hover:text-primary hover:bg-background rounded-md transition-all opacity-0 group-hover:opacity-100"title={`Copy ${label}`}
 >

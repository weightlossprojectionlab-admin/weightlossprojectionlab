...
export interface PhotoGalleryGridProps {photos: PhotoGalleryItem[]
 ...
 */
export function PhotoGalleryGrid({photos, onPhotoClick, loading, enableSocialShare = false}: PhotoGalleryGridProps) {if (loading) {return (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
 {[...Array(8)].map((_, i) => (<div
 key={i}
 className="bg-gray-200 rounded-xl animate-pulse"style={{aspectRatio:'9/16'}}
 />))}
 </div>)}

 if (photos.length === 0) {return (<div className="text-center py-16 px-4 max-w-2xl mx-auto">
 ...
 <a
 href="/log-meal"className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all font-semibold text-lg shadow-lg hover:shadow-xl transform hover:scale-105">
 ...
 {photos.map((photo, index) => (<SocialMediaCard
 key={photo.id}
 photo={photo}
 ...
 */
export interface GroupedGalleryProps {photosByDate: Map<string, PhotoGalleryItem[]>
 onPhotoClick: (photo: PhotoGalleryItem, index: number) => void
 enableSocialShare?: boolean}

export function GroupedPhotoGallery({photosByDate, onPhotoClick, enableSocialShare = false}: GroupedGalleryProps) {// Sort dates in descending order (most recent first)
 const sortedDates = Array.from(photosByDate.keys()).sort((a, b) => b.localeCompare(a))

 if (sortedDates.length === 0) {return (<div className="text-center py-12">
 <PhotoGalleryGrid photos={[]} onPhotoClick={onPhotoClick} />
 </div>)}

 return (<div className="space-y-8">
 {sortedDates.map(dateKey => {const photos = photosByDate.get(dateKey) || []

...
interface VitalsHistoryProps {vitals: VitalSign[]
...
export default function VitalsHistory({vitals,
 ...
 onDelete,
 canEdit = false,
 getDisplayName}: VitalsHistoryProps) {const [dateFilter, setDateFilter] = useState<DateFilter>('week')
 const [typeFilter, setTypeFilter] = useState<VitalType |'all'>('all')

 // Get unique vital types from data
 const availableTypes = useMemo(() => {const types = new Set<string>()
 vitals.forEach(v => types.add(v.type))
 return Array.from(types).sort()}, [vitals])

 // Filter vitals based on selected filters
 const filteredVitals = useMemo(() => {let filtered = [...vitals]

 // Date filter
 const now = new Date()
 const today = new Date(now.getFullYear(), now.getMonth(), now.getDate())

 switch (dateFilter) {case'today':
 filtered = filtered.filter(v => {const vitalDate = new Date(v.recordedAt)
 ...
 new Date(b.recordedAt).getTime() - new Date(a.recordedAt).getTime())}, [vitals, dateFilter, typeFilter])

 if (vitals.length === 0) {return null // Don't show if no vitals}
 ...
 {availableTypes.map(type => (<option key={type} value={type}>
 ...
 {filteredVitals.length > 0 ? (<table className="w-full">
 ...
 {filteredVitals.map((vital) => (<VitalRow
 key={vital.id}
 vital={vital}
 onEdit={onEdit}
 onDelete={onDelete}
 ...
 </table>) : (<div className="px-6 py-12 text-center">
...
function VitalRow({vital,
 ...
 onDelete,
 canEdit,
 getDisplayName}: {vital: VitalSign
 onEdit?: (vital: VitalSign) => void
 onDelete?: (vitalId: string, vitalType: string) => void
 canEdit: boolean
 getDisplayName?: (userId: string) => string}) {const label = getVitalTypeLabel(vital.type)
 const displayValue = formatVitalForDisplay(vital.type, vital.value, vital.unit)

 const dateTime = new Date(vital.recordedAt)
 const dateStr = dateTime.toLocaleDateString('en-US', {month:'short',
 day:'numeric',
 year:'numeric'})
 const timeStr = dateTime.toLocaleTimeString('en-US', {hour:'numeric',
 minute:'2-digit',
 hour12: true})
 ...
 return (<tr className="hover:bg-muted/30 transition-colors group">
 ...
 {status && (<span className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap ${status.className}`}>
 {status.label}
 </span>)}
 </div>
 ...
 {vital.notes ? (<span className="line-clamp-2 text-xs"title={vital.notes}>
 {vital.notes}
 </span>) : (<span className="text-muted-foreground/50 italic text-xs">—</span>)}
 </td>

 {/* Actions */}
 {canEdit && (<td className="px-3 py-3 text-right">
 <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
 {onEdit && (<button
 onClick={() => onEdit(vital)}
 className="p-1 text-blue-600 hover:text-blue-700 hover:bg-blue-50 /20 rounded transition-colors"title="Edit">
 <PencilIcon className="h-3.5 w-3.5"/>
 </button>)}
 {onDelete && (<button
 onClick={() => onDelete(vital.id, vital.type)}
 className="p-1 text-red-600 hover:text-red-700 hover:bg-red-50 /20 rounded transition-colors"title="Delete">
 ...
 */
function getTypeColor(type: string): string {const colors: Record<string, string> = {blood_pressure:'bg-red-100 text-red-700 /30',
 temperature:'bg-orange-100 text-orange-700 /30',
 pulse_oximeter:'bg-purple-100 text-purple-700 /30',
 blood_sugar:'bg-green-100 text-green-700 /30',
 weight:'bg-blue-100 text-blue-700 /30',
 bmi:'bg-indigo-100 text-indigo-700 /30'}

 return colors[type] ||'bg-gray-100 text-gray-700 /30'}

/**
 * Get status indicator (reused from DailyVitalsSummary - DRY)
 */
function getVitalStatus(vital: VitalSign): {label: string; className: string} | null {if (vital.type ==='blood_pressure' && typeof vital.value ==='object' &&'systolic' in vital.value) {const {systolic, diastolic} = vital.value
 if (systolic > 140 || diastolic > 90) {return {label:'High', className:'bg-red-100 text-red-700 /30'}}
 if (systolic < 90 || diastolic < 60) {return {label:'Low', className:'bg-yellow-100 text-yellow-700 /30'}}
 return {label:'Normal', className:'bg-green-100 text-green-700 /30'}}

 if (vital.type ==='temperature') {const temp = vital.value as number
 if (temp > 100.4) {return {label:'Fever', className:'bg-red-100 text-red-700 /30'}}
 if (temp < 97) {return {label:'Low', className:'bg-yellow-100 text-yellow-700 /30'}}
 return {label:'Normal', className:'bg-green-100 text-green-700 /30'}}

 if (vital.type ==='pulse_oximeter' && typeof vital.value ==='object' &&'spo2' in vital.value) {const {spo2} = vital.value
 if (spo2 < 90) {return {label:'Low', className:'bg-red-100 text-red-700 /30'}}
 return {label:'Normal', className:'bg-green-100 text-green-700 /30'}}

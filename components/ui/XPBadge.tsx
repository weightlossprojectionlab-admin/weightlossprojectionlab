...
export interface XPBadgeProps {gamification: UserGamification}

/**
 * Display user's level, XP, and progress to next level
 */
export const XPBadge = memo(function XPBadge({gamification}: XPBadgeProps) {const xpProgress = getXPProgress(gamification.totalXP)
 const nextLevelXP = getXPForNextLevel(gamification.level)

 return (<div className="bg-gradient-to-r from-primary to-primary-dark text-white rounded-lg p-4 shadow-lg">
 ...
 {/* Streak */}
 {gamification.currentStreak > 0 && (<div className="text-center">
 ...
 <div
 className="bg-card h-2 rounded-full transition-all duration-300"style={{width:`${xpProgress.percentage}%`}}
 />
 </div>
 ...
 * Badge showcase grid
 */
export interface BadgeShowcaseProps {badges: Badge[]
 limit?: number}

export function BadgeShowcase({badges, limit}: BadgeShowcaseProps) {const displayBadges = limit ? badges.slice(0, limit) : badges

 if (badges.length === 0) {return (<div className="text-center py-8 text-muted-foreground bg-background rounded-lg">
 ...
 // Rarity colors
 const rarityColors = {common:'border-border bg-background',
 ...
 const sortedBadges = [...displayBadges].sort((a, b) => {const rarityOrder = {legendary: 1, epic: 2, rare: 3, common: 4}
 ...
 {badge.unlockedAt && (<p className="text-xs text-muted-foreground mt-1">
 ...
 {limit && badges.length > limit && (<p className="text-center text-sm text-muted-foreground">
 ...
 */
export interface CompactXPProps {level: number
 totalXP: number}

export function CompactXP({level, totalXP}: CompactXPProps) {return (<div className="flex items-center gap-2 bg-primary text-white px-3 py-1.5 rounded-full text-sm font-medium">

...
export default function ConfirmModal({isOpen,
 ...
 variant ='danger',
 customIcon,
 iconSize ='medium'}: ConfirmModalProps) {if (!isOpen) return null

 const variantStyles = {danger: {icon:'🗑️',
 confirmButton:'bg-error hover:bg-error-dark text-white',
 headerBg:'bg-error-light /20'},
 warning: {icon:'⚠️',
 ...
 headerBg:'bg-primary-light /20'}}

 const styles = variantStyles[variant]

 const iconSizeStyles = {small:'h-10 w-10',
 medium:'h-12 w-12',
 large:'h-16 w-16'}

 const iconTextSizeStyles = {small:'text-2xl',
 medium:'text-3xl',
 large:'text-4xl'}
 ...
 <div
 className="fixed inset-0 bg-black bg-opacity-50 transition-opacity"aria-hidden="true"onClick={onClose}
 />
 ...
 {customIcon ? (<div className="flex items-center justify-center w-full h-full">
 {customIcon}
 </div>) : (<span className={iconTextSizeStyles[iconSize]} role="img"aria-label={variant}>
 {styles.icon}
 </span>)}
 </div>

...
interface WeightReminderModalProps {lastWeightLog: WeightLog | null
 ...
 */
function getTodayDateString(): string {return new Date().toISOString().split('T')[0]}

/**
 * Check if reminder was dismissed today
 */
function wasDismissedToday(): boolean {if (typeof window ==='undefined') return false

 try {const dismissedDate = localStorage.getItem(STORAGE_KEY)
 return dismissedDate === getTodayDateString()} catch {return false}}

/**
 * Mark reminder as dismissed for today
 */
function dismissForToday(): void {if (typeof window ==='undefined') return

 try {localStorage.setItem(STORAGE_KEY, getTodayDateString())} catch (error) {console.error('Failed to save dismissal to localStorage:', error)}}

export function WeightReminderModal({lastWeightLog,
 frequency,
 lastMealLogDate,
 patientName,
 onLogWeight,
 onDismiss}: WeightReminderModalProps) {const [isVisible, setIsVisible] = useState(false)

 useEffect(() => {// Check if we should show the reminder
 ...
 if (reminderStatus.shouldShow && !wasDismissedToday()) {setIsVisible(true)}}, [lastWeightLog, frequency, lastMealLogDate])

 const reminderStatus = shouldShowWeightReminder(lastWeightLog, frequency, lastMealLogDate)
 const colors = getWeightReminderColor(reminderStatus.status)
 const message = getWeightReminderMessage(reminderStatus, frequency)

 const handleRemindLater = () => {setIsVisible(false)
 onDismiss()}

 const handleDismissToday = () => {dismissForToday()
 setIsVisible(false)
 onDismiss()}

 const handleLogWeight = () => {setIsVisible(false)
 onLogWeight()}

 if (!isVisible) return null

 return (<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
 ...
 {patientName && (<p className="text-sm text-muted-foreground mt-1">
 ...
 {reminderStatus.nextDueDate && !reminderStatus.isOverdue && (<p className="text-xs text-muted-foreground mt-2">

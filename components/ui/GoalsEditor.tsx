...
interface GoalsEditorProps {isOpen: boolean
 onClose: () => void
 userProfile: UserProfileData | null
 currentWeight: number
 onSuccess?: () => void}

export function GoalsEditor({isOpen, onClose, userProfile, currentWeight, onSuccess}: GoalsEditorProps) {const [loading, setLoading] = useState(false)
 ...
 const calculateSuggestions = () => {if (!userProfile?.profile || !userProfile?.goals) return null

 const profile = userProfile.profile
 const goals = userProfile.goals

 // Recalculate BMR using current weight instead of start weight
 const bmr = calculateBMR({weight: currentWeight,
 ...
 const suggestedCalories = calculateCalorieTarget({tdee,
 ...
 useEffect(() => {if (userProfile?.goals) {setTargetWeight(userProfile.goals.targetWeight || 0)
 setWeeklyGoal(userProfile.goals.weeklyWeightLossGoal || 1)
 setCalorieGoal(userProfile.goals.dailyCalorieGoal || 2000)}}, [userProfile])

 const handleRecalculate = () => {setShowSuggestion(true)
 const suggestions = calculateSuggestions()
 if (suggestions) {setCalorieGoal(suggestions.suggestedCalories)
 toast.success('Goals recalculated based on your current weight!')}}

 const handleSave = async () => {if (!userProfile) return

 setLoading(true)
 try {// Recalculate BMR/TDEE with new values
 const suggestions = calculateSuggestions()

 await userProfileOperations.updateUserProfile({goals: {targetWeight,
 weeklyWeightLossGoal: weeklyGoal,
 ...
 onSuccess?.()
 onClose()} catch (error) {logger.error('Failed to update goals:', error as Error)
 toast.error('Failed to update goals. Please try again.')} finally {setLoading(false)}}
 ...
 {showSuggestion && suggestions && (<div className="bg-indigo-100 /20 border border-accent rounded-lg p-4">
 ...
 className="form-input w-full"min="100"max="500"step="0.1"/>
 </div>
 ...
 <input
 type="range"value={weeklyGoal}
 onChange={(e) => setWeeklyGoal(parseFloat(e.target.value))}
 className="w-full"min="0.5"max="2"step="0.1"/>
 ...
 <input
 type="number"value={calorieGoal}
 readOnly
 className="form-input w-full bg-muted cursor-not-allowed"min="1200"max="4000"step="50"/>
 ...
 {/* Safety Warning */}
 {calorieGoal < 1200 && (<div className="bg-error-light /20 border border-error rounded-lg p-3">
 ...
 <button
 onClick={handleSave}
 className="btn btn-primary flex-1"disabled={loading}
 >

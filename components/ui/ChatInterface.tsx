  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-background min-h-[400px] max-h-[600px]">
        ...
        {/* Error Message */}
        {error && (
          <div className="bg-error-light dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-error-dark dark:text-red-300 px-4 py-3 rounded-lg text-sm">
            ⚠️ {error}
          </div>
        )}
      ...
      <div className="bg-card p-4 rounded-b-lg border-t">
        <div className="flex gap-2">
          <input
            ...
            disabled={isSending}
            className="flex-1 px-4 py-2 border border-border dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
          />
          <button
            onClick={handleSend}
...
export function ChatWidget({ userId, onExpand }: ChatWidgetProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 w-96 bg-card rounded-lg shadow-2xl z-50 overflow-hidden">
      {/* Close Button */}
      <button
        onClick={() => setIsOpen(false)}
        className="absolute top-2 right-2 text-white hover:bg-background dark:hover:bg-gray-900/20 rounded-full p-2 z-10"

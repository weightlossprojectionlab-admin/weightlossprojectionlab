...
import {shareRecipe,
 ...
 shareViaWebShareAPI,
 canUseWebShare,
 type AspectRatio} from'@/lib/recipe-share-utils'
...
import {checkIngredientsAgainstInventory,
 checkIngredientsWithQuantities,
 calculateRecipeReadiness,
 type IngredientMatchResult} from'@/lib/ingredient-matcher'
 ...
 */
function parseIngredient(ingredient: string): {quantity: string; ingredientName: string} {// Common measurement patterns: numbers, fractions, units
 ...
 if (match) {return {quantity: match[1].trim(),
...
interface RecipeModalProps {suggestion: MealSuggestion
 isOpen: boolean
 onClose: () => void
 userDietaryPreferences?: string[]
 userAllergies?: string[]}

export function RecipeModal({suggestion, isOpen, onClose, userDietaryPreferences, userAllergies}: RecipeModalProps) {const router = useRouter()
 ...
 useEffect(() => {if (isOpen && suggestion.ingredients && allInventoryItems.length > 0) {// Legacy simple matching (for backward compatibility)
 const matches = checkIngredientsAgainstInventory(suggestion.ingredients,
 allInventoryItems)
 setInventoryMatches(matches)

 // New quantity-aware matching
 const quantityResults = checkIngredientsWithQuantities(suggestion.ingredients,
 allInventoryItems)
 setIngredientResults(quantityResults)

 // Auto-check ingredients we have enough of
 const enoughIndices = new Set<number>()
 quantityResults.forEach((result, idx) => {if (result.hasEnough === true) {enoughIndices.add(idx)}})
 setHaveIngredients(enoughIndices)}}, [isOpen, suggestion.ingredients, allInventoryItems])

 // Cleanup on unmount
 useEffect(() => {return () => {sessionCreationRef.current = false}}, [])

 // Scale recipe based on current serving size
 const scaledRecipe = useMemo<ScaledRecipe>(() => {return scaleRecipe(suggestion, servingSize)}, [suggestion, servingSize])

 // Calculate adjusted prep time
 const adjustedPrepTime = useMemo(() => {return calculateAdjustedPrepTime(suggestion.prepTime, suggestion.servingSize, servingSize)}, [suggestion.prepTime, suggestion.servingSize, servingSize])

 // Get action label (Cook vs Prepare)
 const actionLabel = useMemo(() => {return getRecipeActionLabel(suggestion)}, [suggestion])

 if (!isOpen) return null

 const incrementServings = () => {setServingSize(prev => Math.min(prev + 1, 8)) // Max 8 servings}

 const decrementServings = () => {setServingSize(prev => Math.max(prev - 1, 1)) // Min 1 serving}

 const resetServings = () => {setServingSize(getServingSize(suggestion.servingSize))}

 const toggleIngredientSubstitutions = (index: number) => {setExpandedIngredient(prev => prev === index ? null : index)}

 const handleSwapIngredient = (ingredientIndex: number, substitution: Substitution) => {setSwappedIngredients(prev => {const newMap = new Map(prev)
 newMap.set(ingredientIndex, substitution)
 return newMap})
 // Collapse the substitutions after swapping
 setExpandedIngredient(null)}

 const handleResetIngredient = (ingredientIndex: number) => {setSwappedIngredients(prev => {const newMap = new Map(prev)
 newMap.delete(ingredientIndex)
 return newMap})}

 const toggleHaveIngredient = (ingredientIndex: number) => {setHaveIngredients(prev => {const newSet = new Set(prev)
 if (newSet.has(ingredientIndex)) {newSet.delete(ingredientIndex)} else {newSet.add(ingredientIndex)}
 return newSet})}

 const handleSelectAllIngredients = () => {const allIndices = scaledRecipe.scaledIngredients.map((_, idx) => idx)
 setHaveIngredients(new Set(allIndices))}

 const handleClearAllIngredients = () => {setHaveIngredients(new Set())}

 /**
 * Check if ingredient is on shopping list
 */
 const isIngredientOnShoppingList = (ingredientText: string): boolean => {if (!shoppingListItems || shoppingListItems.length === 0) return false

 const ingredientLower = ingredientText.toLowerCase()

 return shoppingListItems.some(item => {// Check if item is needed (on shopping list)
 if (!item.needed) return false

 // Match by manual ingredient name (exact recipe text)
 if (item.manualIngredientName?.toLowerCase() === ingredientLower) {return true}

 // Match by product name (for similar items)
 if (item.productName?.toLowerCase().includes(ingredientLower.split('').slice(-1)[0])) {return true}

 // Match if this recipe added the item (check recipeIds array)
 if (item.recipeIds && item.recipeIds.includes(suggestion.id)) {const itemNameWords = item.productName?.toLowerCase().split('') || []
 ...
 const handleShareClick = () => {setShowTemplateSelector(true)}

 const handleTemplateSelect = (template: TemplateStyle) => {setSelectedTemplate(template)
 setShowTemplateSelector(false)
 setShowAspectRatioSelector(true)}

 const handleAspectRatioSelect = async (aspectRatio: AspectRatio) => {setSelectedAspectRatio(aspectRatio)
 setShowAspectRatioSelector(false)

 try {const {imageBlob, caption, shareUrl} = await shareRecipe(suggestion, aspectRatio, selectedTemplate)

 // Try Web Share API first (mobile)
 if (canUseWebShare()) {const shared = await shareViaWebShareAPI(suggestion, imageBlob, caption, shareUrl)
 if (shared) {setShareStatus('Shared!')
 ...
 if (copied) {setShareStatus('Image downloaded & link copied!')
 setTimeout(() => setShareStatus(''), 3000)}

 // Show platform menu
 setShowShareMenu(true)} catch (error) {logger.error('Share failed:', error as Error)
 setShareStatus('Share failed')
 setTimeout(() => setShareStatus(''), 2000)}}

 const handlePlatformShare = async (platform:'facebook' |'twitter' |'pinterest' |'whatsapp') => {try {const {caption, shareUrl} = await shareRecipe(suggestion, selectedAspectRatio, selectedTemplate)
 const platformUrl = getPlatformShareUrl(platform, shareUrl, caption)

 if (platformUrl) {window.open(platformUrl,'_blank','width=600,height=400')}

 setShowShareMenu(false)} catch (error) {logger.error('Platform share failed:', error as Error)}}

 const handleCookNow = async () => {// Prevent duplicate clicks
 if (startingSession || sessionCreationRef.current) {return}

 if (!suggestion.recipeSteps || suggestion.recipeSteps.length === 0) {toast.error('This recipe doesn\'t have cooking instructions yet')
 return}
 ...
 if (missingItems.length > 0) {// Block cooking! User must have all ingredients
 ...
 try {// Create cooking session
 const stepTimers = createStepTimers(suggestion.recipeSteps)

 const session = await cookingSessionOperations.createCookingSession({recipeId: suggestion.id,
 ...
 onClose()} catch (error) {logger.error('Error starting cooking session:', error as Error)
 toast.error('Failed to start cooking session')
 // Reset on error so user can retry
 sessionCreationRef.current = false
 setStartingSession(false)}}

 const handleAddToQueue = async () => {try {await recipeQueueOperations.addToQueue({recipeId: suggestion.id,
 recipeName: suggestion.name,
 servingSize: servingSize,
 mealType: selectedMealType})
 ...
 onClose()} catch (error) {logger.error('Error adding to queue:', error as Error)
 toast.error('Failed to add recipe to queue')}}

 const handleAddMissingToShoppingList = async () => {if (!auth.currentUser?.uid) {toast.error('You must be logged in to add items')
 return}

 setAddingToShoppingList(true)

 try {// Get ingredients that are missing or insufficient (based on quantity-aware matching)
 const itemsToAdd = scaledRecipe.scaledIngredients
 .map((ingredient, idx) => ({ingredient, idx, result: ingredientResults[idx]}))
 .filter(({result}) => {// Add if not matched at all, or matched but don't have enough
 return !result?.matched || result.hasEnough === false})

 if (itemsToAdd.length === 0) {toast('You already have all ingredients!', {icon:'✅'})
 ...
 for (const {ingredient} of itemsToAdd) {// Check if ingredient already exists on shopping list
 const existingItem = await findExistingIngredientByName(auth.currentUser!.uid, ingredient)

 if (existingItem) {// Item exists - append current recipe ID to its recipeIds array
 await appendRecipeToIngredient(existingItem.id, suggestion.id)
 linkedItemsCount++} else {// Item doesn't exist - create new item with recipeIds array
 await addManualShoppingItem(auth.currentUser!.uid, ingredient, {recipeId: suggestion.id, // This will be converted to recipeIds: [id]
 quantity: 1,
 priority:'medium'})
 newItemsCount++}}
 ...
 setTimeout(() => {setRecentlyAddedIngredients(new Set())}, 3000)

 // Refresh shopping list to show updated status
 await refreshShoppingList()

 // Show smart toast message based on what happened
 if (linkedItemsCount > 0 && newItemsCount > 0) {toast.success(`✓ Added ${newItemsCount} new item${newItemsCount > 1 ?'s' :''}, linked ${linkedItemsCount} existing item${linkedItemsCount > 1 ?'s' :''} to this recipe!`)} else if (linkedItemsCount > 0) {toast.success(`✓ Linked ${linkedItemsCount} item${linkedItemsCount > 1 ?'s' :''} already on your list to this recipe!`)} else {toast.success(`✓ Added ${newItemsCount} item${newItemsCount > 1 ?'s' :''} to shopping list!`)}

 setActiveTab('recipe') // Switch back to recipe tab} catch (error) {logger.error('Error adding to shopping list:', error as Error)
 toast.error('Failed to add items to shopping list')} finally {setAddingToShoppingList(false)}}

 /**
 * Handle barcode scan for missing ingredients
 * Scans items one by one, tracks progress, auto-proceeds when all items scanned
 */
 const handleBarcodeScan = async (barcode: string) => {try {toast.loading('Looking up product...', {id:'barcode-scan'})

 // Lookup product
 const response = await lookupBarcode(barcode)
 const product = simplifyProduct(response)

 if (!product.found) {toast.error(`Product not found (barcode: ${barcode})`, {id:'barcode-scan'})
 return}

 // Add to inventory
 const existing = allInventoryItems.find(item => item.barcode === barcode)

 if (existing) {await updateItem(existing.id, {inStock: true,
 quantity: existing.quantity + 1,
 needed: false})} else {await addItem(response.product!, {inStock: true,
 needed: false,
 quantity: 1})}
 ...
 if (newScannedCount >= missingIngredients.length) {// All items scanned! Proceed to cooking
 ...
 setTimeout(() => {setShowCookingOptions(true)}, 500)} else {// More items to scan
 toast(`Progress: ${newScannedCount} of ${missingIngredients.length} items scanned`, {icon:'📊',
 duration: 2000})}} catch (error: any) {logger.error('Barcode scan error', error as Error)
 toast.error(error?.message ||'Failed to process barcode', {id:'barcode-scan'})}}

 /**
 * Start scanning missing ingredients
 */
 const handleStartScanning = () => {setScanningInProgress(true)
 ...
 <button
 onClick={onClose}
 className="text-muted-foreground hover:text-foreground transition-colors p-2"aria-label="Close modal">
 ...
 className={`px-4 py-3 font-medium transition-colors ${activeTab ==='recipe'
 ...
 className={`px-4 py-3 font-medium transition-colors ${activeTab ==='shopping'
 ?'text-primary border-b-2 border-primary'
 :'text-muted-foreground hover:text-foreground'}`}
 >
 Shopping List
 ...
 {activeTab ==='recipe' ? (<div className="space-y-6">
 {/* Serving Size Selector */}
 <div className="bg-gradient-to-r from-purple-100 to-blue-50 /30 /20 border-2 border-primary rounded-lg p-4">
 ...
 className="w-8 h-8 rounded-full bg-primary text-white font-bold hover:bg-primary-hover disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed transition-colors"aria-label="Decrease servings">
 −
 </button>
 ...
 className="w-8 h-8 rounded-full bg-primary text-white font-bold hover:bg-primary-hover disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed transition-colors"aria-label="Increase servings">
 +
 </button>
 </div>
 </div>
 {servingSize !== suggestion.servingSize && (<div className="mt-3 text-center">
 <button
 ...
 {/* Dietary Tags */}
 {suggestion.dietaryTags && suggestion.dietaryTags.length > 0 && (<div>
 <h3 className="font-semibold text-foreground mb-2">Dietary Info</h3>
 <div className="flex flex-wrap gap-2">
 {suggestion.dietaryTags.map(tag => (<span key={tag} className="text-xs bg-primary-light /20 text-primary px-2 py-1 rounded">
 {tag}
 </span>))}
 </div>
 </div>)}
 ...
 {/* Allergens */}
 {suggestion.allergens?.length > 0 && (<div>
 <h3 className="font-semibold text-foreground mb-2">Allergen Warning</h3>
 <div className="bg-orange-50 /20 border border-orange-200 rounded-lg p-3">
 <p className="text-sm font-medium text-orange-800 mb-1">Contains:</p>
 <div className="flex flex-wrap gap-1">
 {suggestion.allergens.map(allergen => (<span key={allergen} className="text-xs bg-orange-100 /20 text-orange-800 px-2 py-1 rounded">
 {allergen}
 </span>))}
 </div>
 </div>
 ...
 {ingredientResults.length > 0 && (() => {const readiness = calculateRecipeReadiness(suggestion.ingredients || [], allInventoryItems)
 return (<div className={`mb-3 p-3 rounded-lg border-2 ${readiness.canMake
 ?'bg-success-light /20 border-green-500'
 :'bg-orange-50 /20 border-orange-500'}`}>
 <div className="flex items-center justify-between">
 <div>
 <p className={`text-sm font-semibold ${readiness.canMake
 ?'text-green-900'
 :'text-orange-900'}`}>
 ...
 {!readiness.canMake && (<span className="text-xs px-2 py-1 bg-orange-200 text-orange-900 rounded font-medium">
 Shop needed
 </span>)}
 </div>
 </div>)})()}
 {swappedIngredients.size > 0 && (<div className="mb-3 bg-secondary-light border border-secondary-light rounded-lg px-3 py-2">
 ...
 {scaledRecipe.scaledIngredients.map((ingredient, idx) => {const swappedSub = swappedIngredients.get(idx)
 const isSwapped = !!swappedSub

 // Parse ingredient to preserve quantity when swapping
 let displayIngredient = ingredient
 if (isSwapped) {const parsed = parseIngredient(ingredient)
 displayIngredient = parsed.quantity
 ?`${parsed.quantity} ${swappedSub.name}`: swappedSub.name}

 const substitutions = getSubstitutionSuggestions(ingredient,
 ...
 return (<div key={idx} className="border border-border rounded-lg overflow-hidden">
 {/* Main Ingredient */}
 <div className={`p-3 flex items-start ${isSwapped ?'bg-primary-light /20' :''} transition-colors`}>
 {/* Checkbox - I have this */}
 <div className="flex items-center mr-2">
 <input
 type="checkbox"checked={hasIngredient}
 onChange={() => toggleHaveIngredient(idx)}
 className="w-4 h-4 text-success border-border rounded focus:ring-success cursor-pointer"onClick={(e) => e.stopPropagation()}
 />
 </div>
 ...
 {isSwapped && swappedSub.ratio && (<span className="text-xs text-muted-foreground ml-2">({swappedSub.ratio})</span>)}
 {isSwapped && swappedSub.notes && (<p className="text-xs text-muted-foreground mt-1">{swappedSub.notes}</p>)}
 {/* Show quantity-aware inventory status */}
 {ingredientResults[idx] && (<div className="mt-1">
 {ingredientResults[idx].hasEnough === true && (<p className="text-xs text-success-dark flex items-center gap-1">
 <span>✓</span>
 <span>{ingredientResults[idx].comparison}</span>
 </p>)}
 {ingredientResults[idx].hasEnough === false && ingredientResults[idx].matched && (<p className="text-xs text-orange-700 flex items-center gap-1">
 <span>⚠️</span>
 <span>{ingredientResults[idx].comparison}</span>
 </p>)}
 {!ingredientResults[idx].matched && (<p className="text-xs text-error-dark flex items-center gap-1">
 <span>❌</span>
 <span>{ingredientResults[idx].comparison}</span>
 </p>)}
 {ingredientResults[idx].hasEnough === null && ingredientResults[idx].matched && (<p className="text-xs text-muted-foreground flex items-center gap-1">
 <span>ℹ️</span>
 <span>{ingredientResults[idx].comparison}</span>
 </p>)}
 </div>)}
 ...
 className="text-xs text-muted-foreground hover:text-foreground underline"aria-label="Reset to original">
 Reset
 </button>
 </>)}
 {hasSubstitutions && !isSwapped && (<button
 className="text-primary text-xs font-medium flex items-center space-x-1"aria-label="Show substitutions">
 ...
 {/* Substitutions */}
 {hasSubstitutions && isExpanded && !isSwapped && (<div className="bg-muted border-t border-border p-3">
 <p className="text-xs font-semibold text-muted-foreground mb-2">Alternatives:</p>
 <div className="space-y-2">
 {substitutions.map((sub, subIdx) => (<div key={subIdx} className="bg-card rounded-md p-2 border border-border hover:border-primary transition-colors group">
 <div className="flex items-start justify-between">
 <div className="flex-1">
 <p className="text-sm font-medium text-foreground">{sub.name}</p>
 {sub.ratio && (<p className="text-xs text-muted-foreground">Ratio: {sub.ratio}</p>)}
 {sub.notes && (<p className="text-xs text-muted-foreground mt-1">{sub.notes}</p>)}
 <div className="flex flex-wrap gap-1 mt-1">
 {sub.dietaryTags.slice(0, 2).map(tag => (<span key={tag} className="text-[10px] bg-primary-light /20 text-primary px-1.5 py-0.5 rounded">
 {tag}
 </span>))}
 </div>
 </div>
 ...
 {suggestion.recipeSteps && suggestion.recipeSteps.length > 0 && (() => {// Check if user has ALL ingredients
 ...
 {hasAllIngredients ? (/* Show instructions when user has everything */
 <ol className="space-y-3">
 {suggestion.recipeSteps.map((step, idx) => (<li key={idx} className="text-sm text-foreground flex items-start">
 <span className="font-bold text-primary mr-3 min-w-[24px]">{idx + 1}.</span>
 <span>{step}</span>
 </li>))}
 </ol>) : (/* Show locked state when missing ingredients */
 <div className="bg-orange-50 /20 border-2 border-orange-300 rounded-lg p-6">
 <div className="text-center mb-4">
 <div className="text-4xl mb-2">🔒</div>
 <h4 className="font-semibold text-orange-900 text-lg mb-1">
 Instructions Locked
 </h4>
 ...
 <div
 className="bg-orange-500 h-2 rounded-full transition-all"style={{width:`${(availableCount / totalIngredientsCount) * 100}%`}}
 />
 </div>
 </div>
 ...
 {suggestion.cookingTips && suggestion.cookingTips.length > 0 && (() => {// Check if user has ALL ingredients (same check as instructions)
 const hasAllIngredients = ingredientResults.length > 0 &&
 ingredientResults.filter(r => !r.matched || r.hasEnough === false).length === 0
 ...
 {suggestion.cookingTips.map((tip, idx) => (<p key={idx} className="text-sm text-blue-900 flex items-start">
 ...
 {/* No Recipe Available */}
 {(!suggestion.recipeSteps || suggestion.recipeSteps.length === 0) && (<div className="bg-muted rounded-lg p-4 text-center">
 ...
 {neededCount > 0 && (() => {// Check how many needed items are already on shopping list
 const itemsOnList = neededIngredients.filter(({ingredient}) =>
 isIngredientOnShoppingList(ingredient)).length
 const allOnList = itemsOnList === neededCount

 if (allOnList) {// All missing items are on shopping list
 return (<div className="w-full mb-4 px-4 py-3 bg-secondary-light border-2 border-blue-500 rounded-lg text-center">
 ...
 ?'Adding...'
 : itemsOnList > 0
 ?`Add ${remainingCount} More Item${remainingCount > 1 ?'s' :''} to Shopping List`:`Add ${neededCount} Missing Item${neededCount > 1 ?'s' :''} to Shopping List`}
 </span>
 </button>)})()}

 {/* Shopping List - Only items they need */}
 {neededCount === 0 ? (<div className="bg-success-light border border-success rounded-lg p-6 text-center">
 <div className="text-4xl mb-2">🎉</div>
 ...
 {neededIngredients.map(({ingredient, idx, result}) => {const swappedSub = swappedIngredients.get(idx)
 const isSwapped = !!swappedSub
 ...
 return (<div key={idx} className={`p-3 rounded-lg ${isSwapped
 ?'bg-primary-light /20'
 : result && result.matched && result.hasEnough === true
 ?'bg-success-light /10' // Green background for in-stock
 : isIngredientOnShoppingList(ingredient)
 ?'bg-amber-50 /10' // Amber background for on list
 :'bg-muted' // Default}`}>
 <div className="flex items-start space-x-3">
 <input
 type="checkbox"checked={(result && result.matched && result.hasEnough === true) // Green: In inventory
 || isIngredientOnShoppingList(ingredient) // Amber: On shopping list}
 readOnly
 className={`w-4 h-4 mt-0.5 rounded focus:ring-0 pointer-events-none ${result && result.matched && result.hasEnough === true
 ?'text-green-500 border-green-500 bg-success-light0' // Green checkbox
 : isIngredientOnShoppingList(ingredient)
 ?'text-amber-500 border-amber-500 bg-amber-500' // Amber checkbox
 :'border-border' // Default unchecked}`}
 />
 <div className="flex-1">
 ...
 {result && result.hasEnough === false && result.deficit && (<p className="text-xs text-orange-700 mt-1">
 ⚠️ Need {result.deficit.quantity} {result.deficit.unit} more
 </p>)}
 {result && !result.matched && (<>
 {/* Check if on shopping list */}
 {isIngredientOnShoppingList(ingredient) ? (<div className="text-xs mt-1 flex items-center gap-1.5">
 <button
 onClick={(e) => {e.stopPropagation() // Prevent modal from closing
 router.push('/shopping')}}
 className="text-blue-700 hover:text-blue-900 hover:underline cursor-pointer flex items-center gap-1.5 transition-colors"aria-label="View shopping list">
 <span>📋</span>
 <span>On Shopping List</span>
 </button>
 {recentlyAddedIngredients.has(ingredient) && (<span className="text-success font-medium">✓ Just Added</span>)}
 </div>) : (<p className="text-xs text-error-dark mt-1">
 ❌ Not in stock
 </p>)}
 </>)}
 </div>
 {isSwapped && (<span className="text-[10px] bg-success text-white px-1.5 py-0.5 rounded">
 Swapped
 </span>)}
 </div>
 </div>)})}
 ...
 {suggestion.recipeSteps && suggestion.recipeSteps.length > 0 && (<button
 onClick={() => setShowCookingOptions(true)}
 className="px-4 py-2 bg-success text-white rounded-lg hover:bg-success-hover transition-colors font-medium flex items-center space-x-2"disabled={startingSession}
 >
 ...
 {shareStatus && (<div className="absolute bottom-full left-0 mb-2 px-3 py-1 bg-success text-white text-xs rounded shadow-lg whitespace-nowrap">
 {shareStatus}
 </div>)}
 {/* Template Selector */}
 {showTemplateSelector && (<div className="absolute bottom-full left-0 mb-2 bg-card border border-border rounded-lg shadow-xl p-3 min-w-[250px] z-50">
 <p className="text-xs font-semibold text-muted-foreground mb-3">Choose design style:</p>
 <div className="space-y-2">
 {getAvailableTemplates().map((template) => (<button
 key={template.style}
 ...
 {selectedTemplate === template.style && (<svg className="w-5 h-5 text-primary"fill="currentColor"viewBox="0 0 20 20">
 ...
 {showAspectRatioSelector && (<div className="absolute bottom-full left-0 mb-2 bg-card border border-border rounded-lg shadow-xl p-3 min-w-[200px]">
 ...
 <div className="w-8 h-8 border-2 border-primary rounded bg-primary-light /20 group-hover:bg-primary-hover"></div>
 </button>
 <button
 ...
 <div className="w-5 h-8 border-2 border-primary rounded bg-primary-light /20 group-hover:bg-primary-hover"></div>
 </button>
 <button
 ...
 <div className="w-10 h-6 border-2 border-primary rounded bg-primary-light /20 group-hover:bg-primary-hover"></div>
 </button>
 </div>
 </div>)}
 {showShareMenu && (<div className="absolute bottom-full left-0 mb-2 bg-card border border-border rounded-lg shadow-xl p-2 space-y-1 min-w-[150px]">
 <button
 ...
 {showCookingOptions && (<div className="absolute inset-0 bg-black/60 flex items-center justify-center p-4 z-10">
 ...
 {(['breakfast','lunch','dinner','snack'] as const).map(type => (<button
 key={type}
 onClick={() => setSelectedMealType(type)}
 className={`px-4 py-2 rounded-lg font-medium transition-colors capitalize ${selectedMealType === type
 ...
 className="w-full px-6 py-3 bg-muted text-foreground rounded-lg hover:bg-gray-200 transition-colors font-medium"disabled={startingSession}
 >
 Cancel
 </button>
 </div>
 ...
 {showMissingIngredientsModal && (<div className="absolute inset-0 bg-black/60 flex items-center justify-center p-4 z-10">
 <div className="bg-card border border-border rounded-lg shadow-xl max-w-md w-full p-6">
 <div className="text-center mb-4">
 <div className="text-4xl mb-2">🛒</div>
 ...
 {/* Missing Items List */}
 <div className="bg-orange-50 /20 border border-orange-200 rounded-lg p-4 mb-4 max-h-48 overflow-y-auto">
 <p className="text-xs font-semibold text-orange-800 mb-2">
 Missing ({missingIngredients.length}):
 </p>
 <ul className="space-y-1">
 {missingIngredients.map((ingredient, idx) => (<li key={idx} className="text-sm text-orange-900 flex items-start">
 ...
 <p className="text-xs text-blue-900">
 💡 <strong>Smart tip:</strong> {missingIngredients.length <= 3 ?'Just a few items - quick store trip?' :'Several items needed - plan your shopping!'}
 </p>
 </div>
 ...
 <button
 onClick={async () => {setShowMissingIngredientsModal(false)
 ...
 className="w-full px-6 py-3 bg-muted text-foreground rounded-lg hover:bg-gray-200 transition-colors font-medium"disabled={addingToShoppingList}
 >
 Cancel
 </button>
 </div>
 ...
 context="inventory"title={`Scan Ingredients (${scannedItems.size}/${missingIngredients.length})`}
 />
 </div>
 </div>)}
